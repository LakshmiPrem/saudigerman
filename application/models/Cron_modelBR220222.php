        ...
        foreach ($documents as $document) {
            if ($document['expiry_date'] != null
				...
				/*$sclient= $this->db->get_where('tblprojects',array('id'=>$document['project_id']))->row()->clientid;
					$staff_email= $this->db->get_where('tblstaff',array('staffid'=>$document['staffid']))->row()->email;
       ...
       $this->db->where('is_notified', 0);
        ...
        foreach ($documents as $document) {
            if ($document['installment_date'] != null
                ...
                $reminder_before        = 0;//get_option('send_installment_expiry_reminder_before');
                ...
                if ($diff <= $reminder_before ){
                    ...
                    $notificationLink = 'projects/view/'.$document['recovery_id'].'?group=project_settlement';
                  //  $notificationLink = ltrim($notificationLink, '/');
					 $project = $this->get($document['recovery_id']);
                    ...
                    if ($notified) {
                       array_push($notifiedUsers, $member['staff_id']);
							...
							send_mail_template('project_installment_duedate_reminder', $member['email'], $member['staff_id'], $member['recovery_id']);
					 }
				}
			}
		}
