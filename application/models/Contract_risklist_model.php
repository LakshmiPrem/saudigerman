...
class Contract_risklist_model extends App_Model
{
    ...
    */
    public function add($data)
    {
        ...
        if ($insert_id) {
            log_activity('New Contract risk Added [' . $data['name'] . ']');
    ...
    public function get($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix().'risk_checklists')->row();
        }

        $types = $this->app_object_cache->get('risk_checklists');

        if (!$types && !is_array($types)) {
            $types = $this->db->get(db_prefix().'risk_checklists')->result_array();
            $this->app_object_cache->add('risk_checklists', $types);
        }
    ...
    {
        if (is_reference_in_table('risk_checklists', db_prefix().'contracts', $id)) {
            return [
                'referenced' => true,
            ];
        }
