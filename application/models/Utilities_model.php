...
class Utilities_model extends App_Model
{
    ...
    public function get_calendar_data($start, $end, $client_id = '', $contact_id = '', $filters = false)
    {
        ...
        if (get_option('show_projects_on_calendar') == 1 && !$ff || $ff && array_key_exists('projects', $filters)) {
            ...
            $this->db->select('subject as title,tblhearings.id,hearing_date,clientid,company,tblprojects.id as case_id');

            $this->db->from('tblhearings');
            $this->db->where("DATE(hearing_date) BETWEEN '$start' AND '$end'");
            ...
            foreach ($cases as $case) {
                ...
                $name                = $case['title'];
                $_cases['title']    = $name;
                $_cases['color']    = 'green';//get_option('calendar_casediary_color');
                $_cases['_tooltip'] = _l('hearings') . ' - ' . $name . $rel_showcase;
                if (!$client_data) {
                    $_cases['url'] = admin_url('projects/view/' . $case['case_id'].'?group=hearings');
                   // $_cases['url'] = admin_url('casediary/view/' . $c);
                } else {
                    $_cases['url'] = site_url('clients/projects/' . $case['id']);
                }

                $_cases['date'] = $case['hearing_date'];
                array_push($data, $_cases);
            }
