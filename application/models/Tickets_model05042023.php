...
class Tickets_model extends App_Model
{
        ...
        if (!is_admin()) {
            ...
            if (get_option('staff_access_only_assigned_departments') == 1) {
                ...
                if (count($staff_deparments_ids) == 0) {
                    $where= 'AND tbltickets.userid IN (SELECT customer_id FROM tblcustomer_admins WHERE staff_id=' . get_staff_user_id() . ')';
    ...
    public function get($id = '', $where = [])
    {
		...
		$this->db->join(db_prefix() . 'oppositeparty', db_prefix() . 'oppositeparty.id = ' . db_prefix() . 'tickets.opposteparty', 'left');
    ...
    public function get_ticket_by_id($id, $userid = '')
    {
        ...
        if ($ticket) {
            ...
            $ticket->approvals = get_approvals($ticket->ticketid,'ticket');
 ...
 public function get_ticket_tasks($id)
    {
		 $this->db->select('*, ' . db_prefix() . 'task_assigned.staffid ');
    ...
    public function add_reply($data, $id, $admin = null, $pipe_attachments = false)
    {
		...
		if (isset($data['estimated_date'])) {
			$estimated_date = to_sql_date($data['estimated_date']);
            unset($data['estimated_date']);
        } 
        ...
        if ($insert_id) {
            ...
            if ($ticket->userid != 0 && $ticket->contactid != 0) {
                // print_r($ticket->admin);
                if($ticket->admin!=0){
                  
                $email=get_staff($ticket->admin)->email;
    ...
    public function add($data, $admin = null, $pipe_attachments = false)
    {
		...
		$oppositeparty = '';
        if (isset($data['opposite_party'])) {
            $oppositeparty = $data['opposite_party'];
        ...
        if ($ticketid) {
            ...
            if ($admin == null) {
                ...
                pusher_trigger_notification($notifiedUsers);
				 ...
				 $notifiedUsers1                              = [];
                    $ticket_assignees = json_decode(get_option('auto_ticketmail_staffs'),true);
				//print_r($ticket_assignees);
                     foreach ($ticket_assignees as $assignee) {
						
                     if ($assignee != get_staff_user_id()) {
                    $notified = add_notification([
                        'description'     => 'not_new_ticket_created',
                        'touserid'        => $assignee,
                        'fromcompany'     => 1,
                    ...
                    if ($notified) {
                        ...
                        array_push($notifiedUsers1, $assignee);
                    }
                     $this->db->select('email');
                                $this->db->where('staffid', $assignee);//['assigneeid']);
                                $email = $this->db->get(db_prefix() . 'staff')->row()->email;
						 	
                    send_mail_template('ticket_assigned_to_staff', get_staff($assignee)->email, $data['assigned'], $ticketid, $data['userid'], $data['contactid']);
                }
            }
				
				 pusher_trigger_notification($notifiedUsers1);
            }
    ...
    public function update_single_ticket_settings($data)
    {
		...
		if ($data['estimated_date'] == '') {
            ...
            $data['estimated_date'] = to_sql_date($data['estimated_date']);
        }
    ...
    public function update_single_ticket_approvals($data){
        
        $insert['rel_id'] = $data['ticketid'];
        $insert['rel_type'] = 'ticket';
        $apprival_types = get_approval_types('ticket');
        $notifiedUsers=[];
        foreach ($data['approval_assigned'] as $key=>$row) {
               $insert['staffid'] = $row;
               $insert['approval_type'] = $apprival_types[$key]['id'];
               $insert['approval_status']=2 ; 
               $insert['addedfrom'] = get_staff_user_id();
               $insert['dateadded'] = date('Y-m-d H:s:s'); 
               $this->db->insert('tblapprovals',$insert);
         
                $ticket = $this->get_ticket_by_id($data['ticketid']);
                $notified = add_notification([
    ...
    public function change_approval_status($id, $status)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            if($status == 3){
                $nextid = $id+1;
                $notifiedUsers=[];
                $next_approval = $this->db->get_where('tblapprovals',array('id'=>$nextid));
                if($next_approval->num_rows() > 0){
                    $next_approval_data = $next_approval->row();
                    $ticket = $this->get_ticket_by_id($next_approval_data->rel_id);
                    $notified = add_notification([
                        'description'     => 'legal_request_approval',
                        'touserid'        => $next_approval_data->staffid,
                        'fromcompany'     => 1,
                        'fromuserid'      => 0,
                        'link'            => 'tickets/ticket/' . $next_approval_data->rel_id,
                        'additional_data' => serialize([
                            $ticket->subject,
                        ]),
                    ]);
