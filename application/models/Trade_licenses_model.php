...
class Trade_licenses_model extends App_Model
{
    ...
    {
        $this->db->select('*,tbltrade_licenses.id as id, tbltrade_licenses.addedfrom');
        $this->db->where($where);
        //$this->db->join('tblcontracttypes', 'tblcontracttypes.id = tbltrade_licenses.contract_type', 'left');
        $this->db->join('tblclients', 'tblclients.userid = tbltrade_licenses.client');
        if (is_numeric($id)) {
            $this->db->where('tbltrade_licenses.id', $id);
            $contract = $this->db->get('tbltrade_licenses')->row();
        ...
        $contracts = $this->db->get('tbltrade_licenses')->result_array();
        $i         = 0;
    ...
    public function get_contract_attachments($attachment_id = '', $id = '')
    {
        ...
        $this->db->where('rel_type', 'trade_license');
    ...
    public function add($data)
    {
        ...
        $this->db->insert('tbltrade_licenses', $data);
        ...
        if ($insert_id) {
            ...
            log_activity('New Trade License Added [' . $data['license_no'] . ']');
    ...
    public function update($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
          hooks()->apply_filters('after_contract_updated', $id);
            log_activity('Trade License Updated [' . $data['license_no'] . ']');
    ...
    public function copy($id)
    {
        ...
        $fields = $this->db->list_fields('tbltrade_licenses');
    ...
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tbltrade_licenses');
        if ($this->db->affected_rows() > 0) {
            ...
            log_activity('Trade License Deleted [' . $id . ']');
