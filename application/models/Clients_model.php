...
class Clients_model extends App_Model
{
    ...
    {
        $this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . 'clients')) . ',' . get_sql_select_client_company().',(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM '.db_prefix().'customer_groups JOIN '.db_prefix().'customers_groups ON '.db_prefix().'customer_groups.groupid = '.db_prefix().'customers_groups.id WHERE customer_id = '.db_prefix().'clients.userid ORDER by name ASC) as customerGroups');
    ...
    public function update_contact($data, $id, $client_request = false)
    {
        ...
        $data['is_owner'] = isset($data['is_owner']) ? 1 : 0;
        $data['is_partner'] = isset($data['is_partner']) ? 1 : 0;
        $data['is_manager'] = isset($data['is_manager']) ? 1 : 0;
        $data['is_director'] = isset($data['is_director']) ? 1 : 0;
        $data['is_secretary'] = isset($data['is_secretary']) ? 1 : 0;
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('client', $id);
            $documents = $this->db->get(db_prefix() . 'documents')->result_array();
            foreach ($documents as $doc) {
                $this->documents_model->delete($doct['id']);
            }
             $this->db->where('client', $id);
            $documents = $this->db->get(db_prefix() . 'documents_in')->result_array();
            foreach ($documents as $doc) {
                $this->documents_model->delete($doct['id'],'in');
            }
             $this->db->where('client', $id);
            $documents = $this->db->get(db_prefix() . 'communication')->result_array();
            foreach ($documents as $doc) {
                $this->documents_model->delete($doct['id'],'out');
            }
        }
        if ($affectedRows > 0) {
    ...
    */
    ...
    public function delete_contact_profile_image($id)
    {
     ...
     public function fetch_client_details($q,$limit, $start,$where=[])
    {
        ...
        foreach ($clients as $client_) {
            ...
            $number_legals = total_rows('tbltickets',array('userid'=>$client_['userid']));  
			 $number_contracts = total_rows('tblcontracts',array('client'=>$client_['userid']));
            $res .=  ' <a href="'.admin_url('clients/client/'.$client_['userid']).'" >
                        <div class="col-sm-3">
                            <div class="card"  style="box-shadow: 0 2px 5px 0 rgba(255, 0, 0, 0.80), 0 2px 10px 0 rgba(0, 0, 0, 0.62); padding: 10px; margin: 8px; height: 168px;"  >
                                <div class="card-body">
                                    <h5 class="card-title" ><strong>'.$client_['company'].'</strong></h5>
                                    <p class="card-text" style="margin: 0 0 4px;"><b>'._l('city').' :</b>'.$client_['city'].'</p>
                                    <p class="card-text" style="margin:  0 0 4px;"><b>'._l('client_phonenumber').':</b>'.$client_['phonenumber'].'</p>
                                    <p class="card-text" style="margin:  0 0 4px;"><b>'._l('client_no').':</b>'.$client_['client_no'].' &nbsp; </p>';

                              /*  if(isset($primary_contact_id)){ 
                                $res .=  '<img style="float: left;position: absolute; left:160px; " src="'.contact_profile_image_url($primary_contact_id,'thumb').'" id="contact-img" class="staff-profile-image-small">';
                                        }; */
							
                              $res .= '<p class="card-text" style="margin:  0 0 4px;">';
			if(get_option('enable_legaldashboard')==1){
			$res .=' <a href="'.admin_url('clients/client/'.$client_['userid'].'?group=projects').'" title="Number Of Cases" class="btn btn-info" style="border-radius: 12px;"><b>'.$number_cases.'</b></a>';
			}
							   $res .='<a href="'.admin_url('clients/client/'.$client_['userid'].'?group=tickets').'" title="Number Of Legal Requests" class="btn btn-success" style="border-radius: 12px;"><b>'.$number_legals.'</b></a>
							    <a href="'.admin_url('clients/client/'.$client_['userid'].'?group=contracts').'" title="Number Of Contracts" class="btn btn-warning" style="border-radius: 12px;"><b>'.$number_contracts.'</b></a></p>
                                </div>
                            </div>
                        </div></a>';
//<span class="label label inline-block" style="color: black;border: 1px solid #1B8BF9;">'.date('d M Y',strtotime($client_['datecreated'])).'</span>
     ...
     public function fetch_client_details_13042023($q,$limit, $start,$where=[])
    {
        $have_assigned_customers        = have_assigned_customers();
        $have_permission_customers_view = has_permission('customers', '', 'view');
        if ($have_assigned_customers || $have_permission_customers_view) {
        ...
        foreach ($clients as $client_) {
            $primary_contact_id = get_primary_contact_user_id($client_['userid']);
            $number_cases = total_rows('tblcontracts',array('client'=>$client_['userid']));
                        ...
                        <div class="col-sm-3">
                            <div class="card"  style="box-shadow: 0 2px 5px 0 rgba(255, 0, 0, 0.80), 0 2px 10px 0 rgba(0, 0, 0, 0.62); padding: 10px; margin: 8px; height: 168px;"  >
                                <div class="card-body">
                                    ...
                                    <p class="card-text" style="margin: 0 0 4px;"><b>'._l('city').' :</b>'.$client_['city'].'</p>
                                    <p class="card-text" style="margin:  0 0 4px;"><b>'._l('client_phonenumber').':</b>'.$client_['phonenumber'].'</p>
                                    <p class="card-text" style="margin:  0 0 4px;"><b>'._l('client_no').':</b>'.$client_['client_no'].' &nbsp; <span class="label label inline-block" style="color: black;border: 1px solid #1B8BF9;">'.date('d M Y',strtotime($client_['datecreated'])).'</span> </p>';

                                if(isset($primary_contact_id)){ 
                                $res .=  '<img style="float: right;position: absolute; right:30px; " src="'.contact_profile_image_url($primary_contact_id,'thumb').'" id="contact-img" class="staff-profile-image-small">';
                                        }; 

                              $res .= '<p class="card-text" style="margin:  0 0 4px;"> <a href="'.admin_url('clients/client/'.$client_['userid'].'?group=contracts').'" title="Number Of Cases" class="btn btn-info" style="border-radius: 12px;">Contracts: <b>'.$number_cases.'</b></a></p>
                                </div>
                            </div>
                        </div></a>';
     ...
     public function fetch_client_details_num_rows($q,$where=[])
    {
        $have_assigned_customers        = have_assigned_customers();
        $have_permission_customers_view = has_permission('customers', '', 'view');
        if ($have_assigned_customers || $have_permission_customers_view) {
    ...
    //shareholder stake holder adarsh
    public function get_clientshareholders($customer_id = '', $where = ['active' => 1])
    {
        $this->db->where($where);
        if ($customer_id != '') {
            $this->db->where('userid', $customer_id);
        ...
        return $this->db->get(db_prefix() . 'client_shareholder')->result_array();
    }
     ...
     * @return object
     */
    public function get_clientshareholder($id)
    {
        $this->db->where('id', $id);

        return $this->db->get(db_prefix() . 'client_shareholder')->row();
    }
	 /**
     * Add new contact
     * @param array  $data               $_POST data
     ...
     * @param boolean $not_manual_request is manual from admin area customer profile or register, convert to lead
     */
    public function add_shareholder($data, $customer_id)
    {
       
        $data['userid']       = $customer_id;
        $data['datecreated'] = date('Y-m-d H:i:s');

        $this->db->insert(db_prefix() . 'client_shareholder', $data);
        $contact_id = $this->db->insert_id();

        if ($contact_id) {
           log_activity('ShareHolder Created [ID: ' . $contact_id . ']');
            return $contact_id;
        }
	    ...
	    /**
     * Get all customer shareholder
     * @param  string $id
     * @return mixed
     */
    public function get_shareholders($id = '',$where = [])
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);

            return $this->db->get(db_prefix().'shareholders')->row();
        }
		if ((is_array($where) && count($where) > 0) || (is_string($where) && $where != '')) {
            $this->db->where($where);
        }
        ...
        return $this->db->get(db_prefix().'shareholders')->result_array();
    }

    /**
     * Delete customer authority
     * @param  mixed $id group id
     * @return boolean
     */
    ...
    {
      $this->db->where('id', $id);
        $this->db->delete(db_prefix().'shareholders');
        if ($this->db->affected_rows() > 0) {
            $this->db->where('shareholder_id', $id);
            $this->db->delete(db_prefix().'client_shareholder');

            hooks()->do_action('shareholder_deleted', $id);

            log_activity('Customer Shareholder Deleted [ID:' . $id . ']');
	 ...
	 /**
     * Add new customer authority
     * @param array $data $_POST data
     */
    public function new_shareholder($data)
    {
        $this->db->insert(db_prefix().'shareholders', $data);

        $insert_id = $this->db->insert_id();

        if ($insert_id) {
            log_activity('New Shareholder Created [ID:' . $insert_id . ', Name:' . $data['shareholder_name'] . ']');
    ...
    public function edit_shareholder($data)
    {
       $this->db->where('id', $data['id']);
        $this->db->update(db_prefix().'shareholders',$data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Customer Shareholder Updated [ID:' . $data['id'] . ']');
	...
	public function update_shareholder($data, $id)
    {
        $affectedRows = 0;
        $contact      = $this->get_clientshareholder($id);
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'client_shareholder', $data);
	    ...
	    public
	    function delete_clientshareholder( $id ) {
	    	$this->db->where( 'id', $id );
	    	$result = $this->db->get( db_prefix() . 'client_shareholder' )->row();
	    	$customer_id = $result->userid;
     ...
     */
    public function get_constitution_types($id = '')
    {
        if (is_numeric($id)) {
            $this->db->where('id', $id);

            return $this->db->get(db_prefix().'constitution_type')->row();
        }

        $types = $this->app_object_cache->get('constitution-type');

        if (!$types && !is_array($types)) {
            $types = $this->db->get(db_prefix().'constitution_type')->result_array();
            $this->app_object_cache->add('constitution-type', $types);
        }

        return $types;
    }
    public function add_constitution($data, $customer_id)
    {
        ...
        $this->db->insert(db_prefix() . 'client_subfile', $data);
        $contact_id = $this->db->insert_id();

        if ($contact_id) {
           log_activity('Subfile Created [ID: ' . $contact_id . ']');
            return $contact_id;
        }

        return false;
    }
    public function get_clientconstitution($id='',$where=[])
    {
		...
		if (is_numeric($id)) {
			...
			return $this->db->get(db_prefix() . 'client_subfile')->row();
		 }
		
		return $this->db->get(db_prefix() . 'client_subfile')->result_array();
    }
public function change_display_name($id, $newname)
    {
        $this->db->where('id', $id);
...
public function delete_clientconstitution( $id ) {
            $this->db->where( 'id', $id );
            $result = $this->db->get( db_prefix() . 'client_subfile' )->row();
            $customer_id = $result->userid;

            $last_activity = get_last_system_activity_id();

            $this->db->where( 'id', $id );
            $this->db->delete( db_prefix() . 'client_subfile' );
    ...
    public function update_constitution($data, $id)
    {
        $affectedRows = 0;
         $data['issue_date'] = to_sql_date($data['issue_date']);
        if (empty($data['expiry_date'])) {
        ...
        $contact      = $this->get_clientconstitution($id);
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'client_subfile', $data);
