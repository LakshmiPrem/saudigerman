...
class Contract_category_model extends App_Model
{
    ...
    */
    ...
    {
        $this->db->insert(db_prefix().'action_categories', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            log_activity('New Contract Category Added [' . $data['name'] . ']');
     ...
     * @param mixed $data All $_POST data
     * @param mixed $id Contract Category id
     */
    public function update($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix().'action_categories', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('Contract Category Updated [' . $data['name'] . ', ID:' . $id . ']');
     ...
     * @return mixed
     * Get Contract Category object based on passed id if not passed id return array of all types
     */
    public function get($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix().'action_categories')->row();
        }

        $types = $this->app_object_cache->get('contract-types');

        if (!$types && !is_array($types)) {
            $types = $this->db->get(db_prefix().'action_categories')->result_array();
     ...
     * @param  integer ID
     * @return mixed
     * Delete Contract Category from database, if used return array with key referenced
     */
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix().'action_categories');
        if ($this->db->affected_rows() > 0) {
    ...
    /**
     * Get Contract Categorys data for chart
     * @return array
     */
    ...
    /**
     * Get Contract Categorys values for chart
     * @return array
     */
