...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            $this->document_expiry_reminder();
			 $this->creditcontrol_expiry_reminder();
    ...
    private function creditcontrol_expiry_reminder()
    {
        $this->db->select('' . db_prefix() . 'ticket_attachments.*,tbltickets.assigned,tblstaff.email, phonenumber,CONCAT(firstname ," ",lastname) as staff_full_name');
		  $this->db->join(db_prefix() . 'tickets', '' . db_prefix() . 'tickets.ticketid=' . db_prefix() . 'ticket_attachments.ticketid');
        ...
        $documents     = $this->db->get(db_prefix() . 'ticket_attachments')->result_array();
        $notifiedUsers = [];
        $now       = new DateTime(date('Y-m-d'));
        foreach ($documents as $document) {
            if ($document['expiry_date'] != null
                ...
                if ($diff <= $reminder_before ){
                    ...
                    $notificationLink = 'tickets/ticket/'.$document['ticketid'];
                    $notificationLink = ltrim($notificationLink, '/');

                    $notified = add_notification([
                        'fromcompany'     => true,
                        'touserid'        => $document['assigned'],
                        'description'     => 'document_expiry_reminder',
                        'link'            => $notificationLink,
                        'additional_data' => serialize([
                            $document['document_number'] . ' - ' . $document['document_name'],
                        ]),
                    ]);

                    if ($notified) {
                        array_push($notifiedUsers, $document['assigned']);
						
                    }
					  send_mail_template('ticket_document_expiry', get_staff($document['assigned'])->email, $document['assigned'], $document['ticketid'], $document['userid'], $document['contactid']);
                }
	   ...
	   private function settlement_expiry_reminder()
    {
     ...
     //  $this->db->where('is_notified', 0);
      ...
      //  $notifiedUsers = [];
        $now       = new DateTime(date('Y-m-d'));
        foreach ($documents as $document) {
			$notifycount=$document['is_notified'];
            if ($document['installment_date'] != null
                && date('Y-m-d') < $document['installment_date']) {

                $reminder_before        = 2;//get_option('send_installment_expiry_reminder_before');
                $expiry_date            = new DateTime($document['installment_date']);
                $diff                   = $expiry_date->diff($now)->format('%a');
			
				if ($diff <= $reminder_before ){
                   $this->projects_model->send_installmentduedate_reminder($document['recovery_id'], 
                    'project_installment_duedate_reminder', 'project_installment_duedate_reminder', 0, [
                        'staff' => [
                            'installment_id'   => $document['id'],
                            'installment_duedate' => $document['installment_date'],
					    'insallment_amount'   => $document['installment_amount'],
					   'installment_received'   => $document['amount_received'],
                        ],
                    ]); 
                    if($diff <= $reminder_before ){ 
					$NewDate=Date('Y-m-d', strtotime('+9 days'));	
                    $this->db->where('id', $document['id']);
                    $this->db->update(db_prefix() . 'recoveries_installments', array(
                        'is_notified' => 1,'last_due_reminder'=>$NewDate,)
                    );
                      //  $this->db->where('id',$hearing['id']);
                     //   $this->db->update('tblhearings',array('is_hearing_notified'=>1));
                    }
                 }
               
			}elseif($document['installment_amount'] > $document['amount_received'] 
                && date('Y-m-d') < $document['last_due_reminder']  && $notifycount<=4){
				if($notifycount==4)
				{
					 $reminder_before1        = 21;
				}
				else{
				  $reminder_before1        = 14;//get_option('send_installment_expiry_reminder_before');
				}
                $expiry_date1           = new DateTime($document['last_due_reminder']);
                $diff1                 = $expiry_date1->diff($now)->format('%a');
				if ($diff1 <= $reminder_before1 ){
                   $this->projects_model->send_installmentduedate_reminder($document['recovery_id'], 
                    'project_installment_duedate_reminder', 'project_installment_duedate_reminder', 0, [
                        'staff' => [
                            'installment_id'   => $document['id'],
                    ...
                    if($diff1 <= $reminder_before1 ){ 
						$NewDate1=Date('Y-m-d', strtotime('+'.$reminder_before1.' days'));
						 $this->db->where('id', $document['id']);
                    $this->db->update(db_prefix() . 'recoveries_installments', array(
                        'is_notified' =>$notifycount+1,'last_due_reminder'=>$NewDate1,
                    ));
                      //  $this->db->where('id',$hearing['id']);
                     //   $this->db->update('tblhearings',array('is_hearing_notified'=>1));
                    }
	 ...
	 public function hearing_reminder()
    {
        $this->load->model('projects_model');

        $this->db->select('hearing_date,subject,id,project_id,hearing_type');
        $this->db->where('is_hearing_notified', 0);
        $hearings = $this->db->get(db_prefix() . 'hearings')->result_array();
        $now       = new DateTime(date('Y-m-d'));

        foreach ($hearings as $hearing) {
            if (
                $hearing['hearing_date'] != null
                && date('Y-m-d') < $hearing['hearing_date']
            ) {
                $reminder_before        = 2;//get_option('send_proposal_expiry_reminder_before');
                $open_till              = new DateTime($hearing['hearing_date']);
                $diff                   = $open_till->diff($now)->format('%a');

                 if ($diff <= $reminder_before ){
                   $this->projects_model->send_hearing_reminder($hearing['project_id'], 
                    'hearing_reminder_to_staff', 'hearing_reminder_to_staff', 0, [
                        'staff' => [
