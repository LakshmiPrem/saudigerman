...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            $this->document_expiry_reminder();
			  //$this->settlement_expiry_reminder();
            ...
            $this->contract_approval_expiration_reminder();
            $this->contracts_expiration_check_today();
            
            $this->approval_expiration_reminder();
    ...
    private function document_expiry_reminder()
    {
        $this->db->select('' . db_prefix() . 'project_files.*, email, phonenumber,CONCAT(firstname ," ",lastname) as staff_full_name');
        $this->db->join(db_prefix() . 'staff', '' . db_prefix() . 'staff.staffid=' . db_prefix() . 'project_files.staffid');
        $this->db->where('is_notified', 0);
        $this->db->where('expiry_date !=', NULL);
        $documents     = $this->db->get(db_prefix() . 'project_files')->result_array();
        $notifiedUsers = [];
        $now       = new DateTime(date('Y-m-d'));
        foreach ($documents as $document) {
            if ($document['expiry_date'] != null
                && date('Y-m-d') < $document['expiry_date']) {

                $reminder_before        = get_option('send_document_expiry_reminder_before');
                $expiry_date            = new DateTime($document['expiry_date']);
                $diff                   = $expiry_date->diff($now)->format('%a');
                ...
                if ($diff <= $reminder_before ){
                    ...
                    $notificationLink = 'projects/view/'.$document['project_id'].'?group=project_files';
                    $notificationLink = ltrim($notificationLink, '/');

                    $notified = add_notification([
                        'fromcompany'     => true,
                        'touserid'        => $document['staffid'],
                        'description'     => 'document_expiry_reminder',
                        'link'            => $notificationLink,
                        'additional_data' => serialize([
                            $document['subject'] . ' - ' . strip_tags(mb_substr($document['description'], 0, 50)) . '...',
                        ]),
                    ]);

                    if ($notified) {
                        array_push($notifiedUsers, $document['staffid']);
						
                    }
                }
				$sclient= $this->db->get_where('tblprojects',array('id'=>$document['project_id']))->row()->clientid;
					$staff_email= $this->db->get_where('tblstaff',array('id'=>$document['staffid']))->row()->email;
				$client_email= $this->db->get_where('tblcontacts',array('userid'=>$sclient))->row()->email;
				 $template = mail_template('file_expiration_reminder_to_branch', $staff_email, $client_email);
	   ...
	   private function settlement_expiry_reminder()
    {
		$this->db->where('recovery_type', 'project_recovery');
       $this->db->where('is_notified', 0);
        $this->db->where('installment_date !=', NULL);
        $documents     = $this->db->get(db_prefix() . 'recoveries_installments')->result_array();
        $notifiedUsers = [];
        $now       = new DateTime(date('Y-m-d'));
        foreach ($documents as $document) {
            if ($document['installment_date'] != null
                && date('Y-m-d') < $document['installment_date']) {

                $reminder_before        = get_option('send_installment_expiry_reminder_before');
                $expiry_date            = new DateTime($document['installment_date']);
                $diff                   = $expiry_date->diff($now)->format('%a');

                if ($diff <= $reminder_before ){
                    $this->db->where('id', $document['id']);
                    $this->db->update(db_prefix() . 'tblrecoveries_installments', [
                        'is_notified' => 1,
                    ]);
                    $notificationLink = 'projects/view/'.$document['project_id'].'?group=project_settlement';
                    $notificationLink = ltrim($notificationLink, '/');
					   $assignees = $this->projects_model->get_project_members($document['recovery_id']);
					 foreach ($assignees as $member) {
                    ...
                    $notified = add_notification([
                        ...
                        'link'            => $notificationLink,
                        'additional_data' => serialize([
                            $document['installment_amount'] . ' - ' . strip_tags(mb_substr($document['remarks'], 0, 50)) . '...',
                        ]),
                    ]);
     ...
     public function contract_approval_expiration_reminder()
    {
        $contracts_auto_operations_hour = get_option('contracts_auto_operations_hour');
        ...
        foreach ($contracts as $contract) { 
            $this->db->select('id,staffid,approval_type,approvaldue_date,approval_name');
            $this->db->where('approval_status',2);
            $this->db->where('rel_id',$contract['id']);
            $this->db->where('rel_type','contract');
            $approvals = $this->db->get(db_prefix() . 'approvals')->result_array();

            foreach ($approvals as $approval) {  
                if ($approval['approvaldue_date'] != null
                && date('Y-m-d') < $approval['approvaldue_date']) { 

                    $reminder_before        = get_option('approval_expiration_before');
                    $expiry_date            = new DateTime($approval['approvaldue_date']);
                    $diff                   = $expiry_date->diff($now)->format('%a');

                    $staff = $this->staff_model->get($approval['staffid']);
                    ...
                    if ($diff <= $reminder_before ){
                        ...
                        $message = 'Hi,'. ' '.$staff->firstname.' '.$staff->lastname.',

                        This is a gentle reminder that the contract **"'.$contract['subject'].'"** is awaiting your review and approval.

                        Please review the attached contract or access it using the following link:
                        '.admin_url('contracts/contract/' . $contract['id']).'
                        ...
                        {email_signature}
                        ';
                        $this->emails_model->send_simple_email($staff->email, _l('Reminder For Contract Approval'), $message);

                    }

                }

                if ($approval['approvaldue_date'] != null && date('Y-m-d') > $approval['approvaldue_date'] && $approval['is_notified'] != 2) { 
                
                    $staff = $this->staff_model->get($approval['staffid']);

                    $message = 'Hi '.$staff->firstname.' '.$staff->lastname.',<br><br>
                    The approval due date for the contract <strong>"' . $contract['subject'] . '"</strong> has <strong>passed (' . $approval['approvaldue_date'] . ')</strong>.<br><br>
                    Please take immediate action to review or update the contract.<br><br>
                    Access the contract here:<br>
                    <a href="' . admin_url('contracts/contract/' . $contract['id']) . '">' . admin_url('contracts/contract/' . $contract['id']) . '</a><br><br>
                    Kind Regards,<br>
                    {email_signature}';

                    $this->emails_model->send_simple_email($staff->email, _l('Overdue: Contract Approval Date Passed'), $message);

                    $this->db->where('id', $approval['id']);
                    $this->db->update(db_prefix() . 'approvals', ['is_notified' => 2]);
                }
            }
...
public function contracts_expiration_check_today()
{
    $contracts_auto_operations_hour = get_option('contracts_auto_operations_hour');
    ...
    $this->db->select('id, client, dateend, subject, addedfrom, not_visible_to_client, dateadded, contract_type');
    $this->db->where('dateend IS NOT NULL');
    ...
    foreach ($contracts as $contract) {
        $endDate = new DateTime($contract['dateend']);

        // Check if contract end date has passed (yesterday or earlier)
        if ($endDate < $today) {
            // Update contract status to expired (assuming '3' or similar ID represents 'expired')
            $this->db->where('id', $contract['id']);
            $this->db->update(db_prefix() . 'contracts', ['status' => 3]); // or status_id => 3 if using numeric

            // Optional: log activity
            //log_activity('Contract ID ' . $contract['id'] . ' automatically marked as expired. End date: ' . $contract['dateend']);
        }
    }
}
...
public function approval_expiration_reminder()
    {
        $contracts_auto_operations_hour = get_option('contracts_auto_operations_hour');
        ...
        foreach ($approvals as $approval) {
            // Skip if no due date or reminders not enabled
            if ($approval['approvaldue_date'] == null || $approval['enable_reminder'] != 1) {
                continue;
            }

            $due_date = new DateTime($approval['approvaldue_date']);
            $diff_days = $due_date->diff($now)->days;
            $is_overdue = ($now > $due_date);

            // Reminder condition: 3 days passed after due date
            if ($is_overdue && $diff_days >= $approval['reminder_days']) {
                $sendReminder = false;

                // Check repeat condition
                if (!empty($approval['repeat_every']) && $approval['repeat_every'] > 0) {
                    if (!empty($approval['last_reminder_date'])) {
                        $last_reminder = new DateTime($approval['last_reminder_date']);
                        $days_since_last = $last_reminder->diff($now)->days;
                        if ($days_since_last >= $approval['repeat_every']) {
                            $sendReminder = true;
                        }
                    } else {
                        // First reminder after 3 days overdue
                        $sendReminder = true;
                    }
                } else {
                    // No repeat set – only send one reminder after 3 days
                    if (empty($approval['last_reminder_date'])) {
                        $sendReminder = true;
                    }
                }

                if ($sendReminder && ($approval['approval_status'] != 3 || $approval['approval_status'] != 5)) {
                    $staff = $this->staff_model->get($approval['staffid']);
                    if (!$staff) continue;

                    // Fetch related contract (optional if needed for subject/link)
                    $contract = $this->db->select('id, subject')
                        ->where('id', $approval['rel_id'])
                        ...
                        $message = '
                        Hi ' . $staff->firstname . ' ' . $staff->lastname . ',<br><br>
                        This is a reminder regarding your pending approval.<br><br>
                        ...
                        <table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse; width:100%;">
                            ...
                            <tr>
                                <td><strong>Approval Due Date</strong></td>
                                <td>' . _d($approval['approvaldue_date']) . '</td>
                            </tr>
                            <tr>
                                <td><strong>Approval Heading</strong></td>
                                <td>' . get_approval_heading_name_by_id($approval['approval_heading_id']) . '</td>
                            </tr>
                            <tr>
                                <td><strong>Approved By</strong></td>
                                <td>' . get_staff_full_name($approval['staffid']) . '</td>
                            </tr>
                            
                        </table><br><br>
                        Please take necessary action to complete the approval process.<br><br>' .
                        ($contract ? '
                        Access the related contract:<br>
                        <a href="' . admin_url('contracts/contract/' . $contract->id) . '">' . admin_url('contracts/contract/' . $contract->id) . '</a><br><br>' : '') .
                        'Kind Regards,<br>{email_signature}';

                    $this->emails_model->send_simple_email($staff->email, _l('Reminder: Contract Approval Pending'), $message);
