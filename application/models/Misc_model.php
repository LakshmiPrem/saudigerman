...
class Misc_model extends App_Model
{
    ...
    public function add_attachment_to_database($rel_id, $rel_type, $attachment, $external = false)
    {
		...
		if (isset($attachment[0]['document_type'])) {
			  $data['document_type']=$attachment[0]['document_type'];
		  }
		if (isset($attachment[0]['subject'])) {
			  $data['subject']=$attachment[0]['subject'];
		  }
        ...
        $insert_id = $this->db->insert_id();
		 if(get_option('enable_sharepoint')==1){
			 ...
			 if($data['rel_type']=='customer')
				...
				$sharegrah->rungraphuser($attachment[0]['file_name'],$newFilePath,$rel_id,$rel_type);	
				$sharelink=$sharegrah->getweburl($rel_id,$attachment[0]['file_name'],$rel_type);
			 $userfile1= $this->db->get_where('tblfiles',array('id'=>$insert_id))->row()->file_name;
			$contentfile=projectdocumentcontents($userfile1,$rel_id,$rel_type);
			 $this->db->where( 'id', $insert_id);
			 $this->db->update( db_prefix() . 'files', [
				'doccontent' => $contentfile,
				'external_sharepoint_link'=>$sharelink,
			] );
		 }
		 }
    ...
    {
        $this->db->select('tblnotes.*,tblcontactcodes.code,tblstaff.firstname,tblstaff.lastname');
        $this->db->join(db_prefix() . 'staff', db_prefix() . 'staff.staffid=' . db_prefix() . 'notes.addedfrom');
         $this->db->join('tblcontactcodes', 'tblcontactcodes.id=tblnotes.contact_code','left');
    ...
    public function _search_projects($q, $limit = 0, $where = false,$order_by='tblprojects.name',$asc='ASC')
    {
        $result = [
            'result'         => [],
            'type'           => 'projects',
    ...
    public function _search_trade_license($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'trade_license',
            'search_heading' => _l('trade_license'),
        ];

        $has_permission_view_contracts = has_permission('trade_licenses', '', 'view');
        if ($has_permission_view_contracts || has_permission('trade_licenses', '', 'view_own')) {
            // Contracts
            $this->db->select();
            $this->db->from(db_prefix() . 'trade_licenses');
            if (!$has_permission_view_contracts) {
                $this->db->where(db_prefix() . 'trade_licenses.addedfrom', get_staff_user_id());
            }

            $this->db->where('(license_no LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\' OR cr_no LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\')');
    ...
    public function _search_oppositeparty($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'oppositeparty',
            'search_heading' => _l('oppositeparty'),
        ];

        $has_permission_view_contracts = has_permission('projects', '', 'view');
        if ($has_permission_view_contracts || has_permission('projects', '', 'view_own')) {
            // Contracts
            $this->db->select();
            $this->db->from(db_prefix() . 'oppositeparty');
            if (!$has_permission_view_contracts) {
                $this->db->where(db_prefix() . 'oppositeparty.addedfrom', get_staff_user_id());
            }

            $this->db->where('(name LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\' OR firstname LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\')');
 ...
 public function _search_legalrisk($q, $limit = 0)
    {
        $result = [
            'result'         => [],
            'type'           => 'legalrisk',
            'search_heading' => _l('legalrisk'),
        ];

        $has_permission_view_contracts = has_permission('legalrisk', '', 'view');
        if ($has_permission_view_contracts || has_permission('legalrisk', '', 'view_own')) {
            // Contracts
            $this->db->select();
            $this->db->from(db_prefix() . 'legal_risk');
            if (!$has_permission_view_contracts) {
                $this->db->where(db_prefix() . 'legal_risk.addedfrom', get_staff_user_id());
            }

            $this->db->where('(risktitle LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\' OR risk_value LIKE "%' . $this->db->escape_like_str($q) . '%" ESCAPE \'!\')');

            if ($limit != 0) {
                $this->db->limit($limit);
            }
            $this->db->order_by('risktitle', 'ASC');
            $result['result'] = $this->db->get()->result_array();
        }
