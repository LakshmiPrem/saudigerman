...
class Intellectual_property_model extends App_Model
{
    ...
    {
        $this->db->select('*,tblintellectual_property.id as id, tblintellectual_property.addedfrom');
        $this->db->where($where);
        //$this->db->join('tblcontracttypes', 'tblcontracttypes.id = tblintellectual_property.contract_type', 'left');
        $this->db->join('tblclients', 'tblclients.userid = tblintellectual_property.client');
        if (is_numeric($id)) {
            $this->db->where('tblintellectual_property.id', $id);
            $contract = $this->db->get('tblintellectual_property')->row();
        ...
        $contracts = $this->db->get('tblintellectual_property')->result_array();
        $i         = 0;
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(issue_date)) as year FROM tblintellectual_property')->result_array();
    }
    ...
    public function get_contract_attachments($attachment_id = '', $id = '')
    {
        ...
        $this->db->where('rel_type', 'intellectual_property');
    ...
    public function add($data)
    {
        ...
        $this->db->insert('tblintellectual_property', $data);
        ...
        if ($insert_id) {
           ...
           hooks()->apply_filters('after_contract_added', $insert_id);
            log_activity('New IP  Added [' . $data['license_no'] . ']');
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tblintellectual_property', $data);
        if ($this->db->affected_rows() > 0) {
            hooks()->apply_filters('after_contract_updated', $id);
            log_activity('IP Updated [' . $data['license_no'] . ']');
    ...
    public function copy($id)
    {
        ...
        $fields = $this->db->list_fields('tblintellectual_property');
    ...
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete('tblintellectual_property');
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_contract_attachment($attachment_id)
    {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink(get_upload_path_by_type('intellectual_property') . $attachment->rel_id . '/' . $attachment->file_name);
            }
            $this->db->where('id', $attachment->id);
            $this->db->delete('tblfiles');
            if ($this->db->affected_rows() > 0) {
                $deleted = true;
                log_activity('Ip/Trademark Attachment Deleted [ContractID: ' . $attachment->rel_id . ']');
            }

            if (is_dir(get_upload_path_by_type('intellectual_property') . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files(get_upload_path_by_type('intellectual_property') . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    // okey only index.html so we can delete the folder also
                    delete_dir(get_upload_path_by_type('intellectual_property') . $attachment->rel_id);
                }
            }
        }
    ...
    public function renew($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->db->update('tblintellectual_property', $_data);
            if ($this->db->affected_rows() > 0) {
