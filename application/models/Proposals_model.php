...
class Proposals_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if ($this->copy == false) {
            ...
            if($data['proposal_type'] != ''){
                ...
                $content = $this->templates_model->get($data['proposal_type'],array('type'=>'proposals'));
                $data['content'] = $content->content;
            }else{

            $data['content'] = 
                '
                <p>Subject : {proposal_subject}<br><br>Dear {proposal_proposal_to},<br><br>We very much appreciate your decision to engage our service in relation to the matters detailed in the Scope of Services as per this Terms of Engagement and its Schedules. <br><br>Scope Of Services<br>{scope_of_services} <br><br>{proposal_items}<br><br>Terms and Conditions<br>{terms_conditions}</p>

                ';
            }
        }

        if(isset($data['matter_scope'])) 
            $data['matter_scope'] = implode(',', $data['matter_scope']);
    ...
    public function update($data, $id)
    {
        ...
        $proposal_type = $this->db->get_where('tblproposals',array('id'=>$id))->row()->proposal_type;

        if($proposal_type != $data['proposal_type']){
            $this->load->model('templates_model');
            $content = $this->templates_model->get($data['proposal_type'],array('type'=>'proposals'));
            $data['content'] = $content->content;
        }
