...
class Expenses_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
		...
		$this->db->join(db_prefix() . 'expense_approval_names', '' . db_prefix() . 'expense_approval_names.id = ' . db_prefix() . 'expenses.approvalid', 'left');
        ...
        if (is_numeric($id)) {
            ...
            //    $expense->filetype              = '';
             //   $expense->attachment_added_from = 0;
				...
				$expense->payment_data = $this->get_expensepayment($id,$expense->project_id);
    ...
    public function add($data)
    {
		...
		if (isset($data['refundable'])) {
            $data['refundable'] = 1;
			 $data['refund_date'] = to_sql_date($data['refund_date']);
        } else {
            $data['refundable'] = 0;
        }
    ...
    public function get_expenses_total($data)
    {
        ...
        for ($i = 1; $i <= 7; $i++) {
            $this->db->select('amount,paid_amount,last_amount,vat_amount,balance_amount,tax,tax2,invoiceid');
            ...
            $_total_all   = [];  $_ptotal_all   = [];  $_ntotal_all   = []; $_btotal_all   = [];
            $cached_taxes = [];
            foreach ($all_expenses as $expense) {
                $_total = $expense['amount'];
				$_ptotal=$expense['paid_amount'];
				$_ntotal=$expense['last_amount']+$expense['vat_amount'];
				$_btotal=$expense['balance_amount']-($expense['last_amount']+$expense['vat_amount']);
                if ($expense['tax'] != 0) {
				...
				array_push($_ptotal_all, $_ptotal);
				array_push($_ntotal_all, $_ntotal);
				array_push($_btotal_all, $_btotal);
            }
            $_result[$key]['total'] = app_format_money(array_sum($_total_all), $currency);
			 $_result[$key]['ptotal'] = app_format_money(array_sum($_ptotal_all), $currency);
			 $_result[$key]['ntotal'] = app_format_money(array_sum($_ntotal_all), $currency);
			 $_result[$key]['btotal'] = app_format_money(array_sum($_btotal_all), $currency);
        }
	...
	public function get_expenses_cat_total($id)
	{ $this->db->select('sum(amount)as amount,sum(paid_amount) as paid_amount ,sum(last_amount)as last_amount ,sum(balance_amount) as balance_amount,sum(vat_amount) as vat_amount,'. db_prefix() . 'expenses_categories.name as category_name');
        $this->db->from(db_prefix() . 'expenses');
        $this->db->join(db_prefix() . 'clients', '' . db_prefix() . 'clients.userid = ' . db_prefix() . 'expenses.clientid', 'left');
        $this->db->join(db_prefix() . 'expenses_categories', '' . db_prefix() . 'expenses_categories.id = ' . db_prefix() . 'expenses.category');
	 ...
	 public function get_expenses_status($id = '')
    {
        if (is_numeric($id)) {
            $this->db->where('ticketstatusid', $id);

            return $this->db->get(db_prefix() . 'expense_status')->row();
        }
        $this->db->order_by('statusorder', 'asc');

        return $this->db->get(db_prefix() . 'expense_status')->result_array();
    }
  public function add_expensepayment($data)
    {
        $data['addedby'] = get_staff_user_id();
        $data['dateadded'] = date('Y-m-d H:i:s');
		$data['exp_paid_date']= isset($data['exp_paid_date']) ? to_sql_date($data['exp_paid_date']) : '';
        $this->db->insert(db_prefix() . 'expense_payment', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            log_activity('New Expense Payment Added [ID: ' . $insert_id . ']');
    ...
    public function get_expensepayment($expense_id,$project_id)
    {
        $this->db->where('expense_id', $expense_id);
        $this->db->where('project_id', $project_id);

            return $this->db->get(db_prefix() . 'expense_payment')->result_array();
      
    }
    public function delete_expense_payment($id)
    {
       
            if ( $id) {
                
                $this->db->where('id', $id);
                $this->db->delete(db_prefix() . 'expense_payment');
                log_activity('Expense Receipt Deleted [ExpenseID: ' . $id . ']');
