 ...
 * @return string
 */
function render_select($name, $options, $option_attrs = [], $label = '', $selected = '', $select_attrs = [], $form_group_attr = [], $form_group_class = '', $select_class = '', $include_blank = true,$optseparator=' ')
{
    $callback_translate = '';
    ...
    foreach ($options as $option) {
        ...
        if (!is_array($option_attrs[1])) {
            ...
            foreach ($option_attrs[1] as $_val) {
                $val .= $option[$_val] . $optseparator;
            }
			if($optseparator!=' ')
			$val=rtrim($val,$optseparator);
        }
        $val = trim($val);
