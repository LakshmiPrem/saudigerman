...
function get_acceptance_party_info_array($empty = false)
{
    $CI        = &get_instance();
    $signature = null;
    ...
    $data = [
        ...
        'partyacc_firstname' => !$empty ? $CI->input->post('acceptance_firstname') : null,
        'partyacc_lastname'  => !$empty ? $CI->input->post('acceptance_lastname') : null,
        'partyacc_email'     => !$empty ? $CI->input->post('acceptance_email'): null,
        'partyacc_date'      => !$empty ? date('Y-m-d H:i:s') : null,
        'partyacc_ip'        => !$empty ? $CI->input->ip_address() : null,
...
function process_digital_signature_image_contract($partBase64, $path)
{
    if (empty($partBase64)) {
        return false;
    }
    ...
    if (strpos($partBase64, 'data:image') === 0) {
        $partBase64 = explode(',', $partBase64)[1];
    }
    ...
    if ($decoded_image === false) {
        log_message('error', 'Failed to decode base64 signature image.');
        return false;
    }
