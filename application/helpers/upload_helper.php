        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
					...
					$subjectname=$CI->input->post('file_subject');
					if(empty($subjectname)){
						//$subjectname=substr($filename, 0, strrpos($filename, "."));
						$subjectname=substr(str_replace('','-',$filename), 0, strrpos($filename, "."));
					}
                    $data = [
                            ...
                            'subject'    => $subjectname, 
                           'issue_date' => to_sql_date($CI->input->post('issue_date')),
                            'expiry_date'=> to_sql_date($CI->input->post('expiry_date')),
                            'description' => $CI->input->post('file_description'),
                            'document_type'=> $CI->input->post('document_type'),
        ...
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    'file_name' => $filename,
                    'filetype'  => $_FILES['file']['type'][$i],
					'subject' =>substr($filename, 0, strrpos($filename, '.')),
                    ];
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        array_push($uploaded_files, [
                                'file_name' => $filename,
                                'filetype'  => $_FILES[$index_name]['type'][$i],
								'document_type'=> $CI->input->post('document_type')[$i],
                                ]);
                    }
                }
            }
...
function get_upload_path_by_type($type)
{
    ...
    switch ($type) {
        ...
        $path = AGREEMENT_FOLDER;

        break;
        case 'fee_proposal':
        $path = FEE_PROPOSAL_FOLDER;

        break;
        case 'corporate_recovery':
        $path = CORPORATE_RECOVERY_FOLDER;
        break;
        ...
        case 'trade_license':
        $path = TRADE_LICENSE_FOLDER;
        break;

        case 'oppositeparty':
        $path = OPPOSITEPARTY_FOLDER;
        break;
			
		case 'installment':
        $path = INSTALLMENT_FOLDER;
        break;
		case 'legalrisk':
        $path = LEGALRISK_FOLDER;
        break;

        case 'customer_subfile_images':
        $path = CLIENT_SUBFILE_FOLDER;
        break;
        
        case 'chequebounce':
        $path = CHEQUEBOUNCE_FOLDER;
        break;
		case 'template':
        ...
        case 'document':
            return DOCUMENTS_ATTACHMENTS_FOLDER;
            break;
        ...
        break;
        case 'documents_in':
            return DOCUMENTS_IN_ATTACHMENTS_FOLDER;
            break;
        case 'documents_out':
            return DOCUMENTS_OUT_ATTACHMENTS_FOLDER;
            break;
        case 'lawyer':
        $path = LAWYER_ATTACHMENT_FOLDER;
        break;
        case 'esignature':
        $path = ESIGNATURE_FOLDER;//esignature
...
/**
 * Handles upload for Hearing files
 * @param  mixed $hearing id hearing id
 * @return boolean
 */
function handle_hearing_attachment($hearing_id,$project_id)
{
    $filesIDS = array();
    $errors = array();
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                    ...
                    if (is_client_logged_in()) {
                        ...
                        $data['visible_to_customer'] = 0;//($CI->input->post('visible_to_customer') == 'true' ? 1 : 0);
                    }
...
function handle_recovery_attachments_upload($id, $customer_upload = false)
{
    $path = get_upload_path_by_type('corporate') . $id . '/';
    $CI =& get_instance();
    if (isset($_FILES['file']['name'])) {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'corporate', $attachment);
            }
        }
    }
}

function handle_ip_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        //hooks()->do_action('before_upload_contract_attachment', $id);
        $path = get_upload_path_by_type('intellectual_property') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'intellectual_property', $attachment);
...
function handle_trade_license_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        //hooks()->do_action('before_upload_contract_attachment', $id);
        $path = get_upload_path_by_type('trade_license') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'trade_license', $attachment);
 ...
 * @return boolean
 */
function handle_communinication_attachment($communication_id,$project_id)
{
    $filesIDS = array();
    $errors = array();
    ...
    if (isset($_FILES['attachments']['name'])
        ...
        if (!is_array($_FILES['attachments']['name'])) {
            $_FILES['attachments']['name'] = array($_FILES['attachments']['name']);
            $_FILES['attachments']['type'] = array($_FILES['attachments']['type']);
            $_FILES['attachments']['tmp_name'] = array($_FILES['attachments']['tmp_name']);
            $_FILES['attachments']['error'] = array($_FILES['attachments']['error']);
            $_FILES['attachments']['size'] = array($_FILES['attachments']['size']);
        }
        ...
        for ($i = 0; $i < count($_FILES['attachments']['name']); $i++) {
            if (_perfex_upload_error($_FILES['attachments']['error'][$i])) {
                $errors[$_FILES['attachments']['name'][$i]] = _perfex_upload_error($_FILES['attachments']['error'][$i]);
                continue;
            }
...
function handle_oppositeparty_attachments_upload($id, $customer_upload = false)
{
    $path = get_upload_path_by_type('oppositeparty') . $id . '/';
    $CI =& get_instance();
    if (isset($_FILES['file']['name'])) {
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'oppositeparty', $attachment);
            }
        }
    }
}

function handle_installment_receipt_image_upload($installment_id,$recovery_id,$type)
{
    if (isset($_FILES['installment_receipt']) && _perfex_upload_error($_FILES['installment_receipt']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['installment_receipt']['error']));

        return false;
    }
    if (isset($_FILES['installment_receipt']['name']) && $_FILES['installment_receipt']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
        $path        = get_upload_path_by_type($type).'/'.$recovery_id.'/';
        // Get the temp file path
        $tmpFilePath = $_FILES['installment_receipt']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["installment_receipt"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $CI =& get_instance();
                $CI->db->where('id',$installment_id);
                $CI->db->update('tblrecoveries_installments',array('installment_receipt'=>$filename));
                return true;
            }
        }
    }
...
function handle_opposite_profile_image_upload($id)
{
	
    if (isset($_FILES['profile_image']) && _perfex_upload_error($_FILES['profile_image']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['profile_image']['error']));
    ...
    if (isset($_FILES['profile_image']['name']) && $_FILES['profile_image']['name'] != '') {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tbloppositeparty',array('profile_image'=>$filename,'profile_date'=>$dt));
                return true;
            }
        }
    }
...
function handle_contractrenew_file_upload($id,$contract_id)
{
	
    if (isset($_FILES['new_filename']) && _perfex_upload_error($_FILES['new_filename']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['new_filename']['error']));

        return false;
    }
    if (isset($_FILES['new_filename']['name']) && $_FILES['new_filename']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('contract') . $contract_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['new_filename']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["new_filename"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tblcontract_renewals',array('new_filename'=>$filename));
                return true;
            }
        }
    }
...
{
    if (isset($_FILES['pdf_footer_image']) && _perfex_upload_error($_FILES['pdf_footer_image']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['pdf_footer_image']['error']));

        return false;
    }
    if (isset($_FILES['pdf_footer_image']['name']) && $_FILES['pdf_footer_image']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
        $path        = get_upload_path_by_type('company');
        // Get the temp file path
        $tmpFilePath = $_FILES['pdf_footer_image']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["pdf_footer_image"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            // Setup our new file path
            $filename    = 'pdf_footer' . '.' . $extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                update_option('pdf_footer_image', $filename);
...
function handle_header_image_upload()
{
    if (isset($_FILES['pdf_header_image']) && _perfex_upload_error($_FILES['pdf_header_image']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['pdf_header_image']['error']));
        return false;
    }
    if (isset($_FILES['pdf_header_image']['name']) && $_FILES['pdf_header_image']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
        $path        = get_upload_path_by_type('company');
        // Get the temp file path
        $tmpFilePath = $_FILES['pdf_header_image']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["pdf_header_image"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            // Setup our new file path
            $filename    = 'pdf_header' . '.' . $extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                update_option('pdf_header_image', $filename);
...
function handle_legalrisk_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        //hooks()->do_action('before_upload_contract_attachment', $id);
        $path = get_upload_path_by_type('legalrisk') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'legalrisk', $attachment);
...
function handle_project_contract_file_upload($contract_id)
{
	  $CI =& get_instance();
	 
	
    if (isset($_FILES['agree_attachment']) && _perfex_upload_error($_FILES['agree_attachment']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['agree_attachment']['error']));

        return false;
    }
    if (isset($_FILES['agree_attachment']['name']) && $_FILES['agree_attachment']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('contract') . $contract_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['agree_attachment']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["agree_attachment"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
    			...
    			$CI->load->library('sharegraph');
				$sharegrah=new Sharegraph(); 
              $sharegrah->rungraphuser($filename,$newFilePath,$contract_id);	
			  $sharelink=$sharegrah->getweburl($contract_id,$filename);
				}
                $CI->db->where('id',$contract_id);
                $CI->db->update('tblcontracts',array('contract_filename'=>$filename,'file_type'=>$_FILES['agree_attachment']['type'],'sharepoint_link'=>$sharelink));
                return true;
            }
        }
    }
...
// slc v2 update
function handle_client_sub_file_upload($id,$client_id)
{
    
    if (isset($_FILES['file_name']) && _perfex_upload_error($_FILES['file_name']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['file_name']['error']));

        return false;
    }
    if (isset($_FILES['file_name']['name']) && $_FILES['file_name']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('customer_subfile_images') . $client_id . '/';
        // Get the temp file path
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tblclient_subfile',array('file_name'=>$filename,'file_type'=>$_FILES['file_name']['type']));
                return true;
            }
        }
    }
...
function handle_communinication_outlook_attachment($communication_id,$project_id)
{
     if (isset($_FILES['Outlook_msg']) && _perfex_upload_error($_FILES['Outlook_msg']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['Outlook_msg']['error']));

        return false;
    }
    if (isset($_FILES['Outlook_msg']['name']) && $_FILES['Outlook_msg']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('project') . $project_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['Outlook_msg']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["Outlook_msg"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            // Setup our new file path
              $filename = $_FILES['Outlook_msg']['name'];
           // $filename    =   'I'.date('Ymdhis') .'.'.$extension;
            $newFilePath = $path .  preg_replace("/\s+/", "",$filename);
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                
                $CI =& get_instance();
                $CI->db->where('id',$communication_id);
                $CI->db->update('tblcase_communication_center',array('outlook_msg'=>$filename,'filetype'=>$_FILES['Outlook_msg']['type']));
                return true;
            }
        }
    }
...
function handle_project_judgement_file_upload($id,$project_id)
{
    
    if (isset($_FILES['judge_attachment']) && _perfex_upload_error($_FILES['judge_attachment']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['judge_attachment']['error']));

        return false;
    }
    if (isset($_FILES['judge_attachment']['name']) && $_FILES['judge_attachment']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('project') . $project_id . '/';
		  _maybe_create_upload_path($path);
        // Get the temp file path
        $tmpFilePath = $_FILES['judge_attachment']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["judge_attachment"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            $filename    =   'JR'.date('Ymdhis') .'.'.$extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tblproject_judgement',array('judge_attachment'=>$filename,'judge_filetype'=>$_FILES['judge_attachment']['type']));
                return true;
            }
        }
    }
...
function handle_project_contract_version_file_upload($contract_id,$id)
{
	  $CI =& get_instance();
    ...
    if (isset($_FILES['agree_attachment']['name']) && $_FILES['agree_attachment']['name'] != '') {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            $filename    =   'ver'.date('Ymdhis') .'.'.$extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
			...
			//  $sharelink=$sharegrah->getweburl($contract_id,$filename);
                $CI->db->where('id',$id);
                $CI->db->update('tblcontract_versions',array('version_internal_file_path'=>$filename));
                return true;
            }
        }
    }
...
function handle_project_subfile_file_upload_gag($id,$project_id)
{
	
    if (isset($_FILES['pop_attachment']) && _perfex_upload_error($_FILES['pop_attachment']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['poa_attachment']['error']));

        return false;
    }
    if (isset($_FILES['pop_attachment']['name']) && $_FILES['pop_attachment']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('project') . $project_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['pop_attachment']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["pop_attachment"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tblproject_versionfiles',array('file_name'=>$filename,'filetype'=>$_FILES['pop_attachment']['type']));
                return true;
            }
        }
    }
...
function handle_signedLOE_contract_file_upload($contract_id)
{
    
	 $CI =& get_instance();	
    if (isset($_FILES['signed_agree_attachment']) && _perfex_upload_error($_FILES['signed_agree_attachment']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['signed_agree_attachment']['error']));

        return false;
    }
    if (isset($_FILES['signed_agree_attachment']['name']) && $_FILES['signed_agree_attachment']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('contract') . $contract_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['signed_agree_attachment']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["signed_agree_attachment"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            // Setup our new file path
			$sharelink='';
            $filename    =   'unsigned'.date('Ymdhis') .'.'.$extension;
           //$filename = $_FILES['signed_agree_attachment']['name'];
           //$filename    =   'Sign'.date('Ymdhis').$filename.'.'.$extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $CI->db->where('id',$contract_id);
                $CI->db->update('tblcontracts',array('signed_contract_filename'=>$filename));
                return true;
            }
        }
    }
...
function handle_chequebounce_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        hooks()->do_action('before_upload_chequebounce_attachment', $id);
        $path = get_upload_path_by_type('chequebounce') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'chequebounce', $attachment);
...
function handle_template_file_upload($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        hooks()->do_action('before_upload_template_attachment', $id);
        $path = get_upload_path_by_type('template') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->db->where('id',$id);
                $CI->db->update('tbltemplates',array('temp_filename'=>$filename));
 ...
 * @return boolean
 */
function handle_document_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        hooks()->do_action('before_upload_document_attachment', $id);
        $path        = get_upload_path_by_type('document') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'document', $attachment);
 ...
 * @return boolean
 */
function handle_document_in_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        do_action('before_upload_document_attachment', $id);
        $path        = get_upload_path_by_type('documents_in') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'documents_in', $attachment);
 ...
 * @return boolean
 */
function handle_document_out_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        do_action('before_upload_document_attachment', $id);
        $path        = get_upload_path_by_type('documents_out') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'communication', $attachment);
...
function handle_image_file_upload()
{
    if (isset($_FILES['ocr_file']) && _perfex_upload_error($_FILES['ocr_file']['error'])) { 
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['ocr_file']['error']);
        die;
    }
    if (isset($_FILES['ocr_file']['name']) && $_FILES['ocr_file']['name'] != '') {
        hooks()->do_action('before_upload_document_attachment');
        $path        = get_upload_path_by_type('ocrimage');
        // Get the temp file path
        $tmpFilePath = $_FILES['ocr_file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            _maybe_create_upload_path($path);
            $filename    = unique_filename($path, $_FILES["ocr_file"]["name"]);
            $newFilePath = $path . $filename;
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
...
function handle_ipproject_work_file_upload($id)
{
	
    if (isset($_FILES['ip_logo']) && _perfex_upload_error($_FILES['ip_logo']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['ip_logo']['error']));

        return false;
    }
    if (isset($_FILES['ip_logo']['name']) && $_FILES['ip_logo']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('project') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['ip_logo']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["ip_logo"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            // Setup our new file path
			  $filename = str_replace(' ','_',unique_filename($path, $_FILES['ip_logo']['name']));
           // $filename    =   'Logo'.date('Ymdhis') .'.'.$extension;
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tblprojects',array('ip_logo'=>$filename));
                return true;
            }
        }
    }
...
function handle_project_subfile_file_upload($id,$project_id)
{
    ...
    if (isset($_FILES['pop_attachment']['name']) && $_FILES['pop_attachment']['name'] != '') {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tblproject_files',array('file_name'=>$filename,'filetype'=>$_FILES['pop_attachment']['type']));
                return true;
            }
        }
    }
...
function handle_expense_payment_file_upload($id,$expense_id)
{
    ...
    if (isset($_FILES['pop_attachment']['name']) && $_FILES['pop_attachment']['name'] != '') {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            $filename    =   'EP'.date('Ymdhis') .'.'.$extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI =& get_instance();
                $CI->db->where('id',$id);
                $CI->db->update('tblexpense_payment',array('exp_filename'=>$filename,'exp_filetype'=>$_FILES['pop_attachment']['type']));
                return true;
            }
        }
    }
...
function handle_oppositeparty_kycattachments_upload($id, $customer_upload = false)
{
    $path = get_upload_path_by_type('oppositeparty') . $id . '/';
    $CI =& get_instance();
    if (isset($_FILES['file']['name'])) {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $attachment[]= array(
					...
					'contract_id'    => $CI->input->post('contract_id'),
 ...
 * @return array  - Result values
 */
function handle_lawyer_attachments_upload($id, $customer_upload = false)
{
    $path = get_upload_path_by_type('lawyer') . $id . '/';
    $CI =& get_instance();
    if (isset($_FILES['file']['name'])) {
        do_action('before_upload_client_attachment', $id);
        // Get the temp file path
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                ...
                $CI->misc_model->add_attachment_to_database($id, 'lawyer', $attachment);
            }
        }
    }
}
function handle_project_contract_amendment_file_upload($contract_id,$id)
{
	  $CI =& get_instance();
	
	
    if (isset($_FILES['amend_attachment']) && _perfex_upload_error($_FILES['amend_attachment']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['amend_attachment']['error']));

        return false;
    }
    if (isset($_FILES['amend_attachment']['name']) && $_FILES['amend_attachment']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('contract') . $contract_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['amend_attachment']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["amend_attachment"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            $filename    =   'amend'.date('Ymdhis') .'.'.$extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
			...
			//  $sharelink=$sharegrah->getweburl($contract_id,$filename);
                $CI->db->where('id',$id);
                $CI->db->update('tblcontract_amendments',array('amendement_file'=>$filename));
                return true;
            }
        }
    }
...
//----------------------------------------upload digital signature start------------------------------>

function handle_esignature_upload($staff_id = '')
{
    if (isset($_FILES['esignature']['name']) && $_FILES['esignature']['name'] != '') {
        hooks()->do_action('before_upload_digital_signature');
        if (!is_numeric($staff_id)) {
            $staff_id = get_staff_user_id();
        }
        $path = get_upload_path_by_type('esignature') . $staff_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['esignature']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $extension = strtolower(pathinfo($_FILES['esignature']['name'], PATHINFO_EXTENSION));
            ...
            $allowed_extensions = hooks()->apply_filters('esignature_upload_allowed_extensions', $allowed_extensions);

            

            _maybe_create_upload_path($path);
            //$filename    = unique_filename($path, $_FILES['digital_signature']['name']);
            $filename    =   'signature'.date('Ymdhis') .'.'.$extension;
            //$filename    = "signature";
            $newFilePath = $path . $filename;
            // Upload the file into the company uploads dir
            $filename    =   'I'.date('Ymdhis') .'.'.$extension;
...
//----------------------------------------upload digital signature end------------------------------>



function handle_staff_signature_upload($id,$staff_id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['file']['error']));
...
function handle_project_contract_postaction_file_upload($contract_id,$id)
{
      $CI =& get_instance();
    
    
    if (isset($_FILES['post_attachment']) && _perfex_upload_error($_FILES['post_attachment']['error'])) {
        set_alert('warning', _perfex_upload_error($_FILES['post_attachment']['error']));
    ...
    if (isset($_FILES['post_attachment']['name']) && $_FILES['post_attachment']['name'] != '') {
        //do_action('before_upload_signature_image_attachment');
         $path = get_upload_path_by_type('contract') . $contract_id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['post_attachment']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["post_attachment"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            ...
            $filename    =   'post'.date('Ymdhis') .'.'.$extension;
            $newFilePath = $path . $filename;
            _maybe_create_upload_path($path);
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
            ...
            //  $sharelink=$sharegrah->getweburl($contract_id,$filename);
                $CI->db->where('id',$id);
                $CI->db->update('tblcontract_actions',array('post_attachment'=>$filename));
                return true;
            }
        }
    }
