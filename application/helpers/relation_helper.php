...
function get_relation_data($type, $rel_id = '', $extra = [])
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            ...
            $where_contacts = db_prefix() . 'contacts.active=1 AND is_primary=1';
    ...
    }elseif ($type == 'trade_license') {
        if ($rel_id != '') {
            $CI->load->model('trade_licenses_model');
            $data = $CI->trade_licenses_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_trade_license($q);
            $data   = $search['result'];
        }
    }elseif ($type == 'legalrisk') {
        if ($rel_id != '') {
            $CI->load->model('legalrisk_model');
            $data = $CI->legalrisk_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_legalrisk($q);
            $data   = $search['result'];
        }
    }elseif ($type == 'oppositeparty') {
        if ($rel_id != '') {
            $CI->load->model('oppositeparty_model');
            $data = $CI->oppositeparty_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_oppositeparty($q);
            $data   = $search['result'];
        }
    }elseif ($type == 'document' || $type == 'documents') {
        if ($rel_id != '') {
            $CI->load->model('documents_model');
            $data = $CI->documents_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_documents($q);
            $data   = $search['result'];
        }
    }elseif ($type == 'chequebounces' || $type == 'chequebounce') {
        $CI->load->model('chequebounces_model');

        if ($rel_id != '') {
            $CI->load->model('chequebounces_model');
            $data = $CI->chequebounces_model->get($rel_id);
        } else {
            $search = $CI->misc_model->_search_chequebounces($q);
            $data   = $search['result'];
        }
    }
...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if (is_array($relation)) {
            ...
            $id     = $relation['id'];
          //  $name   = get_company_name($relation['userid']).' - '.$relation['firstname'] . ' ' . $relation['lastname'];
			$name   = get_company_name($relation['userid']);
			// $subtext=$relation['firstname'] . ' ' . $relation['lastname'];
        } else {
      ...
      //  $subtext = get_company_name($userid);
    ...
    }elseif ($type == 'trade_license' || $type == 'trade_license') {
        if (is_array($relation)) {
            $id        = $relation['id'];
            $name      = $relation['license_no'];
            $addedfrom = $relation['addedfrom'];
        } else {
            $id        = $relation->id;
            $name      = $relation->license_no;
        ...
        $link = admin_url('trade_licenses/trade_license/' . $id);
    }elseif ($type == 'legalrisk' || $type == 'legalrisks') {
        if (is_array($relation)) {
            $id        = $relation['id'];
            $name      = $relation['risktitle'];
            $addedfrom = $relation['addedfrom'];
        } else {
            $id        = $relation->id;
            $name      = $relation->risktitle;
        ...
        $link = admin_url('legal_risks/legal_risk/' . $id);
    }  elseif ($type == 'document' || $type == 'documents') {
        if (is_array($relation)) {
            $id        = $relation['id'];
            $name      = $relation['subject'];
        ...
        $link = admin_url('documents/document/' . $id);
    }elseif ($type == 'chequebounce' || $type == 'chequebounces') {
        if (is_array($relation)) {
            $id        = $relation['id'];
            $name      = $relation['subject'];
            $addedfrom = $relation['addedfrom'];
        ...
        $link = admin_url('chequebounces/chequebounce/' . $id);
    }
