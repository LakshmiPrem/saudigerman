                     ...
                     <div class="col-md-6 border-right">
                        ...
                        <?php
                           ...
                           <div class="row">
                            ...
                            <div class="row">
                             <?php $fee_statuses = get_proposal_fee_statuses(); ?>
                        <div class="col-md-12">
                              <div class="form-group select-placeholder">
                                 <label for="fee_status" class="control-label"><?php echo _l('proposal_fee_status'); ?></label>
                                 <select name="fee_status" class="selectpicker" data-width="100%"  data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                    <?php foreach($fee_statuses as $fee_status){ ?>
                                    <option value="<?php echo $fee_status->id; ?>" <?php if((isset($proposal) && $proposal->fee_status == $fee_status->id) ){echo 'selected';} ?>><?php echo $fee_status->name; ?></option>
                                    <?php } ?>
                                 </select>
                              </div>
                           </div>
                          ...
                          <div class="row">
                              <div class="col-md-6">
      <?php $value = (isset($proposal) ? $proposal->discussed_with : ''); ?>
      <?php echo render_input('discussed_with','discussed_with',$value); ?>
    </div>
    <div class="col-md-6">
      <?php $value = (isset($proposal) ? _d($proposal->discussed_on) : _d(date('Y-m-d'))); ?>
      <?php echo render_date_input('discussed_on','discussed_on',$value); ?>
    </div>  
                          </div>
                    ...
                    <div class="row">
                      <div class="col-md-12">
                        <hr>  
                            <div class="col-md-3">
                              <?php 
                              //$proposal_types = get_proposal_types();
                               $selected = (isset($proposal) ? $proposal->proposal_type : ''); ?>
                              <?php echo render_select('proposal_type',$proposal_types,array('id','name'),'type_of_proposal',$selected); ?>
                           </div>
                           <div class="col-md-3">
                              <?php $selected = (isset($proposal) ? $proposal->matter_template_id : ''); ?>
                              <?php echo render_select('matter_template_id',$matter_templates,array('id','name'),'matter_templates',$selected); ?>
                           </div>
                           <?php $matter_scopes_arr = array(); 
                            if(isset($proposal)){
                              if(isset($proposal->matter_scope)){
                                $matter_scopes_arr = $this->db->get_where('tblcasetemplate_scopes',array('casetemplate_id'=>$proposal->matter_template_id))->result_array();
                                $selected=[];
                                $matter_scopes = explode(',',$proposal->matter_scope);
                                foreach ($matter_scopes as $value) {
                                  $selected[] = $value;
                                }
                              }
                           } ?>
                           <div class="col-md-5">
                            <?php echo render_select('matter_scope[]',$matter_scopes_arr,array('id','scope_description'),'scope',$selected,array('multiple'=>true)); ?>
                              <!-- <div class="form-group">
                                <label>Select Scope of Service</label>
                                 <select class="form-control selectpicker" id="matter_scope" name="matter_scope[]" multiple>    
                                </select> 
                              </div> -->
                           </div>
                           <div class="col-md-1 mtop25">
                            <div class="form-group">
                              <button type="button" id="btn_scope_add" class="btn btn-info btn-sm">ADD</button>
                            </div>
                           </div>
                        </div>
                        ...
                        <?php #######  Scope Of Services ################?>
                        <?php $value = (isset($proposal) ? $proposal->scope_of_services : ''); ?>
                        <?php echo render_textarea('scope_of_services','scope_of_services',$value,array('rows'=>20)); ?>
                        </div>  


                         <div class="col-md-12">
      <?php ###############  Terms and conditions ##########################
      $value = (isset($proposal) ? $proposal->terms_conditions : ''); ?>
      <?php echo render_textarea('terms_conditions','proposal_terms_conditions',$value,array('rows'=>10,'cols'=>30)); ?>
...
<script type="text/javascript">
    var tmp_flag = 0;
    $("#matter_template_id").on("change",  function (event){
      if(tmp_flag <= 0) {
        var id = $(this).val();
        $.ajax({
          url: admin_url + 'casetemplates/get_matter_scopes/' + id,
          type: "GET",
          dataType: "JSON",
          success: function(response){ 
            var ctype = $('select[name="matter_scope[]"]');//$('#matter_scope');
            var options = [];
            $.each( response, function( key, value ) {
              var option = "<option value="+value.id+">" + value.scope_description + "</option>"
              options.push(option);
            });
            ctype.html(options);
     ...
     $("#btn_scope_add").click(function (event){
        
        tmp_flag = tmp_flag+1  ;
        var str = $("select[name='matter_scope[]']").val()+'';
        var splt = str.split(",");
        var d = '';
        var cnt = 1;
        $.each( splt, function(key,value ) {
          var text =  $("select[name='matter_scope[]'] option[value="+value+"]").text();
          d = d +'\n \n'+ cnt +'.  '+ text;
          cnt++;
        });
        $('#scope_of_services').html(d);
     });
