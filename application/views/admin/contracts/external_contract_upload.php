<div class="modal fade" id="model_contractupload" tabindex="-1" role="dialog" style="z-index:99999999;">
    <div class="modal-dialog">
        <?php echo form_open_multipart(admin_url('contracts/add_contractpdf'), array('id'=>'contract-upload-form')); ?>
...
<script>
appValidateForm($('#contract-upload-form'), {
    ...
    $.ajax({
        ...
        url: formURL
		 ...
		 $('#model_contractupload').modal('hide');
          $('#contract-upload-form').find('button[type="submit"]').button('reset');
             window.location.reload();// window.location.assign(response.url);
         
      });
      return false;
  }
function upload_contractfile(id){
