...
?>
<script>
    function shareholder(client_id, contact_id) {
		
        if (typeof(contact_id) == 'undefined') {
            contact_id = '';
        }
        requestGet('clients/form_shareholder/' + client_id + '/' + contact_id).done(function(response) {
        
            $('#shareholder_data').html(response);
            $('#shareholder').modal({
                show: true,
                backdrop: 'static'
            });
            ...
            $('body').on('shown.bs.modal', '#contact', function() {
                if (contact_id == '') {
                    $('#contact').find('input[name="share_percentage"]').focus();
                }
            });
            init_selectpicker();
            init_datepicker();
            custom_fields_hyperlink();
            validate_shareholder_form();
        }).fail(function(error) {
            var response = JSON.parse(error.responseText);
            alert_float('danger', response.message);
        });
    }
        function validate_shareholder_form() {
        appValidateForm('#shareholder-form', {
            shareholder_id: 'required',
           }, shareholderFormHandler);
    }
...
$(function() {
    ...
    if ($('#company_type').val() == 'branch') { $('.main_div').removeClass('hide'); }else{ $('.main_div').addClass('hide'); } 
    ...
    /* Shareholder table */
    initDataTable('.table-clientshareholders', admin_url+'clients/shareholder_table/'+customer_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);
   
    /* Subfile table */
    initDataTable('.table-client_constitution', admin_url+'clients/constitution_table/'+customer_id, ['undefined'], ['undefined'], undefined, [0, 'desc']);
    ...
    initDataTable('.table-contracts-single-client', admin_url + 'contracts/table/' + customer_id+'/'+'0'+'/'+'contracts', undefined,undefined, 'undefined', [0, 'desc']);
initDataTable('.table-pos-single-client', admin_url + 'contracts/table/' + customer_id+'/'+'0'+'/'+'po', undefined,undefined, 'undefined', [0, 'desc']);
...
function constitution(client_id, contact_id) {
    
        if (typeof(contact_id) == 'undefined') {
            contact_id = '';
        }
        requestGet('clients/form_constitution/' + client_id + '/' + contact_id).done(function(response) {
        
            $('#constitution_data').html(response);
            $('#constitution').modal({
                show: true,
                backdrop: 'static'
            });
            ...
            $('body').on('shown.bs.modal', '#contact', function() {
                if (contact_id == '') {
                    $('#contact').find('input[name="subject"]').focus();
                }
            });
            init_selectpicker();
            init_datepicker();
            custom_fields_hyperlink();
            validate_constitution_form();
        }).fail(function(error) {
            var response = JSON.parse(error.responseText);
            alert_float('danger', response.message);
        });
    }
        function validate_constitution_form() {
        appValidateForm('#constitution-form', {
           document_type: 'required',
            issue_date :'required',
           }, constitutionFormHandler);
    }
    
    function constitutionFormHandler(form) {
        ...
        $.ajax({
            ...
            url: formURL
               ...
               response = JSON.parse(response);
                ...
                if ($.fn.DataTable.isDataTable('.table-client_constitution')) {
                    $('.table-client_constitution').DataTable().ajax.reload(null,false);
                }
             
                    $('#constitution').modal('hide');
    ...
    function shareholderFormHandler(form) {
  ...
  $.ajax({
      ...
      url: formURL
         ...
         response = JSON.parse(response);
          ...
          if ($.fn.DataTable.isDataTable('.table-clientshareholders')) {
              $('.table-clientshareholders').DataTable().ajax.reload(null,false);
          }
...
$('#company_type').on('change',function(){
       ...
       if ($(this).val() == 'branch') { $('.main_div').removeClass('hide'); }else{ $('.main_div').addClass('hide'); } 
     });

function sendsubfile(invoker,id){
    var file_name = $(invoker).data('file_name');
    var file_path = $(invoker).data('file_path');
    var file_type = $(invoker).data('file_type');
    var email_id = $(invoker).data('email_id');
    $('#send_sub_file input[name="send_sub_file_subject"]').val(file_name);
    //$('#send_sub_file input[name="send_sub_file_email"]').val(email_id);
    $('#send_sub_file input[name="file_path"]').val(file_path);
    $('#send_sub_file input[name="filetype"]').val(file_type);
    $('#send_sub_file input[name="file_name"]').val(file_name);
    $('#send_sub_file').modal('show');
...
function update_displayname(th,id) {
      var display = $(th).val();
			
      var data={"display_name" : display};
      $.post(admin_url + 'clients/change_displayname_ajax/'+id, data).done(function(response) {
         response = JSON.parse(response);
         alert_float(response.alert, response.message);
      });
   } 
function update_sharepoint(id,$rel_id,$rel_type) {
		 $.post(admin_url + 'misc/update_attachfile_content/' + id+'/'+$rel_id+'/'+$rel_type).done(function (response) {
           response = JSON.parse(response);
		  alert_float(response.alert, response.message);
		  setTimeout(function() {
              window.location.reload();
...
initDataTable('.table-team_meeting', admin_url + 'clients/table_team_meeting/' + customer_id, undefined,undefined, 'undefined', [6, 'desc']);
</script>
