...
<script>
	$(function(){
	 ...
	 $('#newrequest').prop('disabled', false);
        ...
        {
			var error=false;
			rowCount = $("#dynamic_field_civilattach tr").length;
			
			var attcount=$('input[name="cvattachments[]"]').length;
			
			//var rowCount = $("#dynamic_field_civilattach tr").length;

			for (var i =1; i <= attcount; i++) {
				
 			//$("#cvattachments"+i).prop('required',true);
				var xx=$("#cvattachments" + i).val();
				alert(xx);
				if(xx==''){
					//error=true;
					//break;}
