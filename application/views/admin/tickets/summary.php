...
<div class="row">
 <?php
 $statuses = $this->tickets_model->get_ticket_status('',['ticketstatusid!='=>7]);
 ?>
 <div class="_filters _hidden_inputs hidden tickets_filters">
  ...
  echo form_hidden('my_tickets');
   ...
   for ($m = 1; $m <= 12; $m++) {
    echo form_hidden('tickets_by_month_'.$m);
}
  if(is_admin()){
    ...
    foreach($ticket_assignees as $assignee){
      echo form_hidden('ticket_assignee_'.$assignee['assigned']);
	   ...
	   $ticket_branches = $this->tickets_model->get_tickets_branch_disctinct();
    foreach($ticket_branches as $branch){
      echo form_hidden('ticket_branch_'.$branch['userid']);
    }
	  $ticket_services = $this->tickets_model->get_tickets_services_disctinct();
    foreach($ticket_services as $service){
      echo form_hidden('ticket_services_'.$service['serviceid']);
    }
  }
  ...
  if (!is_admin()) {
    if (get_option('staff_access_only_assigned_departments') == 1) {
     ...
     if(count($departments_ids) > 0){
	  ...
	  $where=$where . 'OR tbltickets.userid IN (SELECT customer_id FROM tblcustomer_admins WHERE staff_id=' . get_staff_user_id() . ')';
}
foreach($statuses as $status){
  $_where = '';
  if($where == ''){
