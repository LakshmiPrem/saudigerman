...
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable,array(),array(),array('id','head_order','rel_type'));
$output  = $result['output'];
$rResult = $result['rResult'];

foreach ($rResult as $aRow) {
    $row = array();
  	 $row[] = '<a href="#" onclick="edit_approval_heading(this,'.$aRow['id'].'); return false;" data-name="'.$aRow['name'].'" data-order="'.$aRow['head_order'].'" data-type="'.$aRow['rel_type'].'">' . $aRow['name']. '</a> ';
    ...
    $row[] = $toggleActive;
    $options = icon_btn('#', 'pencil-square-o','btn-default',array('onclick'=>'edit_approval_heading(this,'.$aRow['id'].'); return false;','data-name'=>$aRow['name'],'data-order'=>$aRow['head_order'],'data-type'=>$aRow['rel_type']));
