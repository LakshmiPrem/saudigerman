...
$aColumns = [
	...
	'amendment_number',
	 '(SELECT tblaction_categories.category_name FROM tblaction_categories WHERE tblaction_categories.id=tblcontracts.postcontract_status ) as postcontract_status1',
    ];
...
array_push($filter, '(signed=1 or marked_as_signed=1)');
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'contracts.id', 'trash', 'client', 'hash', 'marked_as_signed', 'project_id','other_party','postcontract_status']);
...
foreach ($rResult as $aRow) {
   ...
   /* if (has_permission('contracts', '', 'delete')) {
    ...
    if($aRow['signed_contract_filename'] != ''){
        $row[] = '<a download href="'.$path.'"  class="btn btn-primary maleft10"><i class="fa fa-download"></i>'._l('download').'</a>';
    }else{
        $row[] = '';
    }
	$option1='';
	  $row[] = !empty($aRow['amendment_number'])?get_amendment_latest_by_id($aRow['amendment_number'],$aRow['id']):'';
	$pstatus=!empty($aRow['postcontract_status1'])?$aRow['postcontract_status1']:'';
	
	if(!empty($pstatus)){
		$topaction=!empty($aRow['postcontract_status'])?get_postaction_latest_by_id($aRow['postcontract_status'],$aRow['id']):'';
	$option1=' <p class="card-text" style="margin:  0 0 4px;"> <button type="button" class="btn btn-default btn-sm btn-icon mleft10  pop" data-container="body" data-toggle="popover" data-placement="top" data-content="'.$topaction->description.'"
    data-original-title="'._d($topaction->due_date).'" title="'.get_staff_full_name($topaction->created_by).'"> <i class="fa fa-tag"></i></button> |   <span><b>'.$pstatus.'</b></span></p>';
	}
	 $row[] = !empty($aRow['postcontract_status'])? $option1 : '';
	$options= '<a href="#" data-toggle="tooltip" data-title="'. _l('contract_postaction').'" class="btn btn-info  btn-icon" onclick="upload_contractreview(' . $aRow['id'] .'); return false;">
            <i class="fa fa-comment"></i>'._l('post_review').'
                </a>';
	$row[]=$options;
