...
if($this->ci->input->post('clientid')){
    array_push($where, ' AND clientid=' . $this->ci->input->post('clientid'));
}

if( $this->ci->input->post('case_type') != null ){
    array_push($where, ' AND case_type ="' . $this->ci->input->post('case_type').'"');
}

if($this->ci->input->post('work_type')){
    
    array_push($where, ' AND ' . db_prefix() . 'projects.case_type IN (SELECT id FROM ' . db_prefix() . 'project_types WHERE  type ="'.$this->ci->input->post('work_type') .'" )');
}
