...
$branches  = $this->ci->tickets_model->get_tickets_branch_disctinct();

$_branch = [];
foreach ($branches as $__branch) {
    if ($this->ci->input->post('ticket_branch_' . $__branch['userid'])) {
        array_push($_branch, $__branch['userid']);
	
    }
}
if (count($_branch) > 0) {
    array_push($filter, 'AND tbltickets.userid IN (' . implode(', ', $_branch) . ')');
}
$services  = $this->ci->tickets_model->get_tickets_services_disctinct();

$_service = [];
foreach ($services as $__service) {
    if ($this->ci->input->post('ticket_services_' . $__service['serviceid'])) {
        array_push($_service, $__service['serviceid']);
	
    }
}
if (count($_service) > 0) {
    array_push($filter, 'AND tbltickets.service IN (' . implode(', ', $_service) . ')');
}
$years      = $this->ci->tickets_model->get_tickets_years();
$yearsArray = [];
...
$monthArray = [];
for ($m = 1; $m <= 12; $m++) {
    if ($this->ci->input->post('tickets_by_month_' . $m)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        //    $_data = '<a href="' . $url . '" class="valign">' . $_data . '</a>';
            if ($aColumns[$i] == 'subject') {
				...
				if (has_permission('tickets','','delete') ){//is_admin()
            ...
            $_data =get_opposite_party_name($aRow['opposteparty']);//render_tags($_data);
