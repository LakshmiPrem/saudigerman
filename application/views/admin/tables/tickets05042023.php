...
$aColumns = [
  ...
  'opposteparty',
	...
	db_prefix() . 'tickets.estimated_date',
    ];
        ...
        if ($aColumns[$i] == '1') {
            ...
            if ($aColumns[$i] == 'subject') {
                ...
                $_data .= ' <span class="text-dark"> | </span><a href="' . get_ticket_public_url($aRow) . '" target="_blank">' . _l('view_public_form') . '</a>';
                 if (has_permission('tickets','','delete')&& $aRow['assigned']==get_staff_user_id() && $aRow['assigned'] != 0){
            ...
            if ($aRow['userid'] != 0) {
                $_data = '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '">' . $aRow['company'];
            ...
            $_data = _dt($_data);
        } elseif ($aColumns[$i] == db_prefix() . 'tickets.estimated_date') {
            $_data = _d($_data);
