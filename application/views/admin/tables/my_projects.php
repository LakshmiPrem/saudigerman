...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    '(SELECT GROUP_CONCAT(staff_id SEPARATOR ",") FROM ' . db_prefix() . 'project_members WHERE project_id=' . db_prefix() . 'projects.id ORDER BY staff_id) as members_ids','(select file_name from tblproject_files where project_id='. db_prefix() . 'projects.id ORDER BY tblproject_files.id desc limit 1) as final_logo','ip_filingno'
...
foreach ($rResult as $aRow) {
    ...
    $row[] = ($aRow['case_type']=='intellectual_property')?$aRow['ip_filingno']:$aRow['file_no'];
	...
	$row[]=$aRow['ledger_code'];
		$display='';
  $total_attach = total_rows(db_prefix().'project_files',
                          array(
                           'project_id'=>$aRow['id'],'final_doc'=>1
                         )
                        );
                        if($total_attach > 0){
							 $path = base_url('uploads/projects/'.$aRow['id'].'/'.$aRow['final_logo']);
							if (is_image($path)) {
	 
	$display='<img src="'.$path.'"  class="avatar staff-profile-image-small" height="50%" width="50%" >';
							}else{
	$display='<a href="' . site_url('uploads/projects/' . $aRow['id'].'/'.$aRow['final_logo']) . '" download>' . $aRow['final_logo'] . ' <i class="fa fa-download"></i> </a>';							
							}
						}else{
	  $path = base_url('uploads/projects/'.$aRow['id'].'/'.$aRow['ip_logo']);
                if (is_image($path)) {
     
    $display='<img src="'.$path.'"  class="avatar staff-profile-image-small" height="50%" width="50%" >';
                            }else{
    $display='<a href="' . site_url('uploads/projects/' . $aRow['id'].'/'.$aRow['ip_logo']) . '" download>' . $aRow['final_logo'] . ' <i class="fa fa-download"></i> </a>';                          
                            }
						}
