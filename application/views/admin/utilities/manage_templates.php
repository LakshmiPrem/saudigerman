...
<!------------------  Filter Start -------------------------------------->
						...
						<div class="row">
							 <div class="col-md-3">
         <button type="button" class="btn btn-info" onclick="add_template('contracts', 2);"><?php echo _l('add_template'); ?></button>
         <hr>
      </div>
...
<!------------------  Filter End -------------------------------------->
						...
						$custom_fields = get_custom_fields('all_templates',array('show_on_table'=>1));
						foreach($custom_fields as $field){
							array_push($table_data,$field['name']);
						}
						render_datatable($table_data,'all_templates');
						?>
					</div>
				</div>
			</div>
...
<script>
	$(function(){
		...
		ProjectsServerParams['related_user'] = '[name="related_user"]';
		ProjectsServerParams['start_date'] = '[name="start_date"]';
		 ...
		 var tAPI=initDataTable('.table-all_templates',window.location.href,undefined,undefined,ProjectsServerParams,[0, 'desc']);
