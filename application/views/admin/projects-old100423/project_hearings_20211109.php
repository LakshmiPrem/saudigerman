  ...
  <tbody>
   <?php 
      foreach ($hearings as $row_hearing) {
        ...
        <td><?=$row_hearing->court_fee?></td>
        <td><?=$row_hearing->court_no?></td>
        <td><?=$row_hearing->proceedings?></td>
        <td><a class="btn btn-danger _delete btn-icon" href="<?php echo admin_url('projects/delete_hearing/'.$row_hearing->project_id.'/'.$row_hearing->id) ?>"><i class="fa fa-remove"></i></a></td>
      </tr>
      <?php }} ?>  
  </tbody>
 </table>
