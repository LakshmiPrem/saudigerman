   ...
   <div class="content">
      <div class="row">
         ...
         <div class="col-md-6">
            <div class="panel_s">
               <div class="panel-body">
                  ...
                  <?php if(isset($expense) && $expense->docfiles !== ''){ ?>
                  <div class="row">
                    <?php foreach($expense->docfiles as $expense1){?>
                     <div class="col-md-10">
                        <i class="<?php echo get_mime_class($expense1['filetype']); ?>"></i> <a href="<?php echo site_url('download/file/expense/'.$expense1['rel_id']); ?>"><?php echo $expense1['file_name']; ?></a>
                     </div>
                     <?php if($expense1['staffid'] == get_staff_user_id() || is_admin()){ ?>
                     <div class="col-md-2 text-right">
                        <a href="<?php echo admin_url('expenses/delete_expense_attachment/'.$expense1['rel_id']); ?>" class="text-danger _delete"><i class="fa fa fa-times"></i></a>
                     </div>
                     <?php }
						}?>
                  </div>
                  ...
                  <?php if(!isset($expense) || (isset($expense) && $expense->docfiles==null)){ ?>
           ...
           <div class="hide">
               <?php
				$approvals         = get_common_approvals($expense->project_id); 
				     $app_attr = array('disabled'=>true);
                $selected = (isset($expense) ? $expense->approvalid : '');
                 echo render_select('approvalid',$approvals,array('id','approval_name'),'approval_name',$selected,$app_attr);
               ?>
				</div>   
                   <div class="row">
				    <div class="col-md-6">
                  ...
                  <?php echo render_input('amount','expense_add_edit_amount',$value,'number');
                     ...
                     if((isset($expense) && ($expense->billable == 1 || $expense->refundable == 1 || $expense->clientid != 0)) || isset($customer_id)){
                          $hide_billable_options = '';
                     }
						?>
                      </div>
                       <div class="col-md-6">
                  <?php $value = (isset($expense) ? $expense->paid_amount : ''); ?>
                  <?php echo render_input('paid_amount','paid_amount',$value,'number',array('readonly' => 'readonly')); ?></div>
				 <div class="col-md-12"> 
                  <?php $value = (isset($expense) ? $expense->balance_amount : ''); ?>
                  <?php echo render_input('balance_amount','balance',$value,'number',array('readonly' => 'readonly'));?></div>
				   </div>
                  ...
                  <div class="row hide">
                    <div class="col-md-6">
                  <?php $value = (isset($expense) ? $expense->last_amount+$expense->vat_amount : ''); ?>
                  <?php echo render_input('lastpaid_amount','last_paid',$value,'number'); ?></div>
               <div class="col-md-6">
                  <?php $value = (isset($expense) ? $expense->last_amount : ''); ?>
                  <?php echo render_input('last_amount','last_amount','0','number',array('readonly' => 'readonly')); ?></div>
                   <div class="col-md-6"> 
                  <?php $value = (isset($expense) ? $expense->vat_amount : ''); ?>
                  <?php echo render_input('vat_amount','vat','0','number',array('readonly' => 'readonly')); ?></div>
                  ...
                  <?php $hide_project_selector = ' hide';
                     ...
                     ?>
                  <div class="form-group projects-wrapper<?php echo $hide_project_selector; ?> hide">
                  ...
                  <?php
                $selected = (isset($expense) ? $expense->erp_otherparty : '');
                 if($selected == ''){
                          $selected = (isset($opposite_id) ? $opposite_id: '');
					   }
				echo render_select('erp_otherparty',$other_parties,array('id','name'),'other_party',$selected);
               ?>
                  <?php
         ...
         <div class="col-md-6">
            <div class="panel_s">
               <div class="panel-body">
                  ...
                  <div class="row">
                       <div class="col-md-12">
                  <div class="checkbox checkbox-primary billable <?php echo $hide_billable_options; ?>">
               <input type="checkbox" id="refundable" name="refundable" onchange="valueChanged()" <?php if(isset($expense)){if($expense->refundable == 1){echo 'checked';}}; ?>>
               <label for="refundable"><?php echo _l('expense_add_edit_refund'); ?></label>
            </div>
				   </div>
                    <div class="row <?php echo $hide_billable_options; ?>" id="reanswer">
               <div class="col-md-6">
                 <?php $value = (isset($expense) ? $expense->refund_amount : ''); ?>
                 <?php echo render_input('refund_amount','refund_amount',$value,'number'); ?></div>
              <div class="col-md-6">
                   <?php $value = (isset($expense) ? _d($expense->date) : _d(date('Y-m-d')));?>
				  <?php echo render_date_input('refund_date','refund_date',$value); ?></div>
                    <div class="col-md-6">
           ...
           <div class="col-md-6">
                     <?php $value = (isset($expense) ? $expense->refund_remark : ''); ?>
                   <?php echo render_input('refund_remark','expense_refund_remark',$value); ?>
               </div>
					  </div>
                  ...
                  <div class="form-group select-placeholder hide"<?php if(isset($expense) && !empty($expense->recurring_from)){ ?> data-toggle="tooltip" data-title="<?php echo _l('create_recurring_from_child_error_message', [_l('expense_lowercase'),_l('expense_lowercase'), _l('expense_lowercase')]); ?>"<?php } ?>>
...
<script>
   ...
   <?php } ?>
     $(function(){
        $('body').on('change','#project_id', function(){
			...
			$('#approvalid option:not(:selected)').prop('disabled', true);
   ...
   $('#expense-form input[name="amount"] ,input[name="lastpaid_amount"],input[name="last_amount"],input[name="vat_amount"] ,input[name="balance_amount"]').blur(function(){
	  
         var total_amount = $('#expense-form input[name="amount"]').val();
         var paid_amount = $('#expense-form input[name="paid_amount"]').val();
	     var last_amount = $('#expense-form input[name="last_amount"]').val();
	     var lastpaid_amount = $('#expense-form input[name="lastpaid_amount"]').val();
	    var vat_amount = $('#expense-form input[name="vat_amount"]').val();
	   if(isNaN(vat_amount)||vat_amount=='')vat_amount=0;
	    if(isNaN(paid_amount)||paid_amount=='')paid_amount=0;
         var balance = $('#expense-form input[name="balance_amount"]').val();// alert(last_amount);
	   var tt=parseFloat(paid_amount)+parseFloat(lastpaid_amount)+parseFloat(last_amount)+parseFloat(vat_amount);
         var balance = total_amount -tt;
         $('#expense-form input[name="balance_amount"]').val(parseFloat(balance).toFixed(2));
