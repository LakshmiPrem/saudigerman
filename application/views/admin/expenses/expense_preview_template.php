 ...
 <div class="col-md-12">
  ...
  <table class="table dt-table scroll-responsive table-project-hearings" data-order-col="1" data-order-type="desc">
						...
						<thead>
							<tr>
								<th><?php echo _l('#') ?></th>
								<th><?php echo _l('expense_add_edit_amount') ?></th>
						...
						<tbody>
							<?php 
						if(sizeof($expense->payment_data)>0){
							$i=1;
							foreach ($expense->payment_data as $payments) { 
								
								?>
							<tr>
								<td><?php echo $i; ?></td>
								<td><?php echo $payments['exp_paid_amount']; ?></td>
								<td><?php echo $payments['exp_paid_date']; ?></td>
								<td><?php echo $payments['exp_description']; ?></td>
								<td> <?php
								 $path = get_upload_path_by_type('expense') . $payments['expense_id'] . '/'. $payments['exp_filename'];
								if(file_exists($path)){?>
            <a href="<?php echo site_url('download/downloadreceiptfile/'.$payments['expense_id'].'/'.$payments['id']); ?>" class="btn btn-info btn-icon"><i class="fa fa-download"></i></a>
           <?php } ?></td>
                        <td><?php echo icon_btn('expenses/delete_payment/' . $payments['id'].'/'.$expense->expenseid, 'remove', 'btn-danger _delete'); ?></td>
								</tr>	
							<?php $i++; } ?>
						<?php } else{?>
							<tr><td><?php echo _l('no_data_found'); ?></td></tr>	
					<?php } ?>
