			...
			<div class="col-md-12">
				<div class="panel_s">
					<div class="panel-body">
						...
						<?php } ?>
							<?php if(get_option('services') == 1){ ?>
									...
									<?php if(is_admin() || get_option('staff_members_create_inline_ticket_services') == '1'){
										echo render_select_with_input_group('service',$services,array('serviceid','name'),'ticket_settings_service','','<a href="#" onclick="new_service();return false;"><i class="fa fa-plus"></i></a>',array('required'=>'true'));
									} else {
										echo render_select('service',$services,array('serviceid','name'),'ticket_settings_service','',array('required'=>'true'));
									}
									?>
						...
						<div class="form-group select-placeholder" id="ticket_contact_w">
							...
							<select name="contactid" required id="contactid" class="ajax-search" data-width="100%" data-live-search="true" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
								...
								<?php if(isset($contact)) { ?>
									<option value="<?php echo $contact['id']; ?>" selected><?php echo get_company_name($contact['userid']); ?></option>
								<?php } ?>
								<option value=""></option>
							</select>
						...
						<div class="row">
						<div class="col-md-6">
								<?php echo render_input('stamp_type','type_stamp','','text'); ?>
							</div>
							<div class="col-md-6">
					...
					<div class="col-md-6">
						...
						<div class="form-group select-placeholder">
							...
							<select name="assigned" id="assigned" class="form-control selectpicker" data-live-search="true" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" data-width="100%" required>
								<?php  if(get_option('enable_single_assignee') != 1){ ?>
						...
						<div class="row">
							<div class="col-md-<?php if(get_option('services') == 1){ echo 6; }else{echo 12;} ?>">
						  	...
						  	<div class="col-md-6 hide">
								<?php echo render_select('department',$departments,array('departmentid','name'),'ticket_settings_departments',(count($departments) == 1) ? $departments[0]['departmentid'] : $departments[0]['departmentid'] ); ?>
							</div>
							<div class="col-md-6">
						
							<?php 
							echo render_select_with_input_group('opposteparty',$oppositeparty_names,array('id','name'),'opposite_party','','<a href="#" onclick="new_opposite_party();return false;"><i class="fa fa-plus"></i></a>');//'<a target="_Blank" href="'.admin_url('opposite_parties/opposite_party').'"><i class="fa fa-plus"></i></a>'); ?>
