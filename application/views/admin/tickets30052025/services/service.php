    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <div id="additional_ticket_service"></div>
...
<script>
    window.addEventListener('load',function(){
        ...
        $('#ticket-service-modal').on('hidden.bs.modal', function(event) {
            $('#additional_ticket_service').html('');
            $('#ticket-service-modal input[name="name"]').val('');
			 $('#ticket-service-modal input[name="shortname"]').val('');
    ...
    function edit_service(invoker,id){
        var name = $(invoker).data('name');
		 var shortname = $(invoker).data('type');
        $('#additional_ticket_service').append(hidden_input('id',id));
        $('#ticket-service-modal input[name="name"]').val(name);
		 $('#ticket-service-modal input[name="shortname"]').val(shortname);
        $('#ticket-service-modal').modal('show');
