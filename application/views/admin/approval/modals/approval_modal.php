...
<!-- ✅ Common Remarks Field (moved above table) -->
<div class="col-md-12">
  <div class="form-group">
    <label for="approval_remarks"><?php echo _l('remarks'); ?></label>
    <?php 
      $existing_remarks = '';
      $approval_stages_selected = get_approvals($rel_id,$rel_name);
      if(sizeof($approval_stages_selected) > 0) {
        $existing_remarks = $approval_stages_selected[0]['approval_remarks'];
      }
    ?>
    <textarea class="form-control" name="approval_remarks" id="approval_remarks" rows="3" placeholder="Enter remarks for all"><?php echo $existing_remarks; ?></textarea>
  </div>
</div>
