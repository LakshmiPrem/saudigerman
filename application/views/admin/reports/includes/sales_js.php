...
<script>
 ...
 var report_receivables = $('#receivables-report');
 var report_referral = $('#referrals-report');
 var fnServerParams = {
...
$('.table-receivables-report').on('draw.dt', function() {
     var invoiceReportsTable = $(this).DataTable();
     var sums = invoiceReportsTable.ajax.json().sums;
     add_common_footer_sums($(this),sums);
     $(this).find('tfoot td.amount_open').html(sums.amount_open);
 ...
 $('.table-referrals-report').on('draw.dt', function() {
     var invoiceReportsTable = $(this).DataTable();
     var sums = invoiceReportsTable.ajax.json().sums;
     add_common_footer_sums($(this),sums);
     $(this).find('tfoot td.amount_open').html(sums.amount_open);
 ...
 function init_report(e, type) {
   ...
   report_receivables.addClass('hide');
   report_referral.addClass('hide');
   ...
   // Clear custom date picker
      ...
      }else if(type == 'receivables-report'){
          report_receivables.removeClass('hide');
      }else if(type == 'referrals-report'){
          report_referral.removeClass('hide');
      }
      gen_reports();
    }
 ...
 function items_report(){
  ...
  function receivables_report() {
   ...
   function referrals_report() {
     if ($.fn.DataTable.isDataTable('.table-referrals-report')) {
       $('.table-referrals-report').DataTable().destroy();
     }
     _table_api = initDataTable('.table-referrals-report', admin_url + 'reports/referrals_report', false, false, fnServerParams, [
       [2, 'DESC'],
       [0, 'DESC']
       ]);
   }
   ...
   function gen_reports() {
     ...
     if (!$('.chart-income').hasClass('hide')) {
      ...
      credit_notes_report();
    }else if(!report_receivables.hasClass('hide')) {
      receivables_report();
    }else if(!report_referral.hasClass('hide')) {
      referrals_report();
    }
  }
</script>
