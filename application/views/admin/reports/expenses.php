                        ...
                        <div class="panel-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover expenses-report" id="expenses-report-table">
                                    ...
                                    <tbody>
                                        <?php
                                        $taxTotal = array();  $taxTotal1 = array();
                                        $netAmount = array(); $netAmount1 = array();
                                        $totalNetByExpenseCategory = array();$totalNetByExpenseCategory1 = array();
                                        ...
                                        <tr>
                                            ...
                                            // Set the monthly total expenses array
                                                if(!isset($netMonthlyTotal[$m])){
												 ...
												 if(!isset($netMonthlyTotal1[$m])){
                                                    $netMonthlyTotal1[$m] = array();
                                                }
                                                ...
                                                ->from(db_prefix().'expenses')
                                                ->where('MONTH(date)',$m)
                                                ->where('YEAR(date)',$current_year-1)
                                                ->where('category',$category['id'])
                                                ->where('currency',$_currency->id);
                                                ...
                                                $expenses1 = $this->db->get()->result_array();

                                                $total_expenses1 = array();
                                                echo '<td>';
                                                foreach($expenses as $expense){
                                                    $expense = $this->expenses_model->get($expense['id']);
                                                    $total = $expense->paid_amount;
                                                    ...
                                                    if($expense->tax2 != 0){
                                                        $totalTaxByExpense += ($expense->paid_amount / 100 * $expense->taxrate2);
                                                    }
												 ...
												 foreach($expenses1 as $expense1){
                                                    $expense1 = $this->expenses_model->get($expense1['id']);
                                                    $total1 = $expense1->paid_amount;

                                                    $totalTaxByExpense1 = 0;
                                                    // Check if tax is applied
                                                    if($expense1->tax != 0){
                                                        $totalTaxByExpense1 += ($total1 / 100 * $expense1->taxrate);
                                                    }

                                                    if($expense1->tax2 != 0){
                                                        $totalTaxByExpense1 += ($expense1->paid_amount / 100 * $expense1->taxrate2);
                                                    }

                                                    $taxTotal1[$m][] = $totalTaxByExpense1;
                                                    $total_expenses1[] = $total1;
                                                }
                                                $total_expenses = array_sum($total_expenses);
												 $total_expenses1 = array_sum($total_expenses1);
                                                // Add to total monthy expenses
                                                array_push($netMonthlyTotal[$m],$total_expenses);
												 array_push($netMonthlyTotal1[$m],$total_expenses1);
                                                ...
                                                array_push($totalNetByExpenseCategory[$category['id']],$total_expenses);
												 if(!isset($totalNetByExpenseCategory1[$category['id']])){
                                                    $totalNetByExpenseCategory1[$category['id']] = array();
                                                }
                                                array_push($totalNetByExpenseCategory1[$category['id']],$total_expenses1);
                                            ...
                                            <tr class="bg-odd">
                                                <td class="bold text-info">
                                                    <?php echo _l('expenses_report_net1'); ?>
                                                </td>
                                                <?php if(isset($netMonthlyTotal1)) { ?>
                                                <?php foreach($netMonthlyTotal1 as $month => $total1){
                                                    $total1 = array_sum($total1);
                                                    $netMonthlyTotal1[$month] = $total1;
                                                    $current_year_total1[] = $total1;
                                                    ?>
                                                    <td class="bold">
                                                        <?php echo app_format_money($total1, $_currency); ?>
                                                    </td>
                                                    <?php } ?>
                                                    <?php } ?>
                                                    <td class="bold bg-odd">
                                                        <?php
                                                        $totalNetByExpenseCategorySum1 = 0;
                                                        foreach($totalNetByExpenseCategory1 as $totalCat1) {
                                                            $totalNetByExpenseCategorySum1 += array_sum($totalCat1);
                                                        }
                                                        echo app_format_money($totalNetByExpenseCategorySum1, $_currency);
                                                        ?>
                                                    </td>
                                                </tr>
                                            <!--    <tr class="bg-odd">
