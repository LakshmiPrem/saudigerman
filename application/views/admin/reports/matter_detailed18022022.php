    ...
    if ($project->billing_type == 1) {
        ...
        $html = '<b>' . _l('project_rate_per_hour') . ': </b>' . app_format_money($project->project_rate_per_hour, $project->currency_data) . '<br />';
                                 ...
                                 <tbody>
                                    <tr>
                                      <?php $casetypes=get_case_client_types();
										...
										<td><div class="col-md-4"><b><?=_l('case_type')?></div><div class="col-md-8"></b><?=$type1;?></div><br><div class="col-md-4">

                                       <?php $status = get_project_status_by_id($project->status); ?>
                                       <b><?=_l('project_status') ?></div><div class="col-md-8"></b><?=$status['name']?></div><br><div class="col-md-4">

                                       <b><?=_l('civil') ?></div><div class="col-md-8"></b><?=_d($project->project_created)?></div><br><div class="col-md-4">

                                       <b><?=_l('legal_requestdt') ?></div><div class="col-md-8"></b><?=_d($project->start_date)?></div><br><div class="col-md-4">
                                      
                                       <b><?=_l('total_project_members') ?></div><div class="col-md-8"></b><?=$total_members?></div><br><div class="col-md-4">
                                       <b><?=_l('total_project_files') ?></div><div class="col-md-8"></b><?=$total_files_attached?></div>
                                       ...
                                       <td>
                                          ...
                                          <b><?=_l('claiming_amount') ?>:</b><?=app_format_money($project->claiming_amount, $project->currency_data)?><br>
                                          
                                           <b><?=_l('project_overview_expenses') ?>:</b><?=app_format_money(sum_from_table(db_prefix() . 'expenses', ['where' => ['project_id' => $project->id], 'field' => 'paid_amount']), $project->currency_data) ?><br>
									
									
                                    <b><?=_l('total_amount') ?>:</b><?php
											$exp=sum_from_table(db_prefix() . 'expenses', ['where' => ['project_id' => $project->id], 'field' => 'paid_amount']);
											$tamount=app_format_money($project->claiming_amount+$exp, $project->currency_data);echo $tamount; ?><br>
                                          <b><?=_l('execution_amount') ?>:</b><?=app_format_money($project->execution_amount, $project->currency_data)?><br>
                                          <b><?=_l('settlement_amount') ?>:</b><?=app_format_money($project->outstanding_amount, $project->currency_data)?><br>
                                          <b><?=_l('project_overview_settle_paid') ?>:</b><?php echo app_format_money(sum_from_table(db_prefix().'recoveries_installments',array('where'=>array('recovery_id'=>$project->id,'installment_status'=>'paid'),'field'=>'installment_amount')), $project->currency_data); ?><br>
                                           <b><?=_l('project_overview_settle_balance') ?>:</b><?php
			 $total=$project->outstanding_amount;
			$paid=sum_from_table(db_prefix().'recoveries_installments',array('where'=>array('recovery_id'=>$project->id,'installment_status'=>'paid'),'field'=>'installment_amount'));
												  $bal=$total-$paid;
												  echo app_format_money($bal, $project->currency_data); ?><br>
         ...
         <table class="table dt-table table-bordered table-striped" width="100%"  cellspacing="0" cellpadding="5" border="1">
            <thead>
               <tr>
                  ...
                  <th><b><?=_l('date_added')?></b></th>
                  <th><b><?=_l('details')?></b></th>
