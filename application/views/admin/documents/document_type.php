<div class="modal fade" id="type" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(admin_url('documents/type'), array('id'=>'contract-type-form')); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">
                    <span class="edit-title"><?php echo _l('document_type_edit'); ?></span>
                    <span class="add-title"><?php echo _l('new_document_type'); ?></span>
                </h4>
            </div>
    ...
    $.post(url, data).done(function(response) {
        ...
        if(response.success == true){
            ...
            // if($('body').hasClass('contract') && typeof(response.id) != 'undefined') {
                ...
                var ctype1 = $('#contract_type');
                
                ctype1.find('option:first').after('<option value="'+response.id+'">'+response.name+'</option>');
                // alert(response.id);
                ctype1.selectpicker('val',response.id);
                ctype1.selectpicker('refresh');
            // }
        }
        if($.fn.DataTable.isDataTable('.table-document-types')){
            // alert(1);
            $('.table-document-types').DataTable().ajax.reload();
        }
        $('#type').modal('hide');
    });
    return false;
