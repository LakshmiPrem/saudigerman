...
<!-- mention and tommorrow mention -->
    ...
    $total_projects_in_progress = count_active_contracts();
    $percent_in_progress_projects = ($total_projects > 0 ? number_format(($total_projects_in_progress * 100) / $total_projects,2) : 0);
    ?>
    <div class="top_stats_wrapper contract-status-box" style="border: 1px solid <?php echo $project_status['color']; ?>; cursor: pointer; transition: all 0.3s;" data-status-type="active">
...
<div class="quick-stats-projects col-xs-12 col-md-6 col-sm-6 <?php echo $initial_column; ?>">
    ...
    $total_projects = total_rows(db_prefix().'contracts',$_where);
    $where = ($_where == '' ? '' : $_where.' AND ').'status = 3';
    $total_projects_in_progress = count_expired_contracts();
    $percent_in_progress_projects = ($total_projects > 0 ? number_format(($total_projects_in_progress * 100) / $total_projects,2) : 0);
    ?>
    <div class="top_stats_wrapper contract-status-box" style="border: 1px solid <?php echo $project_status['color']; ?>; cursor: pointer; transition: all 0.3s;" data-status-type="expired">
...
<div class="quick-stats-projects col-xs-12 col-md-6 col-sm-6 <?php echo $initial_column; ?>">
    ...
    $total_projects = total_rows(db_prefix().'contracts',$_where);
    $where = ($_where == '' ? '' : $_where.' AND ').'status =1';
    $total_projects_in_progress = count_ongoing_contracts();
    $percent_in_progress_projects = ($total_projects > 0 ? number_format(($total_projects_in_progress * 100) / $total_projects,2) : 0);
    ?>
    <div class="top_stats_wrapper contract-status-box" style="border: 1px solid <?php echo $project_status['color']; ?>; cursor: pointer; transition: all 0.3s;" data-status-type="ongoing">
        <p class="text-uppercase mtop5"><i class="hidden-sm fa fa-cubes"></i> <?php echo _l('contracts') . ' ' . _l('contract_summary_ongoing'); ?><span class="pull-right"><?php echo $total_projects_in_progress; ?> / <?php echo $total_projects; ?></span></p>
        <div class="clearfix"></div>
        <div class="progress no-margin progress-bar-mini">
            <div class="progress-bar no-percent-text not-dynamic" style="background:<?php echo $project_status['color']; ?>" role="progressbar" aria-valuenow="<?php echo $percent_in_progress_projects; ?>" aria-valuemin="0" aria-valuemax="100" style="width: 0%" data-percent="<?php echo $percent_in_progress_projects; ?>">
            </div>
...
<div class="quick-stats-projects col-xs-12 col-md-6 col-sm-6 <?php echo $initial_column; ?>">
    ...
    $total_projects = total_rows(db_prefix().'contracts',$_where);
    $where = ($_where == '' ? '' : $_where.' AND ').'status =1';
    $total_projects_in_progress = count_expired_in_three_months_contracts();
    $percent_in_progress_projects = ($total_projects > 0 ? number_format(($total_projects_in_progress * 100) / $total_projects,2) : 0);
    ?>
    <div class="top_stats_wrapper contract-status-box" style="border: 1px solid <?php echo $project_status['color']; ?>; cursor: pointer; transition: all 0.3s;" data-status-type="expiring">
        <p class="text-uppercase mtop5"><i class="hidden-sm fa fa-cubes"></i> <?php echo _l('contracts') . ' ' . _l('contract_summary_expity_in_three_months'); ?><span class="pull-right"><?php echo $total_projects_in_progress; ?> / <?php echo $total_projects; ?></span></p>
        <div class="clearfix"></div>
        <div class="progress no-margin progress-bar-mini">
            <div class="progress-bar no-percent-text not-dynamic" style="background:<?php echo $project_status['color']; ?>" role="progressbar" aria-valuenow="<?php echo $percent_in_progress_projects; ?>" aria-valuemin="0" aria-valuemax="100" style="width: 0%" data-percent="<?php echo $percent_in_progress_projects; ?>">
            </div>
...
<div style="background:#f9fafb; border:1px solid #ddd; border-radius:10px; padding:25px; margin-top:20px; box-shadow:0 2px 8px rgba(0,0,0,0.05);">
    <h4 style="margin-bottom:25px; font-weight:600; color:#333;"><?php echo _l('contract_summary_heading'); ?><div class="col-md-2 pull-right mtop10"> <?php if (has_permission('contracts','','create')) { ?>
                     <a onclick="new_quick_contract();return false;" href="#" class="btn btn-success mright5 mtop12 test pull-right display-block">
                     <i class="fa fa-plus" aria-hidden="true"></i></a> <?php } ?></div></h4>
  ...
  <div class="summary-row">
    ...
    <a href="<?php echo admin_url('contracts'); ?>" style="text-decoration:none; color:inherit;">
      <div class="summary-card">
        ...
        <h3 class="summary-value" style="color:#3f51b5;"><?php echo $total_projectss; ?></h3>
      </div>
    </a>

    <a href="<?php echo admin_url('contracts?type=payable'); ?>" style="text-decoration:none; color:inherit;">
      <div class="summary-card">
        <span class="summary-label"><?php echo _l('is_payable'); ?></span>
        <h3 class="summary-value" style="color:#3f51b5;"><?php echo $contracts_payable_count; ?></h3>
      </div>
    </a>

    <a href="<?php echo admin_url('contracts?type=receivable'); ?>" style="text-decoration:none; color:inherit;">
      <div class="summary-card">
        <span class="summary-label"><?php echo _l('is_receivable'); ?></span>
        <h3 class="summary-value" style="color:#4caf50;"><?php echo $contracts_receivable_count; ?></h3>
      </div>
    </a>

    <a href="<?php echo admin_url('contracts?type=trash'); ?>" style="text-decoration:none; color:inherit;">
      <div class="summary-card">
        <span class="summary-label"><?php echo _l('contract_trash'); ?></span>
        <h3 class="summary-value" style="color:#ff9800;"><?php echo $contracts_others_count; ?></h3>
      </div>
    </a>
  </div>
  ...
  <!-- SECOND ROW -->
  <div class="summary-row">
  <!-- Example for total_value (if needed later)
  <div class="summary-card">
    <span class="summary-label"><?php echo _l('total_value'); ?></span>
    <?php $total_value = get_contract_value(); ?>
    <h3 class="summary-value" style="color:#ff9800;"><?php echo number_format($total_value, 2); ?></h3>
  </div>
  -->

  <div class="summary-card">
    <span class="summary-label"><?php echo _l('total_is_payable'); ?></span>
    <?php $total_is_payable = get_contract_value('is_payable'); ?>
    <h3 class="summary-value" style="color:#ff9800;"><?php echo number_format($total_is_payable, 2); ?></h3>
  </div>

  <div class="summary-card">
    <span class="summary-label"><?php echo _l('total_is_receivable'); ?></span>
    <?php $total_is_receivable = get_contract_value('is_receivable'); ?>
    <h3 class="summary-value" style="color:#ff9800;"><?php echo number_format($total_is_receivable, 2); ?></h3>
  </div>
  
    <div class="summary-card">
    <span class="summary-label"><?php echo _l('total_others'); ?></span>
    <?php $total_trash = get_contract_value('trash'); ?>
    <h3 class="summary-value" style="color:#ff9800;"><?php echo number_format($total_trash, 2); ?></h3>
  </div>
</div>
</div>
...
<?php if($dashtype=='approval'){?>
  ...
  <!--<h4 style="margin-bottom:20px; font-weight:600; color:#333;"><?php echo _l('approval_summary_heading'); ?></h4>-->
  ...
  <div class="row" id="contract_summary" style="display:flex; justify-content:center; gap:20px; flex-wrap:wrap;">
    ...
    <a href="<?php echo admin_url('contracts?type=unsigned'); ?>" style="text-decoration:none; color:inherit;">
      <div style="flex:1; min-width:150px; max-width:200px; text-align:center; background:#fff; padding:25px 20px; border-radius:10px; box-shadow:0 2px 6px rgba(0,0,0,0.1); transition:transform 0.2s;">
        <span style="display:block; color:#666; font-weight:500;"><?php echo _l('inbox'); ?></span>
        <?php $approval_count=get_contract_approval_count(); ?>
        <h3 style="color:#3f51b5; font-weight:700; margin:10px 0;"><?php echo $approval_count; ?></h3>
      </div>
    </a>

    <a href="<?php echo admin_url('contracts'); ?>" style="text-decoration:none; color:inherit;">
      <div style="flex:1; min-width:150px; max-width:200px; text-align:center; background:#fff; padding:25px 20px; border-radius:10px; box-shadow:0 2px 6px rgba(0,0,0,0.1); transition:transform 0.2s;">
        <span style="display:block; color:#666; font-weight:500;"><?php echo _l('waiting_others_approval'); ?></span>
        <?php $waiting_others_approval=get_contract_approval_count_waiting_others(); ?>
        <h3 style="color:#4caf50; font-weight:700; margin:10px 0;"><?php echo $waiting_others_approval; ?></h3>
      </div>
    </a>

    <a href="<?php echo admin_url('contracts'); ?>" style="text-decoration:none; color:inherit;">
      <div style="flex:1; min-width:150px; max-width:200px; text-align:center; background:#fff; padding:25px 20px; border-radius:10px; box-shadow:0 2px 6px rgba(0,0,0,0.1); transition:transform 0.2s;">
        <span style="display:block; color:#666; font-weight:500;"><?php echo _l('completed'); ?></span>
         <?php $contract_approval_count_completed=get_contract_approval_count_completed(); ?>
        <h3 style="color:#ff9800; font-weight:700; margin:10px 0;"><?php echo $contract_approval_count_completed; ?></h3>
      </div>
    </a>
    <a href="<?php echo admin_url('contracts'); ?>" style="text-decoration:none; color:inherit;">
      <div style="flex:1; min-width:150px; max-width:200px; text-align:center; background:#fff; padding:25px 20px; border-radius:10px; box-shadow:0 2px 6px rgba(0,0,0,0.1); transition:transform 0.2s;">
        <span style="display:block; color:#666; font-weight:500;"><?php echo _l('approching_deadline'); ?></span>
        <?php $approching_deadline=get_contract_approval_count_approching_deadline(); ?>
        <h3 style="color:#4caf50; font-weight:700; margin:10px 0;"><?php echo $approching_deadline; ?></h3>
      </div>
    </a>
  ...
  <div class="panel panel-primary" style="margin-top:40px;"> 
      <div class="panel-heading" style="background-color: #607d8b !important;"><?php echo _l('staff_summary'); ?></div>
      <div class="panel-body">
        <?php $staffs=get_contract_distinct_staff(); 
        foreach($staffs as $staff){ ?>
      <div class="col-md-3" style="padding:10px;">
  <div class="dbox dbox--agent" 
       style="background:#fff; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,0.08); 
              padding:25px 15px; text-align:center; min-height:240px; transition:all 0.2s ease-in-out;">
    
    <div class="dbox__body">
      <a data-toggle="tooltip" href="<?php echo admin_url('profile/' . $staff['staffid']); ?>">
        <?php echo staff_profile_image($staff['staffid'], [
          'staff-profile-image-small'
        ]); ?>
      </a>
      
      <h4 style="margin-top:10px; font-weight:600; color:#333;">
        <?php echo get_staff_full_name($staff['staffid']); ?>
      </h4>
      
      <div style="margin-top:15px; display:flex; flex-direction:column; gap:8px;">
        <span style="background:#e3f2fd; color:#1565c0; border-radius:6px; padding:6px 10px; font-size:13px;">
          <?php echo _l('total_contract'); ?> : 
          <strong><?php echo get_contract_approval_count_with_id($staff['staffid']); ?></strong>
        </span>

        <span style="background:#fff3e0; color:#ef6c00; border-radius:6px; padding:6px 10px; font-size:13px;">
          <?php echo _l('pending_approval'); ?> : 
          <strong><?php echo get_contract_approval_count_with_id_pending($staff['staffid']); ?></strong>
        </span>

        <span style="background:#e8f5e9; color:#2e7d32; border-radius:6px; padding:6px 10px; font-size:13px;">
          <?php echo _l('completed'); ?> : 
          <strong><?php echo get_contract_approval_count_completed_with_id($staff['staffid']); ?></strong>
        </span>
      </div>
    </div>
