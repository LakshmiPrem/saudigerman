...
<?php if(get_option('enable_casediary_in_dashboard')==1){?>
...
<div class="widget<?php if(count($contracts_activity) == 0){echo ' hide';} ?>" id="widget-<?php echo create_widget_id(); ?>" data-name="<?php echo _l('home_contract_activity'); ?>">
	
  <div class="panel_s projects-activity">
   <div class="panel-body padding-10">
    <div class="widget-dragger"></div>
    <p class="padding-5"><?php echo _l('home_contract_activity'); ?></p>
    <hr class="hr-panel-heading-dashboard">
    <div class="activity-feed">
     <?php
     foreach($contracts_activity as $activity){
   ...
   ?>
   <div class="feed-item">
     <div class="date"><span class="text-has-action" data-toggle="tooltip" data-title="<?php echo _dt($activity['date']); ?>">
     ...
     <div class="text">
      <p class="bold no-mbot">
       ...
       <?php } else { echo $name;} ;?>
       <?php echo $activity['description']; ?></p>
       <?php echo _l('contract_name'); ?>: <a href="<?php echo admin_url('contracts/contract/'.$activity['contractid']); ?>"><?php echo $activity['contract_name']; ?></a>
     </div>
     <?php if(!empty($activity['additional_data'])){ ?>
        <p class="text-muted mtop5"><?php echo $activity['additional_data']; ?></p>
     <?php } ?>
