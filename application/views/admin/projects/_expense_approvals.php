         ...
         <table class="table text-center">
            <tr>
               ...
               <th colspan="2" align="center">
                  ...
                  <div class="form-group">
                     ...
                     <input type="text" name="approval_name" id="approval_name" class="form-control" value="<?=$prefix.$_file_number?>" readonly >
                  </div></th>
               <th></th>
            </tr>
            <?php 
            ...
            <tr>
             <?php foreach($approval_types as $approval_type){ ?>  
               <th width="20%"><h5 class="mtop15 mbot15" style="height: 20px;"><?=$approval_type['name']?></h5>
...
<?php
   ...
   ?>
   <li class="nav-item <?php if(strtolower($refno)==$tab_name) echo 'active'; ?>" role="presentation">
    <a class="nav-link <?php if(strtolower($refno)==$tab_name) echo 'active'; ?>" id="<?php echo $tab_name;?>-tab" data-toggle="tab" href="#<?php echo $tab_name;?>" role="tab" aria-controls="home" aria-selected="true"><?php echo $approval['approval_name']; ?><br></a>
  </li>
...
<div class="tab-content" id="expenseApprovalTabsContent">
   ...
   <?php 
	...
	if(is_array($approvals)){
      ...
      <div class="tab-pane fade <?php if(strtolower($refno)==$tab_name) echo 'in active';?>" id="<?php echo $tab_name;?>" role="tabpanel" aria-labelledby="<?php echo $tab_name;?>-tab">
           <?php $record_sts1 = get_approvalreference_record_status($project->id,$approval2['id']);
   if($record_sts1){ 
      ...
      <a href="#" data-toggle="modal" data-target="#new_project_expense" data-id="<?=$approval2['approval_name']?>" id="new_project_expense1" onClick="passapproval(<?=$approval2['id']?>,'<?=$approval2['approval_name']?>')" class="btn btn-info mbot25"><?php echo _l('new_expense'); ?></a>
    
   <?php } 
   }else{ ?>
      <a href="#" data-container="body" data-toggle="popover" data-placement="top" data-content="Please Check approval status of <?=$approval2['approval_name']?>  & Reload the page."  class="btn btn-info mbot25"> <?php echo _l('new_expense'); ?></a>
   <?php } ?>
...
<script type="text/javascript">
    function passapproval(appid,appname)
    {
		
            var my_id_value = appname;
			
		 $('.modal-body #additional').append(hidden_input('approvalid',appid));
		 $('#new_project_expense input[name="appname"]').val(appname);
