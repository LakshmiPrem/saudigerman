                  ...
                  <div class="row">
                     ...
                     <div class="col-md-5 text-right">
                        ...
                        <?php } ?>
						 ...
						 if($project->project_stage!=5  && $project->project_stage==$case_overview->instance_id && $case_overview->stage_status==1){?> 
          <a href="#" class="btn btn-info"  onclick="init_court_instance();" >
           <?=ucwords(_l('goto_nextstage'))?>
        </a>
		<?php }}?>
						 <?php } ?>
                        ...
                        <a href="#" onclick="<?php echo $invoice_func; ?>(<?php echo $project->id; ?>); return false;" class="invoice-project hide btn btn-info<?php if($project->client_data->active == 0){echo ' disabled';} ?>"><?php echo _l('invoice_project'); ?></a>
                        <?php } ?>
						   <?php if($project->case_type!='submatter'){?>
                         <a href="#" class="btn btn-default btn-with-tooltip" data-target="#matter_send_to_customer" data-toggle="modal"><span data-toggle="tooltip" class="btn-with-tooltip" data-title="<?php echo _l('matter_send_to_email'); ?>" data-placement="bottom"><i class="fa fa-envelope"></i></span></a>
                         <?php } ?>
                        <?php
            ...
            <?php
               if(!has_contact_permission('projects',get_primary_contact_user_id($project->clientid))
                 ...
                 && $tab['slug'] != 'project_milestones') {
               ?>
            <div class="alert alert-warning project-permissions-warning mbot15 hide">
               <?php echo _l('project_customer_permission_warning'); ?>
            </div>
            <?php } ?>
            <div class="panel_s">
               <div class="panel-body" <?php if($tab['view']=='project_overview' || $tab['view']=='admin/projects/project_overview'){?>style="background-color: #dce1ef;"<?php } ?>>
...
<?php $this->load->view('admin/projects/hearing_judgement_js'); ?>
<?php init_tail(); ?>
<!-- For invoices table -->
<script>
...
<?php $this->load->view('admin/projects/expense_approval_js'); ?>
<script type="text/javascript">
  /*_validate_form($('#case-form'),{details_type:'required',case_number:'required'},$('#case-form'));*/
 ...
 function edit_scope(id) {
    ...
    if (description != '') {
	 ...
	 function edit_handover(id) {
		
    var description = $("body").find('[data-note-edit-textarea="' + id + '"] textarea').val();
		 
    if (description != '') {
        $.post(admin_url + 'casediary/edit_handover/' + id, {
            description: description
        }).done(function(response) {
            response = JSON.parse(response);
            if (response.success == true) {
...
var formatter = new Intl.NumberFormat('en-US', {
  ...
  function view_communication_details(communication_id) {
    url =  admin_url + 'projects/get_communication/'+communication_id;
    ...
    requestGet(url).done(function(response) {
        $('#_cc').html(response);
        $("body").find('#_communication_modal').modal({ show: true, backdrop: 'static' });
    });
}
</script>
...
<?php $this->load->view('admin/projects/projects_js'); ?>
<?php $this->load->view('admin/projects/hearing_js'); ?>
<?php $this->load->view('admin/projects/case_details_js'); ?>
...
<?php $this->load->view('admin/expenses/expense_payment_modal'); ?>
...
<?php $this->load->view('admin/projects/send_hearing_notice_email_template'); ?>
 <?php $this->load->view('admin/projects/modals/change_subattachment'); ?>
 <?php $this->load->view('admin/projects/modals/project_budget'); ?>
<?php $this->load->view('admin/projects/modals/budget_status_request_tostaff'); ?>
<?php $this->load->view('admin/projects/send_matter_to_email_template'); ?>
<script type="text/javascript">
	init_approval_table( 'expense', '<?php echo $project->id; ?>');
	/* Custome profile projects table */
	 <?php
						 if($work_type=='litigation'){?>
    initDataTable('.table-projects-submatter', admin_url + 'projects/tablesubmatter/' + project_id, undefined, undefined, 'undefined', <?php echo hooks()->apply_filters('projects_table_default_order', json_encode(array(5,'asc'))); ?>);
	hearingTableall(<?=$case_overview->instance_id?>,<?php echo $project->id?> );
	<?php } ?>
	 // Change sale status without replying
    $('select[name="sale_status_top"]').on('change', function() {
        var status = $(this).val();
     if(status=='ruling'){
		   $('#divtask').removeClass('hide');
		 $('#divjudge').addClass('hide');
	 }else if(status=='judgement'){
		   $('#divtask').addClass('hide');
		 $('#divjudge').removeClass('hide');
	 }else{
		  $('#divtask').addClass('hide');
		 $('#divjudge').addClass('hide');
	 }
...
/*budget fund total */
  $('#project-budget-form input[name="budget_preregister"],input[name="budget_consent"],input[name="budget_valuation"] ,input[name="budget_registration"],input[name="budget_disbursement"]').blur(function(){
	
       //  var total_amount = $('#project-expense-form input[name="amount"]').val();
         var exp_consent = $('#project-budget-form input[name="budget_consent"]').val();
	     var exp_preregister = $('#project-budget-form input[name="budget_preregister"]').val();
	     var exp_registration = $('#project-budget-form input[name="budget_registration"]').val();
	    var exp_valuation = $('#project-budget-form input[name="budget_valuation"]').val();
	   var exp_disbursement = $('#project-budget-form input[name="budget_disbursement"]').val();
	   if(isNaN(exp_consent)||exp_consent=='')exp_consent=0;
	    if(isNaN(exp_preregister)||exp_preregister=='')exp_preregister=0;
        if(isNaN(exp_valuation)||exp_valuation=='')exp_valuation=0;
	   if(isNaN(exp_registration)||exp_registration=='')exp_registration=0;
	  if(isNaN(exp_disbursement)||exp_disbursement=='')exp_disbursement=0;
	   var tt=parseFloat(exp_consent)+parseFloat(exp_valuation)+parseFloat(exp_preregister)+parseFloat(exp_registration)+parseFloat(exp_disbursement);
        
         $('#project-budget-form input[name="budget_amount"]').val(parseFloat(tt).toFixed(2));
         
    });
	  function passbudget(budgetid,isbudget)
    {
					
		// $('.modal-body #additional').append(hidden_input('approvalid',appid));
		 $('#project-expense-form input[name="budget_id"]').val(budgetid);
		 $('#project-expense-form input[name="isbudget"]').val(isbudget);
		// $('#project-expense-form select[name="expense_taskid"]').val(50);
		//$('#project-expense-form select[name="expense_taskid"]').selectpicker('val',50);
