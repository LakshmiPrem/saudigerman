  ...
  <tbody>
    <?php 
      foreach ($communication_center as $cc) { 
        ...
        <td><?php $y=1; foreach($cc['attachments'] as $files){ $path = PROJECT_ATTACHMENTS_FOLDER.$cc['case_id'].'/'.$files['file_name'];
                  echo $y.'.<a href="'.site_url('uploads/projects/'.$cc['case_id'].'/'.$files['file_name']).'" target="_blank" >'.$files['file_name'].'</a><br>'; $y++;}
          ?></td>
        ...
        <td><a class="btn btn-danger _delete btn-icon" href="<?php echo admin_url('projects/delete_communications/'.$cc['case_id'].'/'.$cc['id']) ?>"><i class="fa fa-remove"></i></a>
           <a href="<?php echo admin_url('projects/communication_center_print/'.$cc['case_id'].'/'.$cc['id'].'?print=true'); ?>" target="_blank" class="btn btn-primary btn-with-tooltip btn-icon " data-toggle="tooltip" title="<?php echo _l('print'); ?>" data-placement="bottom"><i class="fa fa-print"></i></a> </td>
      </tr>
      <?php } ?>  
  </tbody>
 </table>
