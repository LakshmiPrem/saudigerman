...
<div class="modal fade _project_subfile" tabindex="-1" role="dialog" data-toggle="modal">
   <div class="modal-dialog full-screen-modal" role="document">
      <div class="modal-content">
         <div class="modal-header">
            <button type="button" class="close" onclick="close_modal_manually('._project_subfile'); return false;"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title"><?php echo $file->subject.'-'.$file->matter_subrefno; ?></h4>
         </div>
         <div class="modal-body">
            <div class="row">
               <div class="col-md-12 border-right project_subfile_area">
                  <?php
                  ...
                  <div class="row">
                  <div class="col-md-6">
                     <?php echo render_input('_file_subject1', 'project_discussion_subject', $file->subject, 'text', ['onblur' => 'update_subfile_data(' . $file->id . ')']); ?>
                  </div>
                   <?php $value = (isset($file->issue_date) ? $file->issue_date : _d(date('Y-m-d'))); ?>
					  <?php $std1=(isset($file)? array('readonly'=>'readonly'):array());?>
                  <div class="col-md-6">
                     <?php echo render_input('_issue_date1','issue_date',$value,'date',array('onblur'=>'update_subfile_data('.$file->id.')')); ?> 
                  </div>
                  <?php $value = (isset($file->expiry_date) ? $file->expiry_date : _d(date('Y-m-d'))); ?>
                   <div class="col-md-6">
                     <?php echo render_input('_expiry_date1','expiry_date',$value,'date',array('onblur'=>'update_subfile_data('.$file->id.')')); ?> 
                  </div>
                 
                   <div class="col-md-6">
                  <div class="form-group" <?php if(isset($file)){?> style="pointer-events: none;"<?php }?>>
                     <label class=""><?php echo _l('document_type'); ?></label>
                  <select class="form-control" id="_document_type" name="document_type" readonly>
                     <option></option>
                     <?php foreach($document_types as $doc_type){ ?>
                        <option value="<?=$doc_type['id']?>" <?php if($file->document_type == $doc_type['id']) echo 'selected'; ?>><?=$doc_type['name']?></option>
                     <?php } ?>
                  </select>
                  </div>
				   </div>
                  </div>
                  <?php // echo render_select('document_type',$document_types,array('id','name'),'document_type',$file->document_type); ?>
                  <?php echo render_textarea('_file_description1', 'project_discussion_description', $file->description, ['onblur' => 'update_subfile_data(' . $file->id . ')']); ?>
                  <hr />
                  <?php
                     } else {
                         ?>
         ...
         <div class="modal-footer">
            <button type="button" class="btn btn-default" onclick="close_modal_manually('._project_subfile'); return false;"><?php echo _l('close'); ?></button>
         </div>
      </div>
      <!-- /.modal-content -->
   </div>
...
<script>
 ...
 $('body').find('._project_subfile').modal({show:true, backdrop:'static', keyboard:false});
</script>
