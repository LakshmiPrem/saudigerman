<?php echo form_open_multipart(admin_url('casetemplates/task_temp/'.$id),array('id'=>'task-form')); ?>
<div class="modal fade<?php if(isset($task)){echo ' edit';} ?>" id="_task_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"<?php if($this->input->get('opened_from_lead_id')){echo 'data-lead-id='.$this->input->get('opened_from_lead_id'); } ?>>
  <div class="modal-dialog" role="document">
          ...
          <div class="col-md-12">
            <?php
             ...
             <div class="pull-right mbot10 task-single-menu task-menu-options">
              ...
              $copy_template .= "</div>";
              ?>
              <li class="hide"> <a href="#" onclick="return false;" data-placement="bottom" data-toggle="popover" data-content="<?php echo htmlspecialchars($copy_template); ?>" data-html="true"><?php echo _l('task_copy'); ?></span></a>
              </li>
              <?php } ?>
    ...
    <hr />
    <?php   echo form_hidden('rel_id',$rel_id);
            echo form_hidden('rel_type',$rel_type);?>
    <?php $value = (isset($task) ? $task->name : '');   ?>
    <?php echo render_input('name','task_add_edit_subject',$value); ?>
     <?php $value = (isset($task) ? $task->due_days : '');   ?>
    <?php echo render_input('due_days','due_days',$value,'number',['min'=>0]); ?>
    <?php $hide = '';
        if(isset($task) && $task->rel_type == 'project' && total_rows('tblprojects',array('id'=>$task->rel_id,'billing_type'=>3)) == 0){ $hide = 'hide';}
        if(isset($task) && $task->rel_type == 'casetemplates'){ $hide = 'hide';} ?>
     <!--  <div class="row">
      <div class="col-md-6">
        <?php $create_auto_reminder = (isset($task) ? $task->auto_reminder_hours : '');   ?> 
        <?php render_yes_no_option('create_auto_reminder','create_auto_reminder_on_task_due_date'); ?></div>
      <div class="col-md-6">
        <?php $value = (isset($task) ? $task->auto_reminder_hours : '');   ?>
        <?php echo render_input('auto_reminder_hours','auto_reminder_hours',$value,'number',['min'=>0]); ?>
      </div>
    </div> -->
     
    <?php //$value = (isset($task) ? $task->auto_reminder_description : '');   ?>
    <?php //echo render_textarea('auto_reminder_description','auto_reminder_description',$value,['rows'=>1]); ?>
    <div class="row">
      <div class="col-md-12">
       <div class="form-group">
        <label for="repeat_every" class="control-label"><?php echo _l('task_repeat_every'); ?></label>
        <select name="repeat_every" id="repeat_every" class="form-control" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
           <option value=""></option>
        ...
        <div class="recurring_custom <?php if((isset($task) && $task->custom_recurring != 1) || (!isset($task))){echo 'hide';} ?>">
                  <div class="row">
                     ...
                     <div class="col-md-6">
                        <select name="repeat_type_custom" id="repeat_type_custom" class="form-control" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
    ...
    <div class="row">
      ...
      <div class="col-md-6 hide">
        <?php $value = (isset($task) ? _d($task->duedate) : _d(date('Y-m-d'))); ?>
...
<div class="form-group hide">
  <div id="inputTagsWrapper">
    ...
    <input type="text" class="tagsinput" id="tags" name="tags" value="<?php echo (isset($task) ? prep_tags_input(get_tags_in($task->id,'task_template')) : ''); ?>" data-role="tagsinput">
  </div>
</div>
