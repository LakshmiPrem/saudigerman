   ...
   <div class="content">
      <div class="row">
         <div class="col-md-5 left-column">
           ...
           <!-- <i class="fa fa-question-circle pull-left" data-toggle="tooltip" title="<?php echo _l('contract_subject_tooltip'); ?>"></i>-->
            ...
            <?php ########## Opposite Party ##############  ?>
               ...
               <?php $selected = (isset($contract) ? $contract->customer_name : '');
           
				   echo render_select('customer_name',$oppositeparty_names,array('id','name'),'customer_name',$selected);
				?>
            <?php $value = (isset($contract) ? $contract->customer_code : '');
           
				   echo render_input('customer_code','customer_code',$value);
				?>

          
         <!--   <div class="form-group">
               <label for="contract_value"><?php echo _l('total_outstanding'); ?></label>
       ...
       <!--  <div class="row">
            ...
            <div class="col-md-6">
               <?php //$value = (isset($contract) ? _d($contract->datestart) : ''); ?>
            ...
            <div class="row"><div class="col-md-12">
                        ...
                        <div class="form-group no-margin select-placeholder">
                            <label for="project_id"><?php echo _l('status'); ?></label>
                            <?php $value = (isset($contract) ? $contract->status : '1');?>
                           <select name="status" id="status" class="selectpicker no-margin" data-width="100%" data-title="<?php echo _l('task_status'); ?>">
                             <!-- <option value="" selected><?php echo _l('task_list_all'); ?></option>-->
                              ...
                              <option value="<?php echo $status['chequestatusid']; ?>" <?php if($status['chequestatusid']==$value){echo 'selected'; } ?>><?php echo $status['name']; ?></option>
                              <?php } ?>
                           </select>
                        </div>
				</div></div>
...
<?php if(isset($contract)) { ?>
   <div class="col-md-7 right-column">
               ...
               <div class="horizontal-tabs">
                  <ul class="nav nav-tabs tabs-in-body-no-margin contract-tab nav-tabs-horizontal mbot15" role="tablist">
                    <!-- <li role="presentation" class="<?php if(!$this->input->get('tab') || $this->input->get('tab') == 'tab_content'){echo 'active';} ?>">
                      ...
                      <li role="presentation" class="<?php if(!$this->input->get('tab') || $this->input->get('tab') == 'renewals'){echo 'active';} ?>">
                     <a href="#renewals" aria-controls="renewals" role="tab" data-toggle="tab">
                        <?php echo _l('chequereturn_history_heading'); ?>
                    ...
                    <li role="presentation">
                     <a href="#tab_comments" aria-controls="tab_comments" role="tab" data-toggle="tab" onclick="get_contract_comments(); return false;">
                        ...
                        <?php
                        $totalComments = total_rows(db_prefix().'chequebounce_comments','bounce_id='.$contract->id)
                        ?>
                  ...
                  <li role="presentation" class="tab-separator">
                     <a href="#tab_tasks" aria-controls="tab_tasks" role="tab" data-toggle="tab" onclick="init_rel_tasks_table(<?php echo $contract->id; ?>,'chequebounce'); return false;">
                        <?php echo _l('tasks'); ?>
                     </a>
                  </li>
                  <li role="presentation" class="tab-separator">
                     <a href="#tab_notes" onclick="get_sales_notes(<?php echo $contract->id; ?>,'chequebounces'); return false" aria-controls="tab_notes" role="tab" data-toggle="tab">
                        <?php echo _l('updates'); ?>
                        <span class="notes-total">
                         ...
                         <li role="presentation"  class="<?php if($this->input->get('tab') == 'reminder'){echo 'active';} ?>">
                              <a href="#tab_reminders" onclick="initDataTable('.table-reminders', admin_url + 'misc/get_reminders/' + <?php echo $contract->id ;?> + '/' + 'chequebounce', undefined, undefined, undefined,[1,'asc']); return false;" aria-controls="tab_reminders" role="tab" data-toggle="tab">
      ...
      <div role="tabpanel" class="tab-pane" id="tab_notes">
         <?php echo form_open(admin_url('chequebounces/add_note/'.$contract->id),array('id'=>'sales-notes','class'=>'chequebounce-notes-form')); ?>
      ...
      <div role="tabpanel" class="tab-pane<?php if($this->input->get('tab') == 'attachments'){echo ' active';} ?>" id="attachments">
         <?php echo form_open(admin_url('chequebounces/add_contract_attachment/'.$contract->id),array('id'=>'contract-attachments-form','class'=>'dropzone')); ?>
         <?php echo form_close(); ?>
         ...
         <div id="contract_attachments" class="mtop30">
            ...
            foreach($contract->attachments as $attachment) {
             $href_url = site_url('download/file/chequebounce/'.$attachment['attachment_key']);
...
<div role="tabpanel" class="tab-pane<?php if(!$this->input->get('tab') || $this->input->get('tab') == 'renewals'){echo ' active';} ?>" id="renewals">
   <?php if(has_permission('chequebounces', '', 'create') || has_permission('chequebounces', '', 'edit')){ ?>
      <div class="_buttons">
        <a href="#" onclick="defendar(<?php echo $contract->id; ?>); return false;" class="btn btn-success mtop15 mbot10" ><?php echo _l('chequebounces_return'); ?></a>
      </div>
      <hr />
   <?php } ?>
   <div class="clearfix"></div>
   ...
   if(count($contract_renewal_history) == 0){
     echo _l('no_chequebounce_return_found');
  }
							else{
  ?>
...
<div class="clearfix"></div>
<div class="mtop15">
    <table class="table dt-table table-returncheque1" data-order-col="0" data-order-type="desc">
        <thead>
            <tr>
                <th>
                    <?php echo _l( 'retcheque_date'); ?>
                </th>
                <th>
                    <?php echo _l( 'cheque_no'); ?>
                </th>
                <th>
                    <?php echo _l( 'cheque_amount'); ?>
                </th>
                 <th>
                    <?php echo _l( 'amount_received'); ?>
                </th>
                 <th>
                    <?php echo _l( 'balance'); ?>
                </th>
        ...
        <tbody>
         <?php
           foreach($contract_renewal_history as $row){ ?>
            <tr>
            <td><?=_d($row['retcheque_date'])?></td>
				<td><?=$row['cheque_no']?></td>
        ...
        <td>
                       <?php if(has_permission('chequebounces', '', 'edit')){ ?>
            <a href="#" class="btn btn-default btn-icon" onclick="defendar(<?php echo $row['bounce_id'].','.$row['id']?>);return false;"><i class="fa fa-pencil-square-o"></i></a>
              <?php }
              if(has_permission('chequebounces', '', 'delete')){ ?>
            <a href="<?php echo admin_url('chequebounces/delete_renewal/'.$row['id'].'/'. $row['bounce_id']); ?>" class="btn btn-danger btn-icon _delete"><i class="fa fa-remove"></i></a>
            <?php } ?>
        </td>
    </tr>
    <?php } ?>
...
<?php } ?>
   ...
   <div role="tabpanel" class="tab-pane" id="tab_reminders">
                  <a href="#" class="btn btn-info btn-xs" data-toggle="modal" data-target=".reminder-modal-chequebounce-<?php echo $contract->id; ?>"><i class="fa fa-bell-o"></i> <?php echo _l('set_reminder'); ?></a>
                  <hr />
...
<div role="tabpanel" class="tab-pane" id="tab_tasks">
   <?php init_relation_tasks_table(array('data-new-rel-id'=>$contract->id,'data-new-rel-type'=>'chequebounce')); ?>
</div>
...
<?php if(isset($contract)){ ?>
   ...
   <?php $this->load->view('admin/chequebounces/send_to_client'); ?>
...
<script>
    ...
    if (typeof (Dropbox) != 'undefined' && $('#dropbox-chooser').length > 0) {
     document.getElementById("dropbox-chooser").appendChild(Dropbox.createChooseButton({
        success: function (files) {
           $.post(admin_url + 'chequebounces/add_external_attachment', {
              files: files,
  ...
  $.each(contractsTemplates, function (i, template) {
     _templates.push({
        url: admin_url + 'chequebounces/get_template?name=' + template,
        title: template
     });
  });
...
function save_contract_content(manual) {
  ...
  $.post(admin_url + 'chequebounces/save_contract_data', data).done(function (response) {
 ...
 function delete_contract_attachment(wrapper, id) {
  if (confirm_delete()) {
     $.get(admin_url + 'chequebounces/delete_contract_attachment/' + id, function (response) {
...
function add_contract_comment() {
  ...
  data.bounce_id = contract_id;
  $('body').append('<div class="dt-loader"></div>');
  $.post(admin_url + 'chequebounces/add_comment', data).done(function (response) {
     response = JSON.parse(response);
...
function get_contract_comments() {
  ...
  requestGet('chequebounces/get_comments/' + contract_id).done(function (response) {
...
function remove_contract_comment(commentid) {
  if (confirm_delete()) {
     requestGetJSON('chequebounces/remove_comment/' + commentid).done(function (response) {
        if (response.success == true) {
...
function edit_contract_comment(id) {
  ...
  if (content != '') {
     $.post(admin_url + 'chequebounces/edit_comment/' + id, {
        content: content
...
function contractGoogleDriveSave(pickData) {
   ...
   data.files = pickData;
   $.post(admin_url + 'chequebounces/add_external_attachment', data).done(function () {
    var location = window.location.href;
    window.location.href = location.split('?')[0] + '?tab=attachments';
 });
}
    ...
    $.post(admin_url + 'chequebounces/renew/' + client_id +'/'+contact_id).done(function(response) {
	//	alert(response);
        ...
        $('body').on('shown.bs.modal', '#contact', function() {
            if (contact_id == '') {
                $('#contact').find('input[name="cheque_no"]').focus();
            }
        });
        init_selectpicker();
        init_datepicker();
