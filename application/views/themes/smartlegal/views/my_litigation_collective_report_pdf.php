...
$tblhtml .= '<p align="center"><h3><b style="background-color:#f0f0f0;">'.ucwords(_l('with_description')).'</b></h3></p>';
...
$tblhtml .= '<th  style="text-align:left;" width="22%"><b>'._l('case_updates').'</b></th>';
...
$tblhtml .= '<th  style="text-align:left;" width="11%"><b>'._l('oppositeparty_position').'</b></th>';
...
foreach ($litigations as $aRow) {
	...
	if(isset($aRow['company'])){
		...
		$tblhtml .= '<td  style="text-align:left;" width="22%">'.nl2br($aRow['case_updates']).'</td>';
		...
		$tblhtml .= '<td  style="text-align:left;" width="11%">'.get_opposite_party_name($aRow['opposite_party']).' - '.get_position_name_by_id($aRow['opposite_party_position']).'</td>';
		$tblhtml .= '<td  style="text-align:left;" width="13%">'.$aRow['company'].' - '.get_position_name_by_id($aRow['client_position']).'</td>';
		$tblhtml .= '<td  style="text-align:left;"  width="4%">'.$j++.'</td>';
		$tblhtml .= '</tr>';
		$tblhtml .= '<tr><td colspan="9"  style="text-align:center;"><b>'._l('project_description').'</b></td></tr>';
		$tblhtml .= '<tr><td colspan="9"  style="text-align:left;" >'.strip_tags($aRow['description'],'<br><p>').'</td></tr>';
	}
