      ...
      <div class="container preview-sticky-container">
         <div class="row">
            <div class="col-md-12" >
               ...
               <button type="submit" id="accept_action" class="btn btn-success pull-right action-button"><?php echo _l('first_party').' '._l('e_signature_sign'); ?></button>
               <?php } else { ?>
               <span class="success-bg content-view-status contract-html-is-signed"><?php echo _l('first_party').' '._l('is_signed'); ?></span>
               <?php } ?>
             
                 <div id="google_translate_element"  class=" pull-right action-button mright5 mtop5"></div> 
                <?php if($contract->contract_filename!=''){
                         $path = site_url('download/downloadagreement/'. $contract->id); ?>
			  <a download href="<?php echo $path;?>"  class="btn btn-default pull-right action-button mright5 contract-html-pdf"><i class="fa fa-file-pdf-o"></i> <?php echo _l('clients_invoice_html_btn_download'); ?></a> 
              <?php } else{?>         
...
// get_template_part('identity_confirmation_form', array('formData' => form_hidden('action', 'sign_partycontract')));
   ?>
<script>
   $(function(){
