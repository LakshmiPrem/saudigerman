...
class Settings extends AdminController
{
    ...
    public function index()
    {
        ...
        if ($this->input->post()) {
            ...
            $signatureUploaded = (handle_company_signature_upload() ? true : false);
			 $footerImageUploaded = (handle_footer_image_upload() ? true : false);
            $headerImageUploaded = (handle_header_image_upload() ? true : false);
    ...
    public function delete_option($name)
    {
	  ...
	  public function remove_footer_image()
    {
        if (!has_permission('settings', '', 'delete') && !is_client_admin()) {
            access_denied('settings');
        }

        $sImage = get_option('pdf_footer_image');
        if (file_exists(get_upload_path_by_type('company') . '/' . $sImage)) {
            unlink(get_upload_path_by_type('company') . '/' . $sImage);
        }

        update_option('pdf_footer_image', '');
        redirect(admin_url('settings?group=pdf&tab=pdf_footer'));
    }
     ...
     public function remove_header_image()
    {
        ...
        $sImage = get_option('pdf_header_image');
        if (file_exists(get_upload_path_by_type('company') . '/' . $sImage)) {
            unlink(get_upload_path_by_type('company') . '/' . $sImage);
        }

        update_option('pdf_header_image', '');
        redirect(admin_url('settings?group=pdf&tab=pdf_header'));
    }
}
