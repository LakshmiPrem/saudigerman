...
class Tickets extends AdminController
{
    ...
    public function add($userid = false)
    {
        ...
        if(get_option('enable_single_assignee') == 1){
            $whereStaff['staffid'] = get_option('legal_request_single_assignee_id');
        }
        ...
        if ($this->input->get('project_id') && $this->input->get('project_id') > 0) {
            ...
            if (total_rows(db_prefix().'contacts', ['active' => 1, 'userid' => $data['userid']]) == 1) {
                $contact = $this->clients_model->get_contacts($data['userid'],['is_primary'=>1,'active'=>1]);
                if (isset($contact[0])) {
		 ...
		 elseif ($this->input->get('userid')) {
			 $user_id=$this->input->get('userid');
			  if (total_rows(db_prefix().'contacts', ['active' => 1, 'userid' => $user_id,'is_primary'=>1]) == 1) {
                $contact = $this->db->get_where('tblcontacts', ['active' => 1, 'userid' => $user_id,'is_primary'=>1])->row();
                if ($contact) {
                    $data['contact'] = (array) $contact;
                }
            }
    ...
    public function ticket($id)
    {
       ...
       if (get_option('staff_access_only_assigned_departments') == 1) {
            ...
            if (!is_admin() &&!$have_assigned_customers) {
                $this->load->model('departments_model');
                $staff_departments = $this->departments_model->get_staff_departments(get_staff_user_id(), true);
        ...
        if ($this->input->post()) {
            ...
            if ($success) {
                ...
                if (get_option('staff_access_only_assigned_departments') == 1) {
                    ...
                    $this->load->model('departments_model');
                    $staff_departments = $this->departments_model->get_staff_departments(get_staff_user_id(), true);
                    if (!in_array($ticket->department, $staff_departments) && !is_admin()&& !have_assigned_customers()) {
    ...
    public function ticket_change_data()
    {
        if ($this->input->is_ajax_request()) {
            ...
            echo json_encode([
				...
				//'company_name'			=>get_company_name(get_user_id_by_contact_id($contact_id)),
	 ...
	 public function legal_approval($id)
    {
      /*  if (!has_permission('projects', '', 'view') && !has_permission('projects', '', 'view_own')) {
        ...
        $legalapprove->approval =  get_approvals($id,'ticket',3);
