...
class Tasks extends AdminController
{
    ...
    public function save_checklist_assigned_staff()
    {
     ...
     function get_sla_dead_line_date($days,$startdate,$taskid=''){
        if($taskid == ''){
            echo _d(date('Y-m-d',strtotime("+ $days days",strtotime($startdate))));
        }else{
    
            $sla_deadline_date =  date('Y-m-d',strtotime("+ $days days",strtotime($startdate)));
            $this->db->where('id',$taskid);
            $this->db->update('tblstafftasks',array('sla_deadline_date'=>$sla_deadline_date));
            echo _d($sla_deadline_date);
