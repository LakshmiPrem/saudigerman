...
class Expenses extends AdminController
{
    ...
    public function list_expenses($id = '')
    {
     ...
     public function table($clientid = '',$party_id='')
    {
        if (!has_permission('expenses', '', 'view') && !has_permission('expenses', '', 'view_own')) {
            ajax_access_denied();
        }
    ...
    public function expense($id = '')
    {
        if ($this->input->post()) {
			...
			$pvpaid=$this->input->post('lastpaid_amount');
			unset($data['lastpaid_amount']);
			//$data['paid_amount']=$this->input->post('paid_amount')+$pvpaid;
        ...
        if ($this->input->get('customer_id')) {
         ...
         if ($this->input->get('opposite_id')) {
            $data['opposite_id']        = $this->input->get('opposite_id');
        }
      ...
      //  $this->load->model('tickets_model');
       // $data['appro_statuses']  = $this->tickets_model->get_ticket_status();
		...
		$data['other_parties']=$this->oppositeparty_model->get();
        $data['title']      = $title;
	...
	/* Expense add_expensepayment */
	public function add_expensepayment(){
		 if ($this->input->post()) {
			 $data=$this->input->post();
			 $expenseid=$this->input->post('expense_id');
             $message         = '';
             $success=false;
            $id = $this->expenses_model->add_expensepayment($data);
			if ($id >0) {
				handle_expense_payment_file_upload($id,$expenseid);
                    $success=true;
				$expamount=$this->db->select('amount')->from('tblexpenses')->where('id',$expenseid)->get()->row()->amount;
                   $totalpaid= sum_from_table(db_prefix().'expense_payment',array('where'=>array('expense_id'=>$expenseid),'field'=>'exp_paid_amount'));
				$balance_amount=$expamount-$totalpaid;
				$paid_status=($balance_amount==0)?'Paid':(($expamount>$balance_amount)?'Partially Paid':'Unpaid');
                ...
                $message = $id ? _l('added_successfully', _l('expense_payment')) : '';
			
                if ($success) {
					$message= _l('expense_payment_added');
    ...
    public function delete_payment($id,$e_id)
    {
        

        if (has_permission('expenses', '', 'delete') || is_admin()) {
            $success = $this->expenses_model->delete_expense_payment($id);
            if ($success) {
                $totalpaid= sum_from_table(db_prefix().'expense_payment',array('where'=>array('expense_id'=>$e_id),'field'=>'exp_paid_amount'));
                $this->db->where('id', $e_id);
                $this->db->update(db_prefix() . 'expenses', [
               'paid_amount' => $totalpaid,
               ]);
            
                set_alert('success', _l('deleted', _l('expense_payment')));
            } else {
                set_alert('warning', _l('problem_deleting', _l('expense_payment')));
            }
            
                redirect(admin_url('expenses/list_expenses/' . $e_id));
     ...
     public function table_lawyer($lawyer_id=''){
