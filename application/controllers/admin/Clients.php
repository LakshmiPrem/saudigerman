...
class Clients extends AdminController
{
    ...
    public function client($id = '')
    {
        ...
        if($id!='')
			...
			$first='profile';
        $group         = !$this->input->get('group') ? $first : $this->input->get('group');
        $data['group'] = $group;
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                $data['shareholders'] = $this->clients_model->get_clientshareholders($id); 
                $data['user_notes'] = $this->misc_model->get_notes($id, 'customer');
               $data['client_owners']=$this->clients_model->get_contacts($id,['is_owner'=>1,'active'=>1]);
               $data['client_managers']=$this->clients_model->get_contacts($id,['is_manager'=>1,'active'=>1]);
               $data['client_directors']=$this->clients_model->get_contacts($id,['is_director'=>1,'active'=>1]);
               $data['client_secretarys']=$this->clients_model->get_contacts($id,['is_secretary'=>1,'active'=>1]);
               //$data['subfilesdocs']=get_distinct_constifiledocument_byproject($id);
               $data['branches']=$this->db->get_where('tblclients',array('parent_company'=>$id))->result_array();
                ...
                $this->load->model('invoices_model');
                $data['invoice_statuses'] = $this->invoices_model->get_statuses();
            } elseif ($group == 'constitution') {
                //	$this->load->model('casediary_model');
                //	$data['document_types']     = $this->clients_model->get_constitution_types();
        ...
        $this->load->model('casediary_model');
        $data['comp_person'] =$this->casediary_model->get_oppositeparty();
        $data['clients_'] = $this->clients_model->get();
        $data['bodyclass'] = 'customer-profile dynamic-create-groups';
        $data['parent_company']=$this->clients_model->get('',['company_type'=>'main']);
        $data['title']     = $title;
    ...
    public function form_contact($customer_id, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            $stakers=$this->db->get_where('tblshareholders',array('id'=>$this->input->post('stakeholder_id')))->row();
            $data['email']=$stakers->email;
            $data['firstname']=$stakers->firstname;
            $data['lastname']=$stakers->lastname;
            $data['phonenumber']=$stakers->phonenumber;
        ...
        $data['stakeholders'] = $this->clients_model->get_shareholders('',['stake_type'=>'person']);
    ...
    public function delete_contact($customer_id, $id,$type='')
    {
    ...
    public function pagination()
    {
      ...
      $config = array();
      $config["base_url"] = "#";
      $config["total_rows"] =  $this->clients_model->fetch_client_details_num_rows($q);
      $config["per_page"] = 12;
      $config["uri_segment"] = 4;
      $config["use_page_numbers"] = TRUE;
      $config["full_tag_open"] = '<ul class="pagination client-page">';
      ...
      $output = array(
       ...
       'client_data'   => $this->clients_model->fetch_client_details($q,$config["per_page"], $start),
       'total_clients'=> '<span class="badge badge-success" style="padding: 10px;
    font-size: 15px;"><b>'.$config["total_rows"].' '._l('clients').'</b></span>',
      );
      echo json_encode($output);
    }
public function getDepartmentUsers() {
		 $postData=$this->input->post('branchid');
         $data = $this->clients_model->get_contacts($postData);
 
        echo json_encode($data);
    }
      //shareholder table
      public function shareholder_table($client_id)
      {
          $this->app->get_table_data('clientshareholders', [
              'client_id' => $client_id,
          ]);
      }
          ...
          /*admin add/edit/delete shareholder  */

    public function shareholders()
    {
        if (!is_admin()) {
            access_denied('Customer Shareholder');
        }
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('customers_shareholders');
        }
        $this->load->model('casediary_model');
        $data['comp_person'] =$this->casediary_model->get_oppositeparty();
        $data['clients_'] = $this->clients_model->get();
        $data['title'] = _l('customer_shareholder');
        $this->load->view('admin/clients/groups_shareholder', $data);
    }
    ...
    public function shareholder()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $data = $this->input->post();
            $data['shareholder_name']=$this->input->post('firstname').' '.$this->input->post('lastname');
            if($data['internal_party']=='internal'){
                $data['staketype_id']=$data['internal_party'];
            }else if($data['internal_party']=='external'){
                $data['staketype_id']=$data['external_party'];
            }else{
                $data['staketype_id']=0;
            }
            unset($data['internal_party']);
            unset($data['external_party']);
            if ($data['id'] == '') {
                $id      = $this->clients_model->new_shareholder($data);
                $message = $id ? _l('added_successfully', _l('customer_shareholder')) : '';
                echo json_encode([
                    'success' => $id ? true : false,
                    'message' => $message,
                    'id'      => $id,
                ...
                $success = $this->clients_model->edit_shareholder($data);
                $message = '';
                if ($success == true) {
                    $stakers=$this->db->get_where('tblcontacts',array('stakeholder_id'=>$data['id']))->num_rows();
                    if($stakers>0){
                    $this->db->where('stakeholder_id', $data['id'] );
             $this->db->update( db_prefix() . 'contacts', [
                    ...
                    $message = _l('updated_successfully', _l('customer_shareholder'));
                }
                echo json_encode([
                    'success' => $success,
                    'message' => $message,
    ...
    {
        if (!is_admin()) {
            access_denied('Delete Customer Shareholder');
        }
        if (!$id) {
            redirect(admin_url('clients/shareholders'));
        }
        $response = $this->clients_model->delete_shareholder($id);
        if ($response == true) {
            set_alert('success', _l('deleted', _l('shareholders')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('customer_shareholder')));
        }
        redirect(admin_url('clients/shareholders'));
    }

    public function form_shareholder($customer_id, $contact_id = '')
    {
    if (!has_permission('customers', '', 'view')) {
        if (!is_customer_admin($customer_id)) {
            echo _l('access_denied');
    ...
    if ($this->input->post()) {
        ...
        if ($contact_id == '') {
            ...
            $id      = $this->clients_model->add_shareholder($data, $customer_id);
            $message = '';
            $success = false;
            if ($id) {
               
                $success = true;
                $message = _l('added_successfully', _l('shareholder'));
            }
            echo json_encode([
                'success'             => $success,
                'message'             => $message,
        ...
        $original_contact = $this->clients_model->get_clientshareholder($contact_id);
        $success          = $this->clients_model->update_shareholder($data, $contact_id);
        $message          = '';
        if ($success == true) {
             $message = _l('updated_successfully', _l('shareholder'));
           echo json_encode([
                'success'             => $success,
             //   'proposal_warning'    => $proposal_warning,
                'message'             => $message,
    ...
    if ($contact_id == '') {
        $title = _l('add_new', _l('shareholder'));
    } else {
        $data['contact'] = $this->clients_model->get_clientshareholder($contact_id);
        ...
        $title = _l('edit', _l('shareholder'));
    }
        
    
    $data['shareholders'] = $this->clients_model->get_shareholders();
    $data['title']                = $title;
    $this->load->view('admin/clients/modals/shareholder', $data);

    
}

//constitution table adarsh
public function constitution_table($client_id)
{
    $this->app->get_table_data('client_constitution', [
        'client_id' => $client_id,
    ]);
}
...
public function form_constitution($customer_id, $contact_id = '')
{
...
if ($this->input->post()) {
    ...
    if ($contact_id == '') {
        ...
        $doccount=$this->db->get_where('tblclient_subfile',array('userid'=>$customer_id,'document_type'=>$ctype))->num_rows();
        if($doccount>0)
            $vcount=$doccount+1;
        $project_data   = $this->db->get_where('tblclients',array('userid'=>$customer_id))->row();
        //$matter_refno=$project_data->client_matterrefno;
        $psubcategory= $this->db->get_where('tblconstitution_type',array('id'=>$ctype))->row()->shortname;
        $filedt=date('ymd',strtotime(to_sql_date($this->input->post('issue_date'))));
        // $matter_subrefno=$matter_refno.'##'.$psubcategory.'-'.$filedt.'-'.$vcount;
        // $data['matter_subrefno']=$matter_subrefno;
        $id      = $this->clients_model->add_constitution($data, $customer_id);
        $message = '';
        $success = false;
        if ($id) {
           handle_client_sub_file_upload($id,$customer_id);
            // $userfile1= $this->db->get_where('tblclient_subfile',array('id'=>$id))->row()->file_name;
            // // $contentfile=$this->clientvesioncontents($userfile1,$customer_id);

            // //   $this->db->where( 'id', $id );
            // //  $this->db->update( db_prefix() . 'client_subfile', [
            // //      'content' => $contentfile,
            // //  //	'dateapproved' => date( 'Y-m-d H:i:s' ),
            // //  ] );
            $success = true;
            $message = _l('added_successfully', _l('subfile'));
        }
        echo json_encode([
            'success'             => $success,
            'message'             => $message,
    ...
    $original_contact = $this->clients_model->get_clientconstitution($contact_id);
    $success          = $this->clients_model->update_constitution($data, $contact_id);
     
    $message          = '';
     $updated          = false;
    if ($success == true) {
         $updated = true;
         $message = _l('updated_successfully', _l('subfile'));
    }
     if (handle_client_sub_file_upload($contact_id,$customer_id) && !$updated) {
        $message = _l('updated_successfully', _l('subfile'));
        //  $userfile1= $this->db->get_where('tblclient_subfile',array('id'=>$contact_id))->row()->file_name;
            // $contentfile=$this->clientvesioncontents($userfile1,$customer_id);
...
if ($contact_id == '') {
    $title = _l('add_new', _l('subfile'));
} else {
    $data['contact'] = $this->clients_model->get_clientconstitution($contact_id);
    ...
    $title = _l('edit', _l('subfile'));
}
    
$data['document_types']     = $this->clients_model->get_constitution_types();
$data['title']                = $title;
$this->load->view('admin/clients/modals/client_subfile', $data);


}
public function delete_clientconstitution($customer_id, $id)
{
if (!has_permission('customers', '', 'delete')) {
    if (!is_customer_admin($customer_id)) {
        access_denied('customers');
...
$contact      = $this->clients_model->get_clientconstitution($id);
$this->clients_model->delete_clientconstitution($id);

redirect(admin_url('clients/client/' . $customer_id . '?group=constitution'));
}
public function export_client_data($id)
    {
        if (staff_can('create', 'customers')) {
            app_pdf('client-data', LIBSPATH . 'pdf/Customer_data_pdf', $id);
        }
    }
public function change_displayname_ajax($id)
    {
        if ($this->input->is_ajax_request()) {
            echo json_encode($this->clients_model->change_display_name($id, $this->input->post('display_name')));
        }
    }


    public function create_meeting($clientid='') {

        if ($this->input->post()) {
            $data=$this->input->post();
            $subject=$data['subject'];
            $start_date=to_sql_date($data['start_date']);//'2024-08-20T12:00:00Z';
            $end_date=to_sql_date($data['end_date']);//'2024-08-20T12:00:00Z';
            ...
            $start_date1 = new DateTime($start_date, new DateTimeZone('Asia/Kolkata'));
            $start_date2 = $start_date1->format('Y-m-d\TH:i:sP');

            $end_date1 = new DateTime($end_date, new DateTimeZone('Asia/Kolkata'));
            $end_date2 = $end_date1->format('Y-m-d\TH:i:sP');

            



            $addi_email=$data['addi_email'];

            $this->load->library('team_integration');
            $meeting= new team_integration();
            // Call the createMeeting method from the TeamsMeeting library
            $meeting_details=$meeting->createMeeting($start_date2, $end_date2, $subject);
           //print_r($meeting_details);
            if(isset($meeting_details)){
                $insert['clientid']=$clientid;
                $insert['subject']=$data['subject'];
                $insert['start_date']=$start_date2;
                $insert['end_date']=$end_date2;
                $insert['passcode']=$meeting_details['joinMeetingIdSettings']['passcode'];
                $insert['meeting_id']=$meeting_details['joinMeetingIdSettings']['joinMeetingId'];
                $insert['meeting_url']=$meeting_details['joinWebUrl'];
                $insert['addedby']=get_staff_user_id();
                $insert['dateadded']=date('Y-m-d');
    
                if ($addi_email != '') {
                    // Split the emails into an array
                    $emails = explode(';', $addi_email);
                
                    // Iterate over each email and send the email
                    foreach ($emails as $email) {
                        //print_r($email);
                        $email = trim($email); // Remove any surrounding whitespace
                        if (!empty($email)) {
                            $resut = send_mail_template('team_meetinglink_to_client', $email, $insert);
                        }
                    }
                }
                
                if(isset($data['add_contacts'])){
                    $contacts=get_all_contact_user_id($clientid);
                    
                    if($contacts){
                        foreach($contacts as $contact){
                            if($contact['email']!='')
                            $resut1 = send_mail_template('team_meetinglink_to_client',$contact['email'],$insert);
                        }
                    }
                ...
                $inserted = 0;
                if($resut || $resut1){
                    $this->db->insert(db_prefix() . 'client_teams_meeting',$insert);

                    $event['title']                 = 'Team Meeting';
                    $event['description']           = $meeting_details['joinWebUrl'];
                    $event['userid']                = get_staff_user_id();
                    $event['start']                 = $start_date2;
                    $event['end']                   = $end_date2;
                    $event['public']                ='0';
                    $event['color']                 ='#28B8DA';
                    $event['isstartnotified']       ='1';
                    $event['reminder_before']       ='30';
                    $event['reminder_before_type']  ='minutes';

                    $this->db->insert(db_prefix() . 'events',$event);
                    $inserted = 1 ;
                }
            ...
            if($inserted==1){
                set_alert('success', _l('meeting_url_sent_successfully'));
            }else{
                set_alert('danger', _l('meeting_url_not_sent_successfully'));
            }
    ...
    public function table_team_meeting($clientid='') {
        if (!has_permission('customers', '', 'view')) {
            if (!have_assigned_customers() && !has_permission('customers', '', 'create')) {
                ajax_access_denied();
            }
        ...
        $this->app->get_table_data('team_meeting', [
            'clientid' => $clientid
        ]);
    }
}
