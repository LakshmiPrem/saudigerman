...
class Proposals extends AdminController
{
    ...
    public function proposal($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    // $this->fee_proposal($id,$proposal_data['rel_type'],$proposal_data['rel_id']);
        ...
        $data['matter_templates']         = $this->casetemplate_model->get();
        $this->load->model('templates_model');

        $data['proposal_types'] = $this->templates_model->get('',array('type'=>'proposals'));
    ...
    public function get_due_date()
    {
     ...
     public function fee_proposal($proposal_id,$rel_type,$rel_id){
        ...
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('temp/Fee Proposal.docx');
        ...
        if($rel_type=='customer'){
            ...
            $client_name = $this->db->get_where('tblleads',array('id'=>$rel_id))->row()->name;
        }
        ...
        $submitted_to = $proposal->proposal_to ;
        $max_hours = $proposal->max_hours ;
        $matter = get_matter_template_name_by_id($proposal->matter_template_id);
        $scope_of_services = nl2br($proposal->scope_of_services);
        $breaks = array("<br />","<br>","<br/>");  
      

        $templateProcessor->setValue('client_name', $client_name);
        $templateProcessor->setValue('submitted_to',$submitted_to);
        $templateProcessor->setValue('company_name', get_option('invoice_company_name'));


        $company_name = get_option('invoice_company_name');
        $templateProcessor->setValue('subtotal', $subtotal);
        $templateProcessor->setValue('subject', $matter);
        //$proposal_description  = str_ireplace($breaks,'</w:t><w:br/><w:t>', $proposal_description);

        //$templateProcessor->setValue('amount_in_words', $amount_in_words);
        $scope_of_services  = str_ireplace($breaks,'</w:t><w:br/><w:t>', $scope_of_services);
        $scope_of_services = str_replace('&','&amp;', $scope_of_services) ;
        $templateProcessor->setValue('scope_of_services',$scope_of_services);
        $templateProcessor->setValue('max_hours',$max_hours);
        $num_word = "num_word_".$max_hours;
        $max_hours_in_words = _l($num_word); 
        $templateProcessor->setValue('max_hours_in_words',$max_hours_in_words);
        //$templateProcessor->setValue('discussion_with',$discussed_with);
        //$templateProcessor->setValue('discussed_on',$discussed_on);

       

        //
        $path        = get_upload_path_by_type('fee_proposal').$proposal_id.'/';
        _maybe_create_upload_path($path);
        if(file_exists($path.'Fee Proposal.docx'))
            unlink($path.'Fee Proposal.docx');
        $templateProcessor->saveAs($path.'Fee Proposal.docx');

    }


     public function get_matter_convert_data($id)
    {
        $this->load->model('payment_modes_model');
        ...
        $data['staff']                 = $this->staff_model->get('', 1);
        $data['case_type'] = $this->db->get_where('tblcasetemplates ',array('id'=>$data['proposal']->matter_template_id))->row()->case_type;
        //$data['project_templates']     = $this->projects_model->get_project_templates();
        $data['casetemplates'] = get_case_templates($data['case_type']);
        $data['matter_title'] = get_matter_template_name_by_id($data['proposal']->matter_template_id);
        $this->load->view('admin/proposals/matter_convert_template', $data);
    }

    public function convert_to_matter($id)
    {
        ...
        if ($this->input->post()) {
            ...
            $submit_data['proposal_id'] = $id;
            $matter_id = $this->projects_model->add($submit_data);
            if ($matter_id) {
                set_alert('success', _l('proposal_converted_to_invoice_success'));
                $this->db->where('id', $id);
                $this->db->update('tblproposals', array(
                    'converted_to_matter' => 'y',
                    'status' => 3,
                ));
                // Delete All scopes 
                $this->db->where('case_id',$matter_id);
                $this->db->delete('tblcase_scopes');
                //Update Scopes of proposal 

                $executive_summary = $this->db->get_where('tblproposals',array('id'=>$id))->row()->scope_of_services;
                $str = preg_replace('/[\n]+/m', '', $executive_summary); 
                $s = nl2br($str);
                $sr = explode('<br />', $s);
                $proposal_scopes =  array_filter($sr, function($v) { return !is_null($v); });
                foreach ($proposal_scopes as $scope) {
                    $data['scope_description'] = $scope;
                    $data['case_id']  = $matter_id;
                    if(strlen(trim($scope)) > 0) 
                        $this->db->insert('tblcase_scopes',$data);
                    else
                        continue;
                }

                log_activity('Proposal Converted to Case [Case ID: ' . $matter_id . ', Proposal ID: ' . $id . ']');
                //do_action('proposal_converted_to_invoice', array('proposal_id'=>$id, 'invoice_id'=>$matter_id));
                redirect(admin_url('projects/view/' . $matter_id));
            } else {
                set_alert('danger', _l('proposal_converted_to_invoice_fail'));
            }
            if ($this->set_proposal_pipeline_autoload($id)) {
