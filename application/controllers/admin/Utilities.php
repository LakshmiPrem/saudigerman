...
class Utilities extends AdminController
{
    ...
    {
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('all_templates');
        }

		$data['staffs'] = $this->staff_model->get('', ['active' => 1]);
        $data['title']                = _l('als_template');
        $this->load->view('admin/utilities/manage_templates', $data);  
    }
	/* All templates By Category */
	public function all_clauses()
    {
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('all_clauses');
        }

		$data['staffs'] = $this->staff_model->get('', ['active' => 1]);
        $data['title']                = _l('als_clauses');
        $this->load->view('admin/utilities/manage_clauses', $data);  
    }
}
