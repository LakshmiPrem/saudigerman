...
class Misc extends AdminController
{
    ...
    public function send_sub_file()
    {
        //print_r($this->input->post());
        if ($this->input->post('send_sub_file_email')) {
            if ($this->input->post('file_path')) {
                $this->load->model('emails_model');
                $this->emails_model->add_attachment([
                    'attachment' => $this->input->post('file_path'),
                ...
                $message = $this->input->post('send_sub_file_message');
                $message = nl2br($message);
                $success = $this->emails_model->send_simple_email($this->input->post('send_sub_file_email'), $this->input->post('send_sub_file_subject'), $message);
                if ($success) {
                    set_alert('success', _l('custom_file_success_send', $this->input->post('send_sub_file_email')));
                } else {
                    set_alert('warning', _l('custom_file_fail_send'));
                }
            }
    ...
    public function add_quick_reminder(){
        if ($this->input->post()) {
            if (!$this->input->post('id')) {
                $data = $this->input->post();
                $id = $this->misc_model->add_reminder($this->input->post(), $this->input->post('rel_id'));
                if ($id) {
                    $success = true;
                    $message = _l('reminder_added_successfully');
                }
    ...
    public function contact_email_exists()
    {
	 ...
	 public function sent_whatsapp_test_sms()
    {
        if ($this->input->post()) {
            ...
            $template->subject  = 'Whatsapp Message Testing';

            $template = parse_email_template($template);

            hooks()->do_action('before_send_test_whatsapp_sms');
     				$number = $this->input->post('test_contact');
                    // $number = "918157071336";
                    $msg = get_option('whatsapp_instance_msg');
                    // $msg = "test";
            ...
            if ($result) {
                set_alert('success', 'Seems like your Whatsapp settings is set correctly. Check your Whatsapp now.');
                hooks()->do_action('whatsapp_test_sms_success');
            } else {
                ...
                hooks()->do_action('whatsapp_test_sms_failed');
            }
        }
    }
    ...
    public function dashboard_box(){
        ...
        if ($this->input->post()) {
            ...
            $hearing_type  =   get_court_instance_name_by_id($this->input->post('h_instance_id')); 
            ...
            if ($id == '') {
                ...
                handle_hearing_attachment($id,$project_id); 
                /* notify to client */
        if($this->input->post('notify_by_email')){
            
            $contact=$this->db->get_where('tblclients',array('userid'=>$project->clientid))->email_id;
           $template = mail_template('project_hearing_file_to_customer', $project, $contact,$id);
             if ($template->send()) {
                        $sent = true;
                    }
        }
                echo json_encode([
                    ...
                    'id'       => $id,
                    'message'  => $message,
                    'leadView' => $id ? $this->_get_hearing_data() : [],
           ...
           $template = mail_template('project_hearing_file_to_customer', $project, $contact,$id);
             if ($template->send()) {
                ...
                echo json_encode([
                    ...
                    'message'          => $message,
                    'id'               => $id,
                    'leadView'         => $this->_get_hearing_data(),
                    'hearing_type'     =>$hearing_type,
                ]);
            }
            die;
        ...
        echo json_encode([
            'leadView' => $this->_get_dashboard_box_data(),
        ]);
    }

     private function _get_dashboard_box_data()
    {
        $reminder_data         = '';
        $data['dashboard_box'] =  $this->db->get('tbldashboard_boxes')->result_array();
        return [
            'data'          => $this->load->view('admin/dashboard/my_dashboard_box', $data, true),
            //'reminder_data' => $reminder_data,
        ];
    }

    public function change_dash_box_status($box_id,$box_status){
        $this->db->where('id',$box_id);
        $this->db->update('tbldashboard_boxes',['box_status' => $box_status]);
    }

    public function change_box_order(){
        $id    = $this->input->post('box_id');
        $order = $this->input->post('box_order');

        
        //                     $this->db->select('id,box_order,box_default_order');
        //                     $this->db->from('tbldashboard_boxes');
        // $data['boxOrder'] = $this->db->get()->result_array();

        // $boxOrder = array_column($data['boxOrder'], 'box_order');
        //if (!in_array($order, $boxOrder)){
            $this->db->where('id',$id);
            $this->db->update('tbldashboard_boxes',['box_order' => $order]);
        //}
        

    }

    public function add_quick_chatgpt(){
		$success=false;
        if ($this->input->post()) {
                $data = $this->input->post();
        ...
        $prompt =$this->input->post('prompt_text'); // Get the user's input
	...
	public function update_attachfile_content($id,$rel_id,$rel_type)
    {
        ...
        if ($this->input->is_ajax_request()) {
			...
			$sharegraph->download_updatefile($rel_id,$attachment->file_name,$rel_type);
		//	$this->projectvesioncontents($attachment->file_name,$project_id);
			$contentfile=projectdocumentcontents($attachment->file_name,$rel_id,$rel_type);
			 $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'files', [
