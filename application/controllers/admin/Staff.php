...
class Staff extends AdminController
{
    ...
    public function member($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                handle_staff_profile_image_upload($id);
                  handle_esignature_upload($id);//digital signature upload
        ...
        if ($id == '') {
		 ...
		 if ($this->input->get('lawyer')) {
            $data['lawyer'] = $this->input->get('lawyer');
        }
    ...
    public function remove_digital_signature($id = '')
    {
        ...
        hooks()->do_action('before_remove_staff_profile_image');
        $member = $this->staff_model->get($staff_id);
        if (file_exists(get_upload_path_by_type('esignature') . $staff_id)) {
            delete_dir(get_upload_path_by_type('esignature') . $staff_id);
        }
        $this->db->where('staffid', $staff_id);
        $this->db->update(db_prefix() . 'staff', [
            'esignature' => null,
