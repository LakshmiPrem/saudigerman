...
class Dashboard extends AdminController
{
    ...
    public function index($dashtype='contract')
    {
        close_setup_menu();
		...
		$data['contracts_status_stats'] = json_encode($this->dashboard_model->contracts_status_stats());
		...
		$data['contracts_activity'] = $this->contracts_model->get_activity('', hooks()->apply_filters('projects_activity_dashboard_limit', 20));
        add_calendar_assets();
 ...
 $data['contracts_payable_count'] = $this->contracts_model->count_status_contracts();
       $data['contracts_receivable_count'] = $this->contracts_model->count_status_contracts('is_receivable');
        $data['contracts_others_count'] = $this->contracts_model->count_status_contracts('trash');
        ...
        $data['clients_po'] = $this->clients_model->get('', [
                'tblclients.active' => 1,
                'tblclients.ctype'  => 'po']);
        ...
        $data['active_boxes'] = array_column($this->db->select('id')->get_where('tbldashboard_boxes',['box_status'=>1])->result_array(),'id');
                            $this->db->order_by('box_order', 'ASC');
	...
	public function pagination()
    {
      $q = $date = '';
      if($this->input->post()){
        $q= to_sql_date($this->input->post('q'));
		   $q1= to_sql_date($this->input->post('q1'));
		   $dash = $this->input->post('dashtype');
     ...
     //  'pagination_link'  => $this->pagination->create_links(),
       'client_data'   => $this->dashboard_model->fetch_details($q,$q1,$dash),
