...
class Email extends REST_Controller {
    ...
    */
    public function index_post()
    {
		...
		//	print_r($input);	
			$subfileno=$this->api_casediary_model->find_sub_matter_id($input['subject']);
		//	print_r($subfileno);
			if($subfileno=='')
			$input['matter_related']=$this->api_casediary_model->get_export_projectfileno($input['matter_id']);
			else
			 $input['matter_related']=$subfileno;
            ...
            if ($inserted)
            {
                if($input['related_to'] == 'matter'){
                    handle_communinication_outlook_attachment($inserted,$input['matter_id']);
