        ...
        if (contactid != '') {
            $.post(admin_url + 'tickets/ticket_change_data/', {
                ...
                if (response.contact_data) {
					// $('input[name="name"]').val(response.company_name);
...
function show_ticket_no_contact_email_warning(userid, contactid) {
    if ($('#contact_email_notifications_warning').length == 0) {
        $('#new_ticket_form, #single-ticket-form').prepend('<div class="alert alert-warning hide" id="contact_email_notifications_warning">Email notifications for tickets is disabled for this contact, if you want the contact to receive ticket emails you must enable by clicking <a href="' + admin_url + 'clients/client/' + userid + '?contactid=' + contactid + '" target="_blank">here</a>.</div>');
    }
}
