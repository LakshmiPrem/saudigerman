Dropzone.options.projectFilesUpload = false;
  ...
  $(function() {
    ...
    initDataTable('.table-project-judgement', admin_url + 'projects/judgement_table/' + project_id, undefined, undefined, 'undefined', [1, 'desc']);
  ...
  if ($('#project-expense-form').length > 0) {
	  ...
	  appValidateForm($('#courtorder_form'), {

          corder_name: 'required',
      ...
      $('#discussion').on('hidden.bs.modal', function(event) {
          ...
          $d.find('input[name="meeting_date"]').val('');
 ...
 function manage_courtorder(form) {
      ...
      $.post(url, data).done(function(response) {
          ...
          $('.table-project-courtorder').DataTable().ajax.reload(null, false);

        //  $('#discussion').modal('hide');

          $('#courtorder_form').find('button[type="submit"]').button('reset');
  ...
  function edit_discussion(invoker, id) {
      ...
      $('#discussion input[name="meeting_date"]').val($(invoker).data('meeting_date'));
 ...
 appValidateForm($('#courtorder-form'), {
      ...
      }, projectCourtorderSubmitHandler);

function projectCourtorderSubmitHandler(form) {
 ...
 appValidateForm($('#project-instance-form'), {
      ...
      }, projectInstanceSubmitHandler);

function projectInstanceSubmitHandler(form) {
...
function new_projectsub() {
	
      $('#projectsub').modal('show');
	$('#projectsub .edit-title').addClass('hide');
  }
 appValidateForm($('#project-subfile-form'), {
          subfiling_date: 'required',
         subject: 'required',
        document_type: 'required',
	//pop_attachment:'required',
		//  documentid:'required'
      }, projectSubfileSubmitHandler);
 function projectSubfileSubmitHandler(form) {
    ...
    $.ajax({
        ...
        url: formURL
		  ...
		  if (response.id) {
              if (response.success == true) {
            alert_float('success', response.message);
				//   $('.table-project-files').DataTable().ajax.reload(null, false);
        }
			  $('#project-subfile-form').find('button[type="submit"]').button('reset');  
			   $('#projectsub').modal('hide');
  ...
  function update_file_data(id) {
      ...
      data.subject = $('body input[name="_file_subject"]').val();

      data.description = $('body textarea[name="_file_description"]').val();

      data.issue_date = $('body input[name="_issue_date"]').val();

      data.expiry_date = $('body input[name="_expiry_date"]').val();

	  data.document_type = $('body select[name="_document_type"]').val(); 
  ...
  function project_files_bulk_action(e) {
      ...
      var mass_mail = $('#mass_mail').prop('checked');
      ...
      if (confirm_delete()) {
        ...
        if(mass_mail){
            ...
            data.email_subject     = $('#send_file_subject_').val();

            data.send_file_email   = $('#send_file_email_').val();

            data.bcc               = $('#bcc_').val();

            data.send_file_message = $('#send_file_message_').val();
            ...
            setTimeout(function() {
                ...
                $.post(admin_url + 'projects/bulk_email_files', data).done(function() {
...
$('#mass_mail').change(function(){
  ...
  $('#b_delete_div').toggleClass("hide");

  $('.mass_mail_separator').toggleClass('hide');

});



function edit_case_update(id) {

    //var description = $("body").find('[data-note-edit-textarea="' + id + '"] textarea').val();

	

    var description =document.getElementById("content"+id).value;// tinymce.get("content"+id).getContent();

    if (description != '') {

        $.post(admin_url + 'casediary/edit_case_update/' + id, {
...
function delete_case_update(wrapper, id) {

    if (confirm_delete()) {

        requestGetJSON('casediary/delete_case_update/' + id).done(function(response) {
 ...
 function view_project_versionfile(id, $project_id) {
	
      $('#project_subfile_data').empty();
      $("#project_subfile_data").load(admin_url + 'projects/versionfile/' + id + '/' + project_id, function(response, status, xhr) {
          if (status == "error") {
              alert_float('danger', xhr.statusText);
          }
      });
  }
function update_subfile_data(id) {
	 
      var data = {};
      data.id = id; 
      data.subject = $('body input[name="_file_subject1"]').val();
      data.description = $('body textarea[name="_file_description1"]').val();
      data.issue_date = $('body input[name="_issue_date1"]').val();
      data.expiry_date = $('body input[name="_expiry_date1"]').val();
	  data.document_type = $('body select[name="_document_type"]').val(); 
	  $.post(admin_url + 'projects/update_subfile_data/', data);
  }
