...
$(function () {
    ...
    init_ajax_search('customer', '#clientid2.ajax-search');
    ...
    if (table_tasks.length) {
		...
		TasksServerParams['taskrelated_to'] = '[name="taskrel_type"]';
        TasksServerParams['taskrel_id'] = '[name="taskrel_id"]';
        // Tasks not sortable
        ...
        _table_api = initDataTable(table_tasks, tasksTableURL, tasksTableNotSortable, tasksTableNotSortable, TasksServerParams, [1, 'asc']);//table_tasks.find('th.#').index()
    ...
    $('#send_file').on('show.bs.modal', function (e) {
        ...
        $sendFile.find('input[name="send_file_subject"]').val($($(e.relatedTarget)).data('file-name'));
    ...
    $("body").on('submit', '#sales-notes', function () {
        ...
        $.post(form.attr('action'), $(form).serialize()).done(function (rel_id) {
            ...
            }else if (form.hasClass('chequebounce-notes-form')) {
                get_sales_notes(rel_id, 'chequebounces');
            }else if (form.hasClass('legalrisk-notes-form')) {
                get_sales_notes(rel_id, 'legal_risks');
            }
        });
        return false;
    });
...
function initDataTable(selector, url, notsearchable, notsortable, fnserverparams, defaultorder,defaultread=null) {
  ...
  createdCell: function(cell) {
        ...
        if(!empty($content) &&(contentLength>50)){
        $(cell).append($("<button>Show more</button>"));
        }
...
function do_filter_active(value, parent_selector) {
    if (value !== '' && typeof (value) != 'undefined') {
        ...
        var selector = $('[data-cview="' + value + '"]');//alert(selector);
        ...
        return value;
           ...
           $.each($('._filters input'),function(){
            //alert($(this).attr('name'));
        if($(this).attr('name') != 'case_type'){
            $(this).val('');  
            } 
...
function do_filter_active_old(value, parent_selector) {
    if (value !== '' && typeof (value) != 'undefined') {
...
function init_table_tickets(manual) {
    ...
    TicketServerParams['project_id'] = '[name="project_id"]';
	 TicketServerParams['service_type'] = '[name="service_type"]';
    ...
    if (_table_api && $("body").hasClass('dashboard')) {
        ...
        var notVisibleDashboardDefault = [tickets_date_created_index];
...
function tasks_bulk_action(event) {
    if (confirm_delete()) {
        ...
        if (mass_delete == false || typeof (mass_delete) == 'undefined') {
            ...
            var handover = $('#is_handover').prop('checked');
			data.is_handover=handover;
			
			 var notifyclient = $('#is_notifyclient').prop('checked');//alert(notifyclient);
			data.is_notifyclient=notifyclient;
			
			data.reason_handover = $('#reason_handover').val();
			data.expected_returndate = $('#expected_returndate').val();
        ...
        setTimeout(function () {
            if(handover){
                var fd = new FormData();
                var files = $('#handover_document')[0].files;
                fd.append('handover_document',files[0]);

                var csrf_token_name = csrfData.hash;
                fd.append('csrf_token_name',csrf_token_name);

                assignees = assignees.length ? assignees.selectpicker('val') : '';
                reason_handover = $('#reason_handover').val();
                expected_returndate = $('#expected_returndate').val();
                tags = tags_bulk.length ? tags_bulk.tagit('assignedTags') : '';

                fd.append('assignees',assignees);
                fd.append('ids',ids);
                fd.append('reason_handover',reason_handover);
                fd.append('expected_returndate',expected_returndate);
                fd.append('tags',tags);

                $.ajax({
                     url: admin_url + 'tasks/handover_attachment',
                     type:'post',
                     data:fd,
                     dataType: 'json',
                     contentType: false,
...
function add_clause(rel_type, rel_id) {
    $('#modal-wrapper1').load(admin_url + 'templates/closuremodal', {
        slug: 'new',
        rel_type: rel_type,
        rel_id: rel_id,
    }, function () {
        if ($('#ClosureModal').is(':hidden')) {
            $('#ClosureModal').modal({
                backdrop: 'static',
                show: true
            });
     ...
     appValidateForm($('#closure-form'), {
            name: 'required'
        });
        tinymce.remove('#content1');
        init_editor('#content1');
    });
}
...
function validate_template_form() {
    ...
    appValidateForm($('#template-form'), validationObject, template_form_handler, messages);
}
// Lead profile data function form handler
    ...
    $.ajax({
		...
		response = JSON.parse(response);
            ...
            alert_float('success', response.message);
			  ...
			  $('#template-form').find('button[type="submit"]').button('reset');  
			   $('#template-form').modal('hide');
...
function insert_template(wrapper, rel_type, id) {
    requestGetJSON(admin_url + 'templates/index/' + id).done(function (response) {
        ...
        if (rel_type == 'proposals') {
            ...
            $('a[aria-controls="tab_contract"]').click()
        }
...
function edit_alltemplate(id, rel_id) {
    $('#modal-wrapper').load(admin_url + 'templates/modal', {
        slug: 'edit',
        id: id,
        rel_type:'contracts',
...
function delete_alltemplate(id) {
    if (confirm_delete()) {
        $.post(admin_url + 'templates/delete/' + id).done(function (response) {
            response = JSON.parse(response);
...
function edit_allclosure(id, rel_id) {
    $('#modal-wrapper1').load(admin_url + 'templates/closuremodal', {
        slug: 'edit',
        id: id,
        rel_type:'contracts',
...
function init_dash_box() {
    if ($('#task-modal').is(':visible')) {
        $('#task-modal').modal('hide');
    }
    // In case header error
    if (init_dash_box_modal_data()) {
        $('#dashboard-box-modal').modal('show');
    }
   }
   // Fetches lead modal data, can be edit/add/view
function init_dash_box_modal_data() {
    
    var requestURL = 'misc/dashboard_box/';//(typeof (url) != 'undefined' ? url : 'misc/dashboard_box/') + (typeof (id) != 'undefined' ? id : '')+'?projectid='+project_id;
    var str = requestURL;
    var res = str.charAt(str.length-1);
    if(res == '/')
      //requestURL += '?hearing_type='+$("#myTab li.active").attr('id')+'&projectid='+project_id;
    requestGetJSON(requestURL).done(function (response) {
        _dash_box_init_data(response);
    }).fail(function (data) {
        alert_float('danger', data.responseText);
    });
}
...
// Add lead data returned from server to the lead modal
function _dash_box_init_data(data) {   
    var hash = window.location.hash;
    var $hearingModal = $('#dashboard-box-modal');
    //$('#lead_reminder_modal').html(data.leadView.reminder_data);
...
// Lead form validation
function validate_dash_box_form() {
    var validationObject = {
        subject: 'required',
    };

    var messages = {};
    appValidateForm($('#dash-box-form'), validationObject, dash_box_form_handler, messages);
}
// Lead profile data function form handler
function dash_box_form_handler(form) {
   //  var data = form.serialize();
    form = $(form);
    var formURL = $(form).attr("action");
    var formData = new FormData($(form)[0]);
...
$('#ocr_btn_upload').click(function(){
      var fd = new FormData();
      var files = $('#ocr_file_choose')[0].files[0];
      fd.append('ocr_file',files);
      var csrf_token_name = csrfData.hash;
      fd.append('csrf_token_name',csrf_token_name);
      
      $.ajax({
        url: admin_url + 'ocr/ocr',
        type:'post',
        data:fd,
        dataType: 'json',
        contentType: false,
        processData: false,
        success:function(response){
            //$(".ocr_text").val(response.text);
            tinyMCE.activeEditor.setContent(response.text);
        }
   });
});
appValidateForm($('#quick_chatgpt-form'), {
        prompt_text:'required'
		 
      }, manage_chatgpt_shares);
  function manage_chatgpt_shares(form) { 
    var data = $(form).serialize();
    ...
    $.post(url, data).done(function(response) {
        ...
        if(response.success == true){ 
			$('#gptresult').removeClass('hide');
         //   alert_float('success',response.message);
    	// console.log(response.message);
		
		 var editor = tinyMCE.activeEditor;
		editor.setContent(response.message);
			//$('#prompt_textresult').val(response.message);
        }else{
		  alert_float('warning',response.message);
    	 alert(response.message);	
		}
     //  window.location.reload();
       // $('#model_sharetask').modal('hide');
    });
    return false;
}
function new_quick_chatgpt(clientid='',company=''){
...
/*	if(clientid!=''){
    ...
    $('#quick_chatgpt').modal('show');
    $('.edit-title').addClass('hide');
}
function get_prompt_chattemplate_ajax() { 
		
        var mtypeSelected = $('select[name="contract_id"]').val();
        if(mtypeSelected>0){
			
            $.get(admin_url + 'contracts/get_prompt_of_contract/'+mtypeSelected,function(response){
               // var ctype = $('select[name="template_id"]');
              // alert(mtypeSelected);alert(response);
                if(response ){
                  
					// var editor = tinyMCE.activeEditor;
					//editor.setContent(response.message);
					$('#prompt_text').val( response);
