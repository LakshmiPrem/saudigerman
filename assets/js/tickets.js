$(function() {
    ...
    $('#ticket_no_contact').on('click', function(e) {
        ...
        $('#email').val('').rules('add', { required: true });
		 $('#service').rules('add', { required: true });
         $(this).addClass('hide');
    ...
    vRules = {
       // client_id:'required',
		makani_land:'required',
		outstandingamount:'required',
		returncheque_amount:'required',
		totalamount:'required',
		current_credit_appamount:'required',
		current_credit_days:'required',
		total_outstanding_amount:'required',
		
		//address:'required',
		  
    }

    _validate_form($('#civil-ticket-form'), vRules);
		 // Admin ticket civil case add
    $('.add_civilcase_ticket').on('click', function(e) {
	
         e.preventDefault();
		
        var data = {};

        var $settingsArea = $('#civilrequest');
        var errors = false;
		 if ($settingsArea.find('input[name="makani_land"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="makani_land"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="makani_land"]').parents('.form-group').removeClass('has-error');
        }
		if ($settingsArea.find('input[name="outstandingamount"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="outstandingamount"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="outstandingamount"]').parents('.form-group').removeClass('has-error');
        }
		
		if ($settingsArea.find('input[name="totalamount"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="totalamount"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="totalamount"]').parents('.form-group').removeClass('has-error');
        }
		if ($settingsArea.find('input[name="returncheque_amount"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="returncheque_amount"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="returncheque_amount"]').parents('.form-group').removeClass('has-error');
        }
		if ($settingsArea.find('input[name="current_credit_appamount"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="current_credit_appamount"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="current_credit_appamount"]').parents('.form-group').removeClass('has-error');
        }
		if ($settingsArea.find('input[name="current_credit_days"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="current_credit_days"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="current_credit_days"]').parents('.form-group').removeClass('has-error');
        }
		if ($settingsArea.find('input[name="total_outstanding_amount"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="total_outstanding_amount"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="total_outstanding_amount"]').parents('.form-group').removeClass('has-error');
        }
		if ($settingsArea.find('textarea[name*="owner_detail[contact1][]"]').val() == '') {
            errors = true;
            $settingsArea.find('textarea[name*="owner_detail[contact1][]"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('textarea[name*="owner_detail[contact1][]"]').parents('.form-group').removeClass('has-error');
        }
        ...
        data = $('#civil-ticket-form').serialize();
        data += '&ticketid=' + $('input[name="ticketid"]').val();
		
        if (typeof(csrfData) !== 'undefined') {
            data += '&' + csrfData['token_name'] + '=' + csrfData['hash'];
        }
        $.post(admin_url + 'tickets/save_single_ticket_civil', data).done(function(response) {
            ...
            if (response.success == true) {
                ...
                if (typeof(response.message) !== '') {
					 alert_float('success', response.message);
					 window.location.reload();
                  //  window.location.href = response.url;//admin_url + 'tickets/';
	 ...
	 // Admin ticket civil case add
    $('.add_policecase_ticket').on('click', function(e) {
	
         e.preventDefault();
		
        var data = {};

        var $settingsArea = $('#policerequest');
        var errors = false;
		
		
		if ($settingsArea.find('input[name="amount_filed_case"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="amount_filed_case"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="amount_filed_case"]').parents('.form-group').removeClass('has-error');
        }
		if ($settingsArea.find('input[name="amount_soa"]').val() == '') {
            errors = true;
            $settingsArea.find('input[name="amount_soa"]').parents('.form-group').addClass('has-error');
        } else {
            $settingsArea.find('input[name="amount_soa"]').parents('.form-group').removeClass('has-error');
        }
        ...
        data = $('#police-ticket-form').serialize();
        data += '&ticketid=' + $('input[name="ticketid"]').val();
		
        if (typeof(csrfData) !== 'undefined') {
            data += '&' + csrfData['token_name'] + '=' + csrfData['hash'];
        }
        $.post(admin_url + 'tickets/save_single_ticket_police', data).done(function(response) {
    ...
    $('.save_changes_settings_single_ticket').on('click', function(e) {
        ...
        data = $('#closecase-ticket-form').serialize();
        data += '&ticketid=' + $('input[name="ticketid"]').val();
        if (typeof(csrfData) !== 'undefined') {
            data += '&' + csrfData['token_name'] + '=' + csrfData['hash'];
        }
	...
	/*	
		...
		alert('suc');
			...
			alert_float('warning', 'All Attachments are Mandatory');	
			return false;
